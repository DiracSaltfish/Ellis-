...
func New(cfg Config) (*Service, error) {
	...
	s := &Service{cfg: cfg, store: st, candidates: u.Candidates, baskets: map[string]iopv.Basket{}, quotes: map[string]Quote{}, latest: map[string]Point{}, pending: map[string]map[string]Point{}, errors: map[string]string{}, feedState: "starting", refresh: make(chan struct{}, 1), restart: make(chan struct{}, 1), runID: fmt.Sprint(time.Now().UnixNano()), started: time.Now()}
	return s, nil
}
	...
	for w := 0; w < 4; w++ {
		...
		go func() {
			...
			for c := range jobs {
				...
				if _, e = s.store.DB.Exec("INSERT OR IGNORE INTO pcf VALUES(?,?,?,?,?)", b.Symbol, b.Date, b.Hash, raw, time.Now().Format(time.RFC3339Nano)); e != nil {
...
func (s *Service) RefreshFX(ctx context.Context) {
	...
	s.mu.Unlock()
	s.setError("fx", nil)
	_, e = s.store.DB.Exec("INSERT OR IGNORE INTO fx VALUES(?,?,?)", snapshot.Generated.Format(time.RFC3339Nano), snapshot.Date, raw)
...
func (s *Service) feedLoop(ctx context.Context) {
	...
	for {
		...
		s.mu.RLock()
		symbols := append([]string(nil), s.plan.Subscriptions...)
		s.mu.RUnlock()
		if len(symbols) == 0 {
			select {
			case <-ctx.Done():
