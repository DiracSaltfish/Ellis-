...
from ._task_id import TaskIdGenerator
...
def Close():
    ...
    try:
        ...
        # The official wrapper releases its process-global API object.  A
        # later Login in the same interpreter must therefore create a fresh
        # transport instead of reusing a backend in CLOSED state.
        _g_backend = None


def GetVersion():
    """Return the official vendor client version without opening a connection."""
    return _b.get_client_version()


def GetTaskID():
    """返回本地 ``MMDDHHmmSS + 序号`` 格式的唯一任务号。"""
    return _task_id_generator.next()
...
def QuerySnapshot(req_snapshot, query_spi=None, return_df_format=True):
    """Query the independently verified internet L1 snapshot subranges.

    Current targets are SZSE ``159518`` and SSE ``510300``, each restricted
    to ``data_type=0`` and ``level_type=0``.  Other public Snapshot variants
    remain explicitly unsupported until their own evidence exists.
...
def QueryETFInfo(req_etf_info_cfg, query_spi=None, return_df_format=True):
    """对齐官方 QueryETFInfo 同步子范围（已验证：单条沪/深市 ETF 样本）。
...
def QuerySecuritiesInfo(req_security_info_cfg, query_spi=None, return_df_format=True):
    """对齐官方 ``QuerySecuritiesInfo`` 的同步窄范围。

    已验证：SSE ``510300`` 或 SZSE ``159919`` 单项，以及输入顺序固定的
    ``[SSE 510300, SZSE 159919]`` 双项。返回容器与官方 json 格式一致：
    ``list[dict]``，每行 43 键（``MDCodeTableRecord``）。批量响应沿服务端
    顺序返回（官方实测为 SZSE 后 SSE），不得按请求顺序重排。数值保持服务端
    原样（缩放由调用方按头文件注释处理）；异步 SPI 未实现。
    """
    if query_spi is not None:
        raise NotImplementedError(
            "asynchronous query SPI callbacks are not implemented on macOS"
    ...
    items_in = (
        req_security_info_cfg
        if isinstance(req_security_info_cfg, list)
        else [req_security_info_cfg]
    )
    if len(items_in) not in (1, 2):
        raise NotImplementedError(
...
def _QueryThirdInfoPage(task_id, *, offset=0, count=1000, return_df_format=False):
    """Internal one-page ThirdInfo fetch used by audited high-level wrappers.

    The public ``QueryThirdInfo`` contract remains a default-page request.
    High-level compatibility code may use this helper only after it has an
    official output contract that requires paged accumulation.
    """
    if (isinstance(offset, bool) or not isinstance(offset, int) or offset < 0
            or isinstance(count, bool) or not isinstance(count, int) or count <= 0):
        raise ValueError("third-info offset/count must be non-negative/positive integers")
    be = _backend()
    params = getattr(be, "_third_params", {}).pop(int(task_id), {})
    result = be.query("third_info", {
        "task_id": task_id,
        "params": params,
        "_offset": offset,
        "_count": count,
