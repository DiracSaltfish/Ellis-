...
# The official Linux SDK exposes this exact client build string through
# ``GetVersion()`` and sends the same value in ``ReqLogon.Version``.  Keep one
...
def get_client_version() -> str:
    ...
    return os.environ.get("TGW_CLIENT_VERSION", DEFAULT_TGW_CLIENT_VERSION)
...
class LiveBackend(BaseBackend):
    ...
    def build_query(self, kind: str, req: dict[str, Any]) -> tuple[int, bytes, Any, list[str]]:
        ...
        if kind == "third_info":
            ...
            offset = req.get("_offset", 0)
            count = req.get("_count", 1000)
            if (isinstance(offset, bool) or not isinstance(offset, int)
                    or isinstance(count, bool) or not isinstance(count, int)):
                raise ValueError("invalid third-info pagination")
    ...
    def _query_etf_info(self, _public_task_id: int, items: Any) -> Any:
        """Run one ETF info exchange on the *persistent push* connection.

        Captured official behavior (independent SSE and SZSE samples): the request and its
        ...
        # Unlike one-shot historical queries, this codelist request does not
        # put the public TaskID on the push wire.  Its independent sequence
        # starts at 1 in the official client and the response echoes that id.
        wire_request_id = self.client.next_codelist_request_id()
        payload = build_etf_info_request(
            self.client.username, self.client.token, wire_request_id, items
        )
        packets = self.client.request_many(
            wire_request_id, payload, done=lambda _message: True, timeout=self.timeout
        )
        try:
            self.client.send(build_etf_codelist_complete_request(
                self.client.username, self.client.token, wire_request_id
            ))
        except TgwTransportError:
            pass  # parse what we received even if the push reader just stopped
        return parse_etf_info_packets(packets, expected_request_id=wire_request_id)

    def _query_securities_info(self, _public_task_id: int, items: Any) -> Any:
        ...
        and no packet counters. Each verified single-item or two-item request
        had one response frame; that is all we accept.
        """
        if not isinstance(items, list):
            raise ValueError("invalid securities-info request items")
        # This request family shares the persistent codelist channel with ETF
        # information.  The official client puts an independent low sequence
        # on the wire; the public MMDDHHmmSS task id is not the response key.
        wire_request_id = self.client.next_codelist_request_id()
        ...
        try:
            ...
            pass  # parse what we received even if the push reader just stopped
        return parse_secinfo_packets(packets, expected_request_id=wire_request_id)
