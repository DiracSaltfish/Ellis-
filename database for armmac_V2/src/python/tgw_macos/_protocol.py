...
_VERIFIED_SNAPSHOT_TARGETS = frozenset({
    # Each target has an independent Linux-official/Mac live evidence record.
    (102, "159518"),
    (101, "510300"),
})
...
def build_snapshot_request(username: str, token: str, request_id: int, request: Any) -> bytes:
    ...
    market_type = int(getattr(request, "market_type"))
    if (market_type, security_code) not in _VERIFIED_SNAPSHOT_TARGETS:
        raise NotImplementedError(
            "only independently verified L1 snapshot targets are supported "
            "in internet mode"
        )
...
#
# Wire contract captured from authorized official Linux SDK sessions on
# 2026-08-26 (SSE) and 2026-08-29 (SZSE): unlike K-line/snapshot/third-info
# queries this one runs on the *persistent push* connection (/amd/dgw/push),
# not on a one-shot dgw*_query endpoint.  The client sends
# ReqGetETFCodeTableList with a single ``Security`` param formatted
# "<code>|<market>", then ReqGetCodelistComplete.
...
# Live-verified market samples. The public header allows SSE/SZSE only, and
# each enumerated market has completed its own Linux/wire/macOS loop.
VERIFIED_ETF_INFO_MARKETS = {101, 102}
...
def parse_etf_info_packets(packets: list[dict[str, Any]], *,
    ...
    for packet in packets:
        ...
        # The captured codelist response has no paging controls.  Do not
        # silently invent a packet protocol from partial or repeated counters.
        if "pack_num" in headers or "all_pack_num" in headers:
            raise TgwProtocolError("unexpected ETF-info packet counters")
...
# pack_num/all_pack_num paging controls. Each verified single-item or two-item
# request completed in one data frame; unobserved response branches fail
# explicitly below.
# ---------------------------------------------------------------------------
SECINFO_WIRE_TAG = "109"
SECINFO_RECORD_FIELD_COUNT = 43

# This codelist-channel sub-scope is deliberately item-specific, not merely a
# market whitelist.  The public header permits SSE/SZSE/NEEQ and arbitrary
# lists, whereas the observed wire cases are only the two entries below and
# their exact two-item order.  Do not generalize this to all SZSE codes or to
# arbitrary batches.
VERIFIED_SECINFO_ITEMS = frozenset({
    (101, "510300"),
    (102, "159919"),
})
VERIFIED_SECINFO_PAIR = ((101, "510300"), (102, "159919"))


def build_secinfo_request(username: str, token: str, request_id: int,
                          items: list[dict[str, Any]]) -> bytes:
    """Build one observed internet-mode ``ReqGetCodeTableList`` envelope.

    A single item is supported only for the two independently observed ETF
    samples.  The only supported batch has the exact public input order SSE
    ``510300`` followed by SZSE ``159919``; the official *response* order is
    server-provided and must not be locally reordered.
    """
    if len(items) not in (1, 2):
        raise NotImplementedError(
            "only one verified securities-info item or the verified two-item pair is supported"
        )
    normalized: list[tuple[int, str]] = []
    for item in items:
        market = int(item["market"])
        security_code = str(item["security_code"]).strip()
        if not security_code:
        ...
        normalized.append((market, security_code))
    if len(normalized) == 1 and normalized[0] not in VERIFIED_SECINFO_ITEMS:
        raise NotImplementedError(
            "securities-info single item has not been wire-verified in internet mode"
        )
    if len(normalized) == 2 and tuple(normalized) != VERIFIED_SECINFO_PAIR:
        raise NotImplementedError(
            "only the ordered SSE 510300 then SZSE 159919 securities-info pair "
            "has been wire-verified in internet mode"
        )
    ...
    return _compact_json({
        ...
        "params": {
            "Security": ",".join(
                f"{security_code}|{market}" for market, security_code in normalized
            ),
        },
    })
...
def parse_secinfo_packets(packets: list[dict[str, Any]], *,
    ...
    for packet in packets:
        ...
        code_num = headers.get("code_num")
        if isinstance(code_num, bool) or not isinstance(code_num, int):
            raise TgwProtocolError("securities-info response code_num is not an integer")
        if code_num != len(data):
            raise TgwProtocolError(
                "securities-info response code_num does not match data length"
            )
        for record in data:
...
class TgwWssClient:
    ...
    def __init__(self, *, endpoint: str = "/amd/dgw/push", timeout: float = 15.0,
        ...
        # Codelist requests (ETF information and securities information) use
        # neither public query task ids nor the subscription sequence:
        # captured official sessions begin at one.
        self._codelist_request_id = 1
    ...
    def next_codelist_request_id(self) -> int:
        """Allocate a captured codelist-channel wire request id (1, 2, ...)."""
        with self._waiters_lock:
            value = self._codelist_request_id
            self._codelist_request_id += 1
            return value
