...
def securities_info_security_control(security: Any) -> dict[str, Any]:
    """Summarize ``Security``'s item encoding without emitting a code value.

    The multi-item form was previously unobserved.  This accepts either a
    scalar delimited sequence or a JSON list, retaining only item count, empty
    code flags and integer market enums.  Any unrecognized form stays
    explicitly ``invalid`` rather than being guessed into support.
    """
    if isinstance(security, list):
        raw_entries = security
        container = "list"
        separator = None
    elif isinstance(security, str):
        # The separator is syntax metadata only.  Codes are never serialized.
        separator_match = re.search(r"[,;]", security)
        separator = separator_match.group(0) if separator_match else None
        raw_entries = security.split(separator) if separator else [security]
        container = "str"
    else:
        return {"format": "invalid", "container": type(security).__name__}
    parsed: list[tuple[bool, int]] = []
    for entry in raw_entries:
        if not isinstance(entry, str):
            return {"format": "invalid", "container": container}
    ...
    return {
        "format": (
            ...
            if len(parsed) == 1
            else "multi-code|market"
        ),
        "container": container,
        "item_count": len(parsed),
        "markets_in_order": [market for _empty, market in parsed],
        "empty_code_flags": [empty for empty, _market in parsed],
        "separator": separator,
    }
        ...
        if opcode in {1, 2}:
            try:
                ...
                if isinstance(headers, dict) and isinstance(headers.get("tag"), str):
                    ...
                    item["codelist_response_controls"] = {
                        "id": headers.get("id") if isinstance(headers.get("id"), int) else None,
                        "code_num": (
                            headers.get("code_num")
                            if isinstance(headers.get("code_num"), int)
                            else None
                        ),
                    }
                ...
                if isinstance(value, dict) and isinstance(value.get("method"), str):
                    ...
                    if value["method"] == "ReqGetKline":
                        # The wire period enum is protocol metadata, not a
                        # business value; it pins the public->wire conversion.
                        item["request_period_type"] = (
                            params.get("period_type")
                            if isinstance(params, dict)
                            and isinstance(params.get("period_type"), int)
                            else None
                        )
                    if value["method"] == "ReqGetETFCodeTableList":
                        # The code itself is business data.  Retain only the
                        # delimiter contract and the wire market enum needed
                        # for ETF-info parity, never the code portion.
                        security = params.get("Security") if isinstance(params, dict) else None
                        parts = security.rsplit("|", 1) if isinstance(security, str) else []
                        market_token = (
                            int(parts[1])
                            if len(parts) == 2 and parts[1].isdigit()
                            else None
                        )
                        item["etf_security_control"] = {
                            "format": "code|market" if len(parts) == 2 and parts[0] else "invalid",
                            "market": market_token,
                        }
                    if value["method"] == "ReqGetCodeTableList":
                        # The high-level AmazingData path can deliberately
                        # request an empty code for an entire market.  Preserve
                        # only the empty/non-empty fact and market enum; never
                        # serialize the code itself.
                        security = params.get("Security") if isinstance(params, dict) else None
                        item["securities_info_control"] = securities_info_security_control(
                            security
                        )
