...
def _quiet_vendor_call(callback, /, *args, **kwargs):
    """Run vendor SDK lifecycle code without relaying diagnostic session data.

    The official wheel can print connection diagnostics during login/close.
    Oracle output is restricted to its explicit, desensitized JSON summary.
    """
...
def _etf_info_shapes(result: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if isinstance(first_basic, dict):
        ...
        # Market enums and row positions are protocol metadata for this
        # deliberately narrow two-item probe.  Keep codes and all market data
        # out of the oracle output.
        shape["market_types_in_response_order"] = [
            item.get("market_type") if isinstance(item, dict) else None
            for item in basics
        ]
    ...
    if first_cons is not None:
        ...
        shape["constituent_counts_in_response_order"] = [
            len(item) if isinstance(item, list) else None for item in constituents
        ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--username-stdin",
        action="store_true",
        help="read one authorized username without persisting it or placing it on argv",
    )
    parser.add_argument(
        "--password-stdin",
        action="store_true",
        help="read one authorized password without persisting it or placing it on argv",
    )
    ...
    parser.add_argument(
        "--etf-sync-only",
        action="store_true",
        help="issue exactly one synchronous ETF request; do not run the async collector",
    )
    parser.add_argument(
        "--etf-sse-szse-pair",
        action="store_true",
        help=(
            "issue only the fixed two-item SSE/SZSE ETF pair in one synchronous "
            "official request (requires --kind etf-info --etf-sync-only)"
        ),
    )
    ...
    if args.etf_sse_szse_pair and (
        args.kind != "etf-info" or not args.etf_sync_only
    ):
        parser.error("--etf-sse-szse-pair requires --kind etf-info --etf-sync-only")
    ...
    if args.password_stdin:
        password = getpass.getpass("one-run password: ")
        if not password:
            raise ValueError("one-run password must not be empty")
    ...
    logged_in = bool(_quiet_vendor_call(tgw.Login, cfg, tgw.ApiMode.kInternetMode))
    if not logged_in:
    ...
    try:
        ...
        if args.kind == "etf-info":
            if args.etf_sse_szse_pair:
                items = []
                for market, security_code in ((101, "510300"), (102, "159919")):
                    item = tgw.SubCodeTableItem()
                    item.market = market
                    item.security_code = security_code
                    items.append(item)
                result, error = tgw.QueryETFInfo(items, return_df_format=False)
            else:
                item = tgw.SubCodeTableItem()
                item.market = int(args.market_type)
                item.security_code = str(args.security_code)
            ...
            req_defaults = {}
            if args.etf_sync_only:
                etf_summary = {"skipped": "sync-only"}
            else:
