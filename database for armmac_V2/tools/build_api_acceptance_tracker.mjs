...
const contracts = {
  ...
  GetTaskID: ["无", "int64_t；MMDDHHmmSS+同秒序号1..1000000", "无网络本地返回；线程并发唯一"],
  ...
  QuerySnapshot: ["ReqDefault(pack(1), sizeof=55；含 level_type:uint16)", "Snapshot族；公开行57键", "同步 (rows,0)/(None,-76)；异步 query_spi(result,err_code) 已对齐"],
  ...
  QueryCodeTable: ["无业务入参", "MDCodeTable 6 字段；pack(1)=191", "同步全量累计已实现；IGMDCodeTableSpi 异步面/OnStatus/FreeMemory 未实现"],
  QuerySecuritiesInfo: ["SubCodeTableItem(pack(1), sizeof=36；market:i32 signed)", "MDCodeTableRecord 43 字段；pack(1)=555", "同步单 item 已实现；IGMDSecuritiesInfoSpi 异步面未实现"],
  QueryExFactorTable: ["const char* code（无市场参数）", "MDExFactorTable 5 字段；pack(1)=52；double×2", "同步单代码已实现；IGMDExFactorSpi 异步面未实现"],
  QueryFactor: ["ReqFactor", "Factor", "Factor SPI"],
  SetThirdInfoParam: ["task_id:int64 + key/value:str", "int 错误码", "参数暂存；QueryThirdInfo 后移除"],
  QueryThirdInfo: ["task_id:int64；offset/count 私有分页控制", "ThirdInfoData：嵌套 JSON body.data", "同步 list/DataFrame；高层日历已全量分页"],
  QueryETFInfo: ["SubCodeTableItem(pack(1), sizeof=36；market:int32)", "35字段基础信息 + ConstituentStockInfo[13字段][]", "SSE/SZSE 各单 item 同步 JSON/DataFrame 已实现；异步未实现"],
...
function nativeConstruction(api) {
  const map = {
    ...
    QueryThirdInfo: "参数表 + offset/count → ReqGetThirdInfo → 1..N 包 → ReqGetComplete",
    QueryCodeTable: "无入参 → dgw*_query one-shot ReqGetReduceCodeTable{QueryBandWidth:0.0} → 响应 tag 11103、pack_num/all_pack_num 分页 → 缺包 ReqGetPackage 补拉一次 → ReqGetComplete → 反引号 6 字段解析；同步累计全批",
    QueryETFInfo: "单个 SubCodeTableItem(101+510300 或 102+159919) → 独立低位 codelist id → push WSS ReqGetETFCodeTableList(Security=code|market) → tag 111 → ReqGetCodelistComplete → 两级容器",
...
for (const [apiCell, pdfPage, keyContract, mode, fullStatus, next] of parseTableBetween("## 1. 原生 TGW", "## 2. 原生 TGW")) {
  ...
  for (const api of names) {
    nativeRows.push({
      ...
      fullStatus,
      linuxDate: /LIVE_ALIGNED|WIRE_VERIFIED|LINUX_OBSERVED/.test(fullStatus) ? "2026-08-30" : "",
      linuxResult: /LIVE_ALIGNED|WIRE_VERIFIED/.test(fullStatus) ? "已有脱敏证据；范围见状态/证据" : "未执行",
      wire: "待逐接口取证",
      macDate: /LIVE_ALIGNED|ARM_IMPLEMENTED|WIRE_VERIFIED|CHANGES_REQUESTED/.test(fullStatus) ? "2026-08-30" : "",
      ...
      evidence: api === "GetTaskID" ? "docs/evidence/get_task_id.md" : "docs/PDF_API_PARITY_MATRIX.md",
      next,
      notes: keyContract,
      batch: "",
    });
...
for (const [api, pdfPage, keyContract, mode, fullStatus, next] of parseTableBetween("## 2. 原生 TGW", "## 3. AmazingData")) {
  nativeRows.push({
    ...
    linuxDate: /LIVE_ALIGNED|WIRE_VERIFIED|LINUX_OBSERVED/.test(fullStatus) ? "2026-08-30" : "",
    linuxResult: /LIVE_ALIGNED|WIRE_VERIFIED/.test(fullStatus) ? "已有脱敏证据；范围见状态/证据" : "未执行",
    wire: api === "QueryKline" ? "ReqGetKline；1min=10000，daily=10100，weekly=10101，monthly=10102，quarterly=10103，yearly=10104；3min=10001仅静态" : api === "QuerySnapshot" ? "ReqGetSnapshot；数据tag=11000；空数据帧 tag=\"DataEmpty\"/status=-100→公开-76；错误路径不发 ReqGetComplete" : api === "QueryCodeTable" ? "dgw*_query one-shot ReqGetReduceCodeTable；tag=11103；反引号6字段；缺包 ReqGetPackage" : api === "QuerySecuritiesInfo" ? "push WSS ReqGetCodeTableList；独立低位 codelist id；Security=code|market；tag=字符串109；ReqGetCodelistComplete；43字段" : api === "QueryExFactorTable" ? "dgw*_query one-shot ReqGetExFactor；tag=11102；5字段CSV；double 18位小数字符串；ReqGetComplete" : api === "QueryThirdInfo" ? "ReqGetThirdInfo；tag=11101；日历高层offset/count分页" : api === "QueryETFInfo" ? "push WSS ReqGetETFCodeTableList；独立低位 codelist id；Security=code|market；tag=字符串111；ReqGetCodelistComplete" : "待逐接口取证",
    macDate: /LIVE_ALIGNED|ARM_IMPLEMENTED|WIRE_VERIFIED|CHANGES_REQUESTED/.test(fullStatus) ? "2026-08-30" : "",
    macResult: /LIVE_ALIGNED/.test(fullStatus) ? "同参子范围已对齐" : /ARM_IMPLEMENTED/.test(fullStatus) ? "本地实现，待 live 同参" : "待实现/复验",
    conclusion: fullStatus,
    construction: nativeConstruction(api),
    ...
    responseType: contracts[api]?.[1] || keyContract,
    callback: contracts[api]?.[2] || keyContract,
    evidence: api === "QueryKline" ? "docs/evidence/query_kline_daily.md；docs/evidence/query_kline_week.md；docs/evidence/query_kline_month.md；docs/evidence/query_kline_quarter_live.md；docs/evidence/query_kline_year_live.md；docs/evidence/query_kline_three_minute.md" : api === "QuerySnapshot" ? "docs/evidence/query_snapshot_szse_etf.md；docs/evidence/query_snapshot_error_async_contract.md" : api === "QueryCodeTable" ? "docs/evidence/query_code_table_static.md；docs/evidence/query_code_table_live_closure.md" : api === "QuerySecuritiesInfo" ? "docs/evidence/query_securities_info_sse.md" : api === "QueryExFactorTable" ? "docs/evidence/query_ex_factor_table_000001.md" : api === "QueryThirdInfo" ? "docs/evidence/amazingdata_get_calendar.md" : api === "QueryETFInfo" ? "docs/evidence/query_etf_info_static.md；docs/evidence/query_etf_info_sse_etf.md；docs/evidence/query_etf_info_szse_etf.md" : "docs/PDF_API_PARITY_MATRIX.md",
    next,
    notes: keyContract,
    batch: "",
  });
...
for (const [category, apiCell, pdfPage, fullStatus] of parseTableBetween("## 3. AmazingData", "## 4. AmazingData")) {
  ...
  for (const api of names) {
    ...
    const lowLevel = api === "query_kline" ? "QueryKline" : api === "query_snapshot" ? "QuerySnapshot" : api === "get_calendar" ? "SetThirdInfoParam + QueryThirdInfo" : api === "get_code_info" ? "QuerySecuritiesInfo × 3（空代码）" : ["login", "logout", "update_password"].includes(api) ? api : "QueryThirdInfo / 高层 wrapper（待逐项证明）";
    amazingRows.push({
      ...
      fullStatus,
      linuxDate: /LIVE_ALIGNED|WIRE_VERIFIED|LINUX_OBSERVED/.test(fullStatus) ? "2026-08-30" : "",
      linuxResult: /LIVE_ALIGNED|WIRE_VERIFIED|LINUX_OBSERVED/.test(fullStatus) ? "已有范围化证据" : "未执行",
      wire: api === "query_kline" ? "委托 QueryKline；底层已有六个周期子范围" : api === "query_snapshot" ? "委托 QuerySnapshot；实验" : api === "get_calendar" ? "A010061003；SH→SSE；offset/count 每页1000全量累计" : api === "get_code_info" ? "官方映射 QuerySecuritiesInfo 102→101→2 空代码；wire capture为空" : "待 Linux capture 确认",
      macDate: /LIVE_ALIGNED|ARM_IMPLEMENTED|WIRE_VERIFIED|CHANGES_REQUESTED/.test(fullStatus) ? "2026-08-30" : "",
      macResult: api === "get_calendar" ? "默认 SH/隐式str已对齐8714条；显式str/datetime待live" : api === "get_code_info" ? "离线7列normalizer；网络入口显式阻塞" : /LIVE_ALIGNED/.test(fullStatus) ? "同参子范围已对齐" : "未实现/待验证",
      conclusion: fullStatus,
      ...
      evidence: api === "get_calendar" ? "docs/evidence/amazingdata_get_calendar.md" : api === "get_code_info" ? "docs/evidence/amazingdata_get_code_info_extra_etf.md" : "docs/PDF_API_PARITY_MATRIX.md",
...
const functionRows = inventoryRows
  ...
  .map((row) => {
    ...
    // The live AmazingData wrapper uses A010061003, which is absent from this
    // older C++ PDF catalogue.  Do not paint adjacent catalogue ids green.
    const catalogueStatus = /A010060001|A010061001/.test(functionId) ? "INVENTORIED" : row.status;
    return [
      row.category, name, functionId, row.pdfPage, "是", catalogueStatus,
...
structureRows.unshift(
  ...
  ["TGW C++ 开发手册", "ReqKline", "33–34（25–26）", "pack(1) ctypes", "sizeof=71；cyc_type 公共枚举；日期/时间为整数", "LIVE_ALIGNED", "LIVE_ALIGNED(1-minute + daily/weekly/monthly/quarterly/yearly subranges)", "3分钟10001仅静态契约，等Linux数据权限"],
  ...
  ["TGW C++ 开发手册", "MDKLine", "60 起", "回调数组", "市场/代码/时间/OHLC/量额/周期；整数缩放", "LIVE_ALIGNED", "LIVE_ALIGNED(1-minute + daily/weekly/monthly/quarterly/yearly low-level)", "只有159691特定1分钟样本完成单位核验"],
  ["TGW C++ 开发手册", "MDCodeTable", "36（28）/头文件", "pack(1) callback", "code[16]/symbol[32]/en_name[128]/market u8/type[10]/currency[4]；sizeof=191", "ARM_IMPLEMENTED", "ARM_IMPLEMENTED", "wire 已证（tag 11103 反引号 6 字段）；服务端全市场缺第 3 包阻塞成功同参"],
  ["TGW C++ 开发手册", "SubCodeTableItem", "39（31）/头文件", "pack(1) request", "market:int32（有符号） + security_code[32]；sizeof=36", "LIVE_ALIGNED", "LIVE_ALIGNED(SSE+SZSE single ETF requests)", "ETF已验101+510300与102+159919；证券信息仅SSE单代码"],
  ["TGW C++ 开发手册", "MDETFCodeTableRecord", "85–87（77–79）/头文件", "非 POD callback / wire 两级容器", "35 个固定字段=507 bytes + LP64 std::vector 24 bytes；成分股项 13 字段/sizeof=245", "LIVE_ALIGNED", "LIVE_ALIGNED(SSE+SZSE single ETF parsed shapes)", "不整体 ctypes 化；已验SSE 1×35+300×13，SZSE 1×35+301×13"],
);
...
const batches = [
  ...
  ["B-20260826-10", "2026-08-26", "QuerySnapshot", "SZSE ETF L1 同步错误码+异步 SPI（同参复验）", "盘后查询", "空窗 (None,-76)；数据窗 (11行,0)；异步 submit(True,None)+回调(list,11)/(None,-76)", "四场景逐一一致（同步双格式+异步双分支）", "sync+async error contract 一致；多包异步交付仍未观测", "是", "docs/evidence/query_snapshot_error_async_contract.md", "LIVE_ALIGNED", "本轮查询通道3次1000准入回收，低频间隔后恢复；wire 新增 DataEmpty 错误帧形状"],
  ["B-20260826-11", "2026-08-26", "QueryCodeTable", "互联网全市场 wire 取证 + Mac 实现（ARM）", "盘后查询", "登录成功；sync probe -83 kTimeout（服务端12包缺第3包，ReqGetPackage无响应）；wire: dgw*_query/ReqGetReduceCodeTable/tag11103/反引号6字段/11包共167,269行", "登录成功；缺包超时（同因同果）；dgw2 备用端点 1000 accept conn active close 流控", "无成功同参样本（两侧均被服务端全市场缺包阻塞）；错误语义同因同果", "是", "docs/evidence/query_code_table_live_closure.md", "ARM_IMPLEMENTED", "完成 method 未证；currency/security_type 空白串 trim 待成功样本复核；query_spi 显式拒绝"],
  ["B-20260826-12", "2026-08-26", "QueryKline", "季线 cyc_type=10011 静态三方契约", "静态核对", "未执行", "未实现", "PDF/HDR/官方 wrapper 三方一致；MDKLine 11 字段含补列，周期无关", "否", "docs/evidence/query_kline_quarter_static.md", "STATIC_MATCHED", "10011→wire 10103 为外推假设，未实捕，禁止预填实现"],
  ["B-20260826-13", "2026-08-26", "QueryKline", "季线 cyc_type=10011 Linux/wire/Mac 同参闭环", "盘后查询", "login true、error 0、1行/11列（10 int+1 str）、kline_time 8位、market=[101]；wire 实捕 10011→period_type/tag=10103、单包、ReqGetComplete+正常close", "登录成功、error 0、rows=1、11 列集合/类型与 Linux 完全一致", "quarterly 子范围同参一致", "是", "docs/evidence/query_kline_quarter_live.md", "LIVE_ALIGNED(quarterly only)", "年线/分钟族未验且显式拒绝；季线线上单包样本；跨季多包仅合成覆盖"],
  ["B-20260826-14", "2026-08-26", "QueryKline", "年线 cyc_type=10012 Linux/wire/Mac 同参闭环", "盘后查询", "login true、error 0、1行/11列；wire 实捕 10012→period_type/tag=10104、单包、ReqGetComplete+正常close", "Mac 首次被 1000 流控回收，低频换备用端点成功后 error 0、rows=1、11 列类型一致", "yearly 子范围同参一致", "是", "docs/evidence/query_kline_year_live.md", "LIVE_ALIGNED(yearly only)", "分钟族未验且显式拒绝；年线线上单包样本；跨年多包仅合成覆盖"],
  ["B-20260826-15", "2026-08-26", "QuerySecuritiesInfo", "SSE 510300 单代码最小只读闭环", "盘后查询", "login true、error 0、1行/43列（int/str）、market=[101]、variety=[2]；wire: push ReqGetCodeTableList/Security=510300|101/tag=字符串109/ReqGetCodelistComplete", "error 0、1 行、43 列类型/不变量与 Linux 逐项一致", "SSE single code 子范围同参一致", "是", "docs/evidence/query_securities_info_sse.md", "LIVE_ALIGNED(SSE single code only)", "全市场/多item/SZSE/NEEQ 显式拒绝；空结果/非零status未取证；push 无重连"],
  ["B-20260826-16", "2026-08-26", "QueryExFactorTable", "000001 单代码最小只读闭环", "盘后查询", "login true、err 0、33行/5列（inner_code/code str、ex_date int、ex_factor/cum_factor float）、double 18位往返无损、cum_factor 单调违例2处；wire: dgw1_query ReqGetExFactor/tag=11102/5字段CSV/ReqGetComplete", "err 0、33 行、5 列类型/不变量与 Linux 逐项一致（含单调违例2处吻合）", "000001 子范围同参一致", "是", "docs/evidence/query_ex_factor_table_000001.md", "LIVE_ALIGNED(000001 only)", "空结果/非零status未取证；多包未线上观测；异步 SPI 未实现；double 缩放按 N38(15) 由调用方处理"],
  ["B-20260829-17", "2026-08-29", "GetTaskID", "无登录本地任务号", "本地契约", "官方int/本地秒前缀/同秒连续/跨秒重置", "格式一致；32线程×128唯一", "不需网络的本地契约对齐", "否", "docs/evidence/get_task_id.md", "LIVE_ALIGNED(local task-id format)", "官方溢出/时钟回拨/跨进程未观测"],
  ["B-20260829-18", "2026-08-29", "QueryETFInfo", "SZSE 159919 单 ETF 同步", "盘后查询", "error 0；1×35 + 301×13；wire tag 111", "error 0；shape/列/类型/不变量一致", "SZSE 单 ETF 对齐；修复独立 codelist id", "是", "docs/evidence/query_etf_info_szse_etf.md", "LIVE_ALIGNED(SZSE single ETF)", "多item/异步/空错误/多帧未验"],
  ["B-20260829-19", "2026-08-29", "QueryKline", "历史3分钟 cyc_type=10001", "静态+权限探测", "PDF/HDR/官方对象确认3分钟；登录-95，未发查询", "未运行；继续NotImplementedError", "只到静态契约", "否", "docs/evidence/query_kline_three_minute.md", "STATIC_MATCHED", "需先开通Linux历史行情数据权限"],
  ["B-20260829-20", "2026-08-29", "BaseData.get_calendar", "默认SH/隐式str/全量", "盘后高层", "list[int]；8714；升序唯一", "修复分页后8714；类型/状态一致", "默认分支对齐", "是", "docs/evidence/amazingdata_get_calendar.md", "LIVE_ALIGNED(default SH implicit str)", "显式str/datetime仍待Mac live；高层不随wheel"],
  ["B-20260829-21", "2026-08-29", "BaseData.get_code_info", "EXTRA_ETF", "Linux高层观测", "102→101→2三次空代码QuerySecuritiesInfo；1631行7列", "未live；离线7列normalizer+显式blocker", "仅Linux观测，不得标绿Mac", "否", "docs/evidence/amazingdata_get_code_info_extra_etf.md", "LINUX_OBSERVED(EXTRA_ETF only)", "capture为空；缺wire/分页/完成语义；PDF漏list_day"],
  ["B-20260830-22", "2026-08-30", "QuerySecuritiesInfo", "SSE 510300 task-id修复后回归", "盘后查询", "沿用已验Linux/wire证据", "login true；error 0；1行43列；market/category不变", "独立低位codelist id未破坏既有范围", "是", "docs/evidence/query_securities_info_sse.md", "LIVE_ALIGNED(SSE single code only)", "不扩展全市场/SZSE/NEEQ/多item"],
];
...
styleTitle(summary, "A1:L1", "macOS ARM64 适配验收总览", "盘后工作台 · 由接口台账公式汇总 · 更新时间 2026-08-30", navy);
...
summary.getRange("D6:H13").values = [
  [1, "get_code_info", "EXTRA_ETF", "LINUX_OBSERVED", "重做可用脱敏wire capture；证明三市场空代码分帧/完成语义"],
  [2, "QueryKline", "3分钟 10001", "STATIC_MATCHED", "先解决Linux账号-95数据权限，再做wire/Mac"],
  [3, "get_calendar", "显式str/datetime", "ARM_IMPLEMENTED", "每分支各一次低频Mac同参"],
  [4, "QueryCodeTable", "全市场完整分包", "ARM_IMPLEMENTED", "等服务端不再缺第3包后闭环"],
  [5, "QueryETFInfo", "空/错误或异步", "LIVE_ALIGNED", "从一个独立分支取证；不外推多item"],
  [6, "get_code_list", "单证券类型", "INVENTORIED", "先证明真实底层映射与列表顺序"],
