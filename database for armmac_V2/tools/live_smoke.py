        ...
        if ok:
            try:
                ...
                if args.snapshot:
                    ...
                    # Keep the audited historical-L1 invocation explicit even
                    # though the public structure constructor defaults both.
                    request.data_type = 0
                    ...
                    if args.snapshot_async:
                        ...
                        snapshot_invariants = {
                            "all_key_sets_identical": all(
                                isinstance(row, dict) and sorted(row) == columns
                                for row in rows
                            ),
                            ...
                            "distinct_market_types": sorted({
                                row.get("market_type") for row in rows
                                if isinstance(row, dict) and isinstance(row.get("market_type"), int)
                            }),
                            "distinct_variety_categories": sorted({
                                row.get("variety_category") for row in rows
                                if isinstance(row, dict) and isinstance(row.get("variety_category"), int)
                            }),
                            "orig_time_digit_lengths": sorted({
                                len(str(row.get("orig_time"))) for row in rows
                                if isinstance(row, dict) and isinstance(row.get("orig_time"), int)
                            }),
                            "trading_phase_code_count": len({
                                row.get("trading_phase_code") for row in rows
                                if isinstance(row, dict) and isinstance(row.get("trading_phase_code"), str)
                            }),
                        }
                        print(
                            f"snapshot_query_error={error} rows={len(rows) if rows else 0} "
                            f"columns={columns} column_types={column_types} "
                            f"request_data_type={request.data_type} "
                            f"request_level_type={request.level_type} "
                            f"invariants={snapshot_invariants}"
                        )
                if args.etf_info:
