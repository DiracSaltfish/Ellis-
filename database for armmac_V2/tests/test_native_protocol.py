...
class StructureContractTests(unittest.TestCase):
    def test_get_version_matches_observed_vendor_format_without_backend_init(self):
        with patch.object(interface, "_g_backend", None):
            self.assertEqual(tgw.GetVersion(), _backend.DEFAULT_TGW_CLIENT_VERSION)
            self.assertRegex(tgw.GetVersion(), r"^V\d+\.\d+\.\d+\.\d+-rc\d+\.\d+-YHZQ$")
            self.assertIsNone(interface._g_backend)

    def test_get_version_and_login_use_same_environment_override(self):
        with patch.dict(os.environ, {"TGW_CLIENT_VERSION": "test-vendor-version"}):
            self.assertEqual(tgw.GetVersion(), "test-vendor-version")
            self.assertEqual(_backend.get_client_version(), "test-vendor-version")

    def test_close_releases_backend_and_allows_fresh_login(self):
        created = []

        def make_backend():
            backend = _backend.SimBackend()
        ...
        with patch.object(interface, "_g_backend", None), patch.object(
            _backend, "get_backend", side_effect=make_backend
        ):
            self.assertTrue(interface.Login(cfg, 2))
            first = created[-1]
            interface.Close()
            self.assertIsNone(interface._g_backend)
            self.assertEqual(first.state, _backend.BackendState.CLOSED)
            self.assertEqual(first.cfg["password"], "")
            self.assertIsNone(first.logon_response)
    ...
    def test_close_releases_global_even_when_backend_close_raises(self):
        class BrokenBackend:
            @staticmethod
            def close():
                raise RuntimeError("close failed")

        with patch.object(interface, "_g_backend", BrokenBackend()):
...
class ProtocolEnvelopeTests(unittest.TestCase):
    ...
    def test_third_info_pagination_envelope_uses_explicit_integer_controls(self):
        raw = build_third_info_request(
            "test_user",
            "test_token",
            1_000_002,
        ...
        value = json.loads(raw)
        self.assertEqual(value["offset"], 1000)
        self.assertEqual(value["count"], 1000)
        self.assertEqual([item["key"] for item in value["item"]], ["market"])
    ...
    def test_snapshot_sse_target_uses_the_same_verified_l1_envelope(self):
        request = ReqDefault().set_code("510300")
        request.market_type = 101
        request.date = 20260825
        request.begin_time = 93000000
        request.end_time = 93030000

        value = json.loads(build_snapshot_request("user", "token", 1, request))

        self.assertEqual(value["method"], "ReqGetSnapshot")
        self.assertEqual(value["params"]["market_type"], 101)
        self.assertEqual(value["params"]["data_type"], 0)
        self.assertNotIn("level_type", value["params"])
    ...
    def test_snapshot_rejects_unverified_level_market_or_code(self):
        ...
        request.market_type = 101
        with self.assertRaisesRegex(NotImplementedError, "verified L1 snapshot"):
            build_snapshot_request("user", "token", 1, request)
