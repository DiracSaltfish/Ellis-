"""Synthetic contract tests for the verified QueryETFInfo single-ETF scopes.
...
class BuildEtfInfoRequestTests(unittest.TestCase):
    ...
    def test_accepts_captured_szse_market(self):
        value = json.loads(build_etf_info_request(
            "user", "token", 8, [{"market": 102, "security_code": "SYNTHSZ"}]
        ))
        self.assertEqual(value["params"], {"Security": "SYNTHSZ|102"})
...
class EtfCodelistRequestIdTests(unittest.TestCase):
    def test_etf_backend_uses_independent_low_wire_sequence(self):
        class FakeCodelistClient:
            ...
            def request_many(self, request_id, payload, *, done, timeout):
                ...
                packet = etf_packet(request_id=request_id)
        ...
        # A public TaskID has the documented date/time form, not the codelist
        # wire id observed for this request family.
        result = backend._query_etf_info(
            829232038000001, [{"market": 102, "security_code": "SYNTHSZ"}]
        )
...
class ParseEtfInfoPacketsTests(unittest.TestCase):
    ...
    def test_rejects_unverified_packet_counters(self):
        missing_counter = etf_packet()
        missing_counter["headers"]["pack_num"] = 1
        with self.assertRaisesRegex(Exception, "packet counters"):
            parse_etf_info_packets([missing_counter])
        repeated_counter = etf_packet()
        repeated_counter["headers"].update({"pack_num": 1, "all_pack_num": 1})
        with self.assertRaisesRegex(Exception, "packet counters"):
            parse_etf_info_packets([repeated_counter])
...
class PublicContractTests(unittest.TestCase):
    ...
    def test_szse_single_item_reaches_backend_unchanged(self):
        item = SubCodeTableItem().set_code("SYNTHSZ")
        item.market = 102
        result, error = interface.QueryETFInfo(item, return_df_format=False)
        self.assertEqual((result, error), ([wire_record_named()], 0))
        _, req = self.backend.calls[-1]
        self.assertEqual(req["items"], [{"market": 102, "security_code": "SYNTHSZ"}])
