...
> re3 迁移更新：未验证 `level_type`、市场和代码已显式拒绝；`query_spi` 已改为明确
> `NotImplementedError`。以下第 1、3 项已在新工程收口。第 2 项的官方错误码合约仍未完成，
> 所以稳定公开 API 的 `CHANGES_REQUESTED` 结论不变。
...
1. **限制未验证分支（re3 已完成）。** `data_type != 0`、`level_type != 0` 以及非 `SZSE 159518` 已显式 `NotImplementedError`。
...
3. **收口 `query_spi` 语义（re3 已完成临时方案）。** 当前对 `query_spi is not None` 显式拒绝；若未来实现真异步，必须重新对齐官方提交返回与回调时序。
