...
- SSE `510300` 在 2026-08-25 09:30:00.000–09:30:30.000 的历史 L1 同步快照；
- 登录前 `GetVersion()`、`GetErrorMsg()`，以及 `Close()` 后在同一解释器创建新连接。
...
```bash
cd "/Users/ellis/工具程序开发/database for armmac_V2"
python3 -m venv .venv
...
python -m pip install dist/tgw_macos_arm64-1.0.9.2.7-py3-none-any.whl
```

当前已验收 wheel：

```text
dist/tgw_macos_arm64-1.0.9.2.7-py3-none-any.whl
SHA-256: 7bd1b3586f108f0354b5605b370d2860ba61eff2bfe3f6ffc28bfddc5df9cdfd
```
...
`tgw.__version__` 是 Mac 兼容包版本；`tgw.GetVersion()` 是当前对齐的厂商客户端版本，
两者含义不同。
...
`Close()` 会在 `finally` 中释放全局 backend，并清空 backend 内的密码和登录响应；同一解释器
后续调用 `Login()` 会创建新的 transport。Mac 以 5 秒冷却完成两轮在线复登入，结果均成功且
backend identity 不同；同参 Linux 官方 wrapper 的第二轮返回 `False`，所以这是 Mac 已通过的
扩展修复，不是 Linux parity 声明。长时间运行程序仍应由进程监管；正式 `Session` 对象尚未实现。

需要在同一 Python 进程重新建立会话时，必须先完整关闭，再创建下一轮连接；建议保留至少
5 秒冷却，不要并发复用同一账号：

```python
import time

def run_one_session(cfg, work):
    if not tgw.Login(cfg, tgw.ApiMode.kInternetMode):
        raise RuntimeError("login failed")
    try:
        return work()
...
first_result = run_one_session(cfg, first_work)
time.sleep(5)
second_result = run_one_session(cfg, second_work)
```

## 5. 已验证查询

当前开放查询均支持其文档列明子范围的**同步调用**：没有 `query_spi` 时返回
...
`GetTaskID()` 是无网络的本地调用，返回 Python `int`，格式为
`MMDDHHmmSS + 同秒序号`。它有线程锁，同秒序号连续，跨秒从 1 重置；官方对
同秒超过 1,000,000 和时钟回拨的行为未文档化，Mac 对超限显式失败。
...
`experimental/amazingdata_compat` 中的高层 `BaseData.get_calendar()` 已取证官方
`A010061003`、SH→SSE 映射和每页 1,000 条的全量累计；默认 SH/默认日期/隐式
`str` 分支与 Linux 同为 8,714 个升序唯一 `int` 日期。该高层目录仍不随 wheel
安装；显式 `str`/`datetime` 各做过一次 Mac live，均在登录成功后遇到
`TgwTransportError`，维持 `ARM_IMPLEMENTED`；其它市场或日期不得由默认分支外推。
...
`10010`、`10011`、`10012` 外的 `cyc_type` 会明确失败。其中 PDF/HDR 明确定义
`10001` 为 **3 分钟**（不是 2 分钟）；2026-08-29 的 Linux 官方尝试在登录阶段就因
`-95` 数据权限被拒，没有发出 `ReqGetKline`，所以 Mac 不猜测它的 wire 周期/tag。
...
### 5.3 ETF 基础信息与成分股（SSE/SZSE 单 ETF）
...
当前只放行已验证的 `market=101(SSE)` 或 `market=102(SZSE)`、单个
`SubCodeTableItem`、同步返回。JSON 模式返回：

```python
list[tuple[dict[str, object], list[dict[str, object]]]]
```

已验样本为 SSE `510300`（1 条 35 字段基础信息 + 300 条 13 字段成分）与
SZSE `159919`（1 条 35 字段基础信息 + 301 条 13 字段成分）。默认 DataFrame
...
`ReqGetETFCodeTableList`，参数形如 `Security="510300|101"`，响应 tag 为字符串 `"111"`；收到
单帧后发送无 params 的 `ReqGetCodelistComplete`。该路径与订阅共享 reader 生命周期，断线时
查询也会失败。这类 codelist 请求使用独立从 1 起计的 wire id，不使用公开时间格式
`GetTaskID`。异步 `IGMDETFInfoSpi` 尚未实现；多个 item、空结果/错误码、多响应帧均
...
SSE `510300` 只按下面这个已验窄窗使用：

```python
req = tgw.ReqDefault().set_code("510300")
req.market_type = tgw.MarketType.kSSE
req.date = 20260825
req.begin_time = 93000000
req.end_time = 93030000
...
assert error == 0
assert len(rows) == 11
assert all(len(row) == 57 for row in rows)
```

放行目标仅为 `SZSE 159518` 与 `SSE 510300`、`data_type=0`、`level_type=0`。其中 SSE
仅验 2026-08-25 09:30:00.000–09:30:30.000；method 为 `ReqGetSnapshot`，数据响应 tag
为 `11000`。该 SSE 窄窗官方 Linux 与 Mac 均返回 11 行 × 57 个公开字段且类型一致；SZSE
既有范围另验空数据窗按官方语义返回 `(None, -76/kDataEmpty)`。
...
2. 空数据 wire 帧为字符串 tag `"DataEmpty"` + status=-100，客户端映射为公开 -76；
   其它错误标签未捕获，出现即显式报协议错；
3. 只验证了两个精确市场/代码及其各自日期窄窗口；DataFrame 列序与官方 `json_normalize`
   的逐列一致性未验，不得外推其它 SSE/SZSE 证券。
...
### 5.6 证券基础信息 `QuerySecuritiesInfo`（沪深单项及固定双项）
...
固定沪深双项必须保持 SSE 在前、SZSE 在后；返回顺序由服务端决定，已验结果为
SZSE 在前、SSE 在后：

```python
sse = tgw.SubCodeTableItem().set_code("510300")
sse.market = tgw.MarketType.kSSE

szse = tgw.SubCodeTableItem().set_code("159919")
szse.market = tgw.MarketType.kSZSE

rows, err = tgw.QuerySecuritiesInfo([sse, szse], return_df_format=False)
assert err == 0
assert len(rows) == 2
assert [row["market_type"] for row in rows] == [102, 101]
```

已验证范围：SSE `510300`、SZSE `159919` 各单 item，以及固定输入顺序
`[SSE 510300, SZSE 159919]` 双 item，同步返回每行 **43 字段**（`MDCodeTableRecord`，
pack(1) sizeof=555）。双项 wire 是常驻 push 通道上的一条 `ReqGetCodeTableList`，两个
`code|market` 段以逗号连接，tag 字符串 `"109"`、`code_num=2`，随后发送
`ReqGetCodelistComplete`。官方与 Mac 双项均按服务端顺序 `[102,101]` 返回，不按输入重排。
注意事项：

1. 全市场（market=kNone）、其它代码/双项组合或顺序、三项以上、NEEQ、异步 `query_spi`
   均显式拒绝；
2. 空结果/非零 status 的 wire 形状未取证；数值字段为原始缩放值，调用方需按手册换算；
3. push 连接与订阅/ETF 共享生命周期，当前无自动重连；
4. 该 codelist 通道使用独立从 1 起计的 wire id，不使用公开时间格式 `GetTaskID`；
   SZSE 单项为 `error=0`/1 行/43 列，固定双项为 `error=0`/2 行/43 列。

AmazingData 高层 `get_code_info(EXTRA_ETF)` **不能**因此两个窄范围成功而视为可用。
官方 Linux 会按 102→101→2 对空代码发起三次全市场 `QuerySecuritiesInfo`，最终
返回 1,631 行且比 PDF 多 `list_day` 第 7 列。当前没有可用的全市场 wire/分页/完成捕获，
实验高层入口会明确 `NotImplementedError`，不返回部分或猜测结果。
