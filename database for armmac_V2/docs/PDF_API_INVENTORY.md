...
- 更新时间：2026-08-30；初始盘点由只读接口子 Agent 生成，中央验收者持续维护。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | TGW-C++ | 24 | 16 | 基础 | `IGMDApi::GetVersion()` | 双模式 | 无入参 | `const char*` 版本串 | 无 | 无 | 是（本地调用） | `LIVE_ALIGNED(local version string)` | Linux/Mac 均为 `V4.3.0.260626-rc2.0-YHZQ` 字符串，登录前可调用 | 与 `ReqLogon.Version` 默认值同源；厂商升级时重验 |
...
| 5 | TGW-C++ | 25 | 17 | 基础 | `IGMDApi::GetTaskID()` | 双模式 | 无 | int64（MMDDHHmmSS+序号1~1000000） | 无 | 格式规则 | 是（本地调用） | `LIVE_ALIGNED(local task-id format)`；同秒连续与跨秒重置已观测，Mac 4096 并发唯一 | 已有无登录 Linux 本地 oracle | 官方同秒溢出、时钟回拨、跨进程唯一性未观测；Mac 超限显式失败 |
...
| 38 | TGW-C++ | 36–37 | 28–29 | 查询 | `QuerySecuritiesInfo(spi,SubCodeTableItem*,cnt)` | 双模式 | `SubCodeTableItem{i32 market,char[32] code}` | `MDCodeTableRecord`（大结构，含涨跌停/单位/期权属性） | `OnMDSecuritiesInfo`+`OnStatus` | market kNone=全市场；支持 SSE/SZSE/NEEQ | 是 | `LIVE_ALIGNED(SSE 510300 single + SZSE 159919 single + ordered pair only)` | 双项单请求 `ReqGetCodeTableList`、tag `"109"`、`code_num=2`；Linux/Mac 均按 `[102,101]` 返回 | 全市场、其它组合/顺序、NEEQ、空结果/多帧/异步未验且显式拒绝 |
...
| 41 | TGW-C++ | 37–38 | 29–30 | 查询 | `SetThirdInfoParam(task_id,key,value)` | 双模式 | 逐 key/value 设置；必须先设 `function_id` | int32 错误码 | 无 | task_id 由 GetTaskID 生成 | 是 | `LIVE_ALIGNED(calendar function only)` | 已有 `A010061003` | 未验功能号必须显式失败；GetTaskID 已对齐本地格式/同秒序号/跨秒重置，但官方溢出/时钟回拨未观测 |
...
| 43 | TGW-C++ | 39 | 31 | 查询 | `QueryETFInfo(etf_info_spi,item,cnt)` | 双模式 | `SubCodeTableItem{market int32,code[32]}` pack(1)=36 | `MDETFCodeTableRecord` 固定 35 字段/507 字节 + LP64 `std::vector` 24 字节；成分股 `ConstituentStockInfo` pack(1)=245 | `OnMDETFInfo`+`OnStatus` | 市场仅沪深有效；spi 生命周期须持续到回调后 | 是 | `LIVE_ALIGNED(SSE+SZSE single ETF, synchronous only)`：push WSS `ReqGetETFCodeTableList`，`Security="code\|market"`，tag `"111"`，完成消息 `ReqGetCodelistComplete` | `510300.SSE`：1×35 + 300×13；`159919.SZSE`：1×35 + 301×13；两端各自同参一致 | codelist wire id 独立从 1 起计；多 item、空/错误、多帧和异步 SPI 未验；返回含 `std::vector`，不可整体 ctypes 化 |
...
| 78 | TGW-C++ | 172–173 | 164–165 | 资讯-交易日历 | A010060001 交易日历(京沪深A) | start_date,end_date(≤30天) | EXCHANGE(+INT),TRADE_DAYS,LAST/NEXT_TRADE_DAYS | 是 | `INVENTORIED`；不得用相邻的 A010061003 在线证据标绿本功能号 | 未对 A010060001 发起请求 | **PDF 未收录 A010061003**；实测高层使用 A010061003，与 PDF 目录不一致，必须分别维护状态 |
...
| 90 | AD-Py | 11–12 | 7–8 | 基础数据 | `BaseData.get_code_info(security_type)` | 互联网 | security_type 默认 EXTRA_STOCK_A | 官方实际 dataframe 为 7 列：PDF 六列 + `list_day` | 无 | 附录 security_type 族 | 是 | `LINUX_OBSERVED(EXTRA_ETF only)`；已证实三次 QuerySecuritiesInfo（102→101→2，空 code） | 待重做脱敏 wire capture | Mac 只证 SSE 非空单代码；三市场空代码/大表分页和完成语义未证，高层显式阻塞 |
...
| 97 | AD-Py | 16 | 12 | 基础数据 | `get_calendar(data_type,market)` | 互联网 | market 默认 SH；data_type 默认 str | 默认/str 实际 `List[int]`；datetime 官方为 `List[datetime]` | 无 | market: SH 等 | 是 | `LIVE_ALIGNED(default SH; implicit str only)`；已证高层使用 A010061003 与全量分页 | 已有 Linux/Mac 默认 8,714 条 | 显式 str/datetime 各一次 Mac live 均为登录后 `TgwTransportError`，维持 `ARM_IMPLEMENTED`；其它 market/date 拒绝 |
...
| E-8 | 官方 Linux AmazingData wheel | `BaseData.get_code_info(EXTRA_ETF)` | 输出差异 | PDF 输出表只列 6 列，官方运行实际返回第 7 列 `list_day:int64` | 官方结果为 1,631 行，四价格列为微单位除 1e6 的 float64 | `LINUX_OBSERVED(EXTRA_ETF only)` | Mac 尚无三市场空代码全表 wire/分页；未同参前必须保留 7 列合约而不是删列迎合 PDF |
