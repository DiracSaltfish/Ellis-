...
更新时间：2026-08-30。本文是 `AGENT_PARITY_WORKFLOW.md` 的盘后执行补充；通用证据等级、
安全约束和验收条件仍以该文档为准。
...
| 3 | `QueryKline` 分钟族（每次一个周期） | 3 分钟 `10001` 已于 2026-08-29 完成静态契约；Linux 账号登录 `-95` 数据权限阻塞，待权限后再闭环 |
...
| 6 | `QueryETFInfo` 后续子范围 | SZSE `159919` 已于 2026-08-29 闭环；下一项应取单 ETF 空/错误或异步，不外推多 item |
| 7 | `get_code_info(EXTRA_ETF)` | 已证官方三市场 `QuerySecuritiesInfo` 映射和 7 列合约；需先重做可用脱敏 wire capture，Mac 保持 blocker |
| 8 | `get_calendar` 显式 str/datetime | 默认 SH/隐式 str 已闭环；两个显式分支各做一次低频 Mac 同参 |
