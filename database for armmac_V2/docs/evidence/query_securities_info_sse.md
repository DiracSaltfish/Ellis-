...
- Arm:
  ...
  - `_backend.py` 新增 `_query_securities_info`：在**常驻 push 连接**上从独立低位
    codelist wire id 分配器取 id，`request_many` 等首个响应帧 → 发送
    `ReqGetCodelistComplete` → 解析；不把公开时间格式 `GetTaskID` 当作此通道
    的响应关联 id，不关闭 push 连接，不触碰 dgw_query 端点池；
...
- Live diff: 2026-08-26 Mac live 用第二授权账号（stdin 注入，未持久化，未打印值）一次
  ...
  ```
- Regression: 2026-08-30，公开 `GetTaskID` 改为手册的时间格式后，复核发现
  codelist 通道不能使用这个大 id。实现改为官方实捕的独立 `1,2,...` wire
  序列，新增合成回归测试；随后 Mac 使用第二授权账号同一 SSE `510300`
  低频重验成功：登录成功、`error=0`、1 行、43 列、类型与 market/category 不变。
  该回归仅证明既有 SSE 单代码子范围未被 task-id 修复破坏，不扩大市场或结果范围。
