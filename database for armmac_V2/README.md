...
数据提供经独立行情界面对照的显式单位输出。历史 3 分钟 `10001` 已确认静态契约，但 Linux
账号因 `-95` 数据权限在登录阶段被拒，未发查询，因此 Mac 仍不支持。`QueryETFInfo` 已完成 SSE
`510300` 与 SZSE `159919` 各单 ETF、`QuerySecuritiesInfo` 已完成 SSE `510300`/SZSE `159919`
各单项及固定沪深双项、`QuerySnapshot` 已新增 SSE `510300` 指定单日窄窗口、`QueryExFactorTable`
已完成 `000001` 单代码的 Linux/wire/Mac 闭环；`QueryCodeTable` 已完成 wire 取证与 Mac
实现（`ARM_IMPLEMENTED`），服务端全市场缺包阻塞成功同参，尚不可作为 `LIVE_ALIGNED` 使用。
`GetVersion` 已对齐 Linux 官方纯本地版本字符串，`Close` 已修复同解释器复登入的 CLOSED backend
复用缺陷，Mac 两轮 live 成功；Linux 官方第二轮返回 `False`，故该分支不标同参。实验高层
`get_calendar` 默认 SH 分支已修复为全量分页并与 Linux 对齐 8,714 条；
`get_code_info(EXTRA_ETF)` 已取得 Linux 7 列合约，但 Mac 全市场空代码 wire/分页未证，继续显式阻塞。
