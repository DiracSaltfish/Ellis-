...
public:
    ...
    static const uint32_t kDetailLen                    = 40;       // 港股经纪明细数组长度
    static const uint32_t kHKExOrderSnapshotLevelLen    = 20;       // 港股委托挂单买单序列档位长度
};
...
public:
    enum
    {
        ...
        kTaskIdRepeat               = -70,   // 任务id重复
        kDqsError                   = -69,   // DQS系统错误
...
public:
    ...
    static const uint32_t kSnapshot             = 10;   // 订阅现货L1快照数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kOptionSnapshot       = 11;   // 订阅期权快照数据（适用模式：：托管机房模式、互联网模式）
    static const uint32_t kHKTSnapshot          = 12;   // 订阅港股快照数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kIndexSnapshot        = 13;   // 订阅指数快照数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kAfterHourFixedPriceSnapshot = 14;   // 订阅盘后快照数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kCSIIndexSnapshot     = 15;   // 订阅中证指数数据（适用模式：互联网模式）
    static const uint32_t kCnIndexSnapshot      = 16;   // 订阅国证指数快照数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kHKTRealtimeLimit     = 17;   // 订阅港股通实时额度数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kHKTProductStatus     = 18;   // 订阅港股通产品状态数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kHKTVCM               = 19;   // 订阅港股VCM数据（适用模式：托管机房模式、互联网模式）
    static const uint32_t kFutureSnapshot       = 20;   // 订阅期货数据（适用模式：托管机房模式、互联网模式）
    static const uint64_t kSnapshotL2           = 21;   // 订阅快照数据（适用模式：托管机房模式、互联网模式）
    static const uint64_t kTickOrder            = 22;   // 订阅逐笔委托数据（适用模式：托管机房模式、互联网模式）
    static const uint64_t kTickExecution        = 23;   // 订阅逐笔成交数据（适用模式：托管机房模式、互联网模式）
    static const uint64_t kOrderQueue           = 24;   // 订阅委托队列数据（适用模式：托管机房模式、互联网模式）
};
...
public:
    static const uint8_t kNone                                 = 0;   // None,表示全品种
...
* @attention    托管机房模式适用
**/
class ColocationDataPermission
{
public:
    enum 
    {
        k1MinKline                      = 1,         // 1分钟k线权限
        k3MinKline                      = 2,         // 3分钟k线权限
        k5MinKline                      = 3,         // 5分钟k线权限
        k10MinKline                     = 4,         // 10分钟k线权限
        k15MinKline                     = 5,         // 15分钟k线权限
        k30MinKline                     = 6,         // 30分钟k线权限
        k60MinKline                     = 7,         // 60分钟k线权限
        k120MinKline                    = 8,         // 120分钟k线权限
        kDayKline                       = 9,         // 日k线权限
        kWeekKline                      = 10,        // 周k线权限
        kMonthKline                     = 11,        // 月k线权限
        kSeasonKline                    = 12,        // 季k线权限
        kYearKline                      = 13,        // 年k线权限
        kSnapshotDerive                 = 14,        // 现货衍生快照
        kTick                           = 15,        // 逐笔（逐笔成交+逐笔委托）
        kOrderQueue                     = 16,        // 委托队列
        kStockSnapshotL2                = 17,        // 现货L2快照
        kIndexSnapshotL2                = 18,        // 指数L2快照
        kOptionSnapshot                 = 19,        // 期权快照
        kHKTSnapshot                    = 20,        // 港股快照
        kFutureTickL1                   = 21,        // 期货L1快照
        kFutureTickL2                   = 22,        // 期货L2快照
        kCodeTable                      = 23,        // 证券代码表权限
        kSecurityInfo                   = 24,        // 证券基本信息权限
        kExFactorTable                  = 25,        // 复权因子表
        kStockSnapshotL1                = 26,        // 现货L1快照(目前仅包含北交所)
        kIndexSnapshotL1                = 27,        // 指数L1快照(目前仅包含北交所)
    };
};
...
public:
    enum 
    {
        ...
        kYearKline                      = 28,         // 年k线权限
        kHKExSnapshot                   = 29,         // 港股商业行情快照（港股商业股票、指数快照、经纪商数据、港股商业委托挂单）
        kHKExTickExcution               = 30,         // 港股商业行情逐笔
        kExFactor                       = 31,         // 复权因子权限
        kCodeTable                      = 32,         // 证券代码表权限
        kSecurityInfo                   = 33          // 证券基本信息权限(含etf的的权限)
    };
};
