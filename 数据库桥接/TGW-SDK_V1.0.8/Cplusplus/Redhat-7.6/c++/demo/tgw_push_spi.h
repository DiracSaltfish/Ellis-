...
public:
    ...
    void OnMDSnapshot(MDSnapshotL2* data, uint32_t cnt) override;
    ...
    void OnMDOrderBook(std::vector<MDOrderBook>& order_book) override;
    void OnMDOrderBookSnapshot(MDOrderBookSnapshot* order_book_snapshots, uint32_t cnt) override;
    void OnMDTickOrder(MDTickOrder* tick_orders, uint32_t cnt) override;
    void OnMDTickExecution(MDTickExecution* tick_execs, uint32_t cnt) override;
    void OnMDOrderQueue(MDOrderQueue* order_queues, uint32_t cnt) override;
private:
};
