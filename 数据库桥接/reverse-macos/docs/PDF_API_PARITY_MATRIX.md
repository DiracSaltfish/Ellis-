...
|---|---:|---|---|---|---|
| QueryKline | 33–34（25–26） | `ReqKline`, `MDKLine` | 两者 | `LIVE_ALIGNED(daily only)` | 每个周期单独取证 |
...
| QueryCodeTable | 36（28） | code table SPI | 两者 | `INVENTORIED` | Linux shape + wire |
...
| QueryETFInfo | 39（31） | `SubCodeTableItem`, ETF SPI | 两者 | `INVENTORIED` | 单 ETF oracle |
...
| 历史行情 | `query_kline` | 26 | `LIVE_ALIGNED(daily low-level only)` |
...
| `Kline` | 145（141） | `LIVE_ALIGNED(daily low-level)` | 日期/时间、价格金额缩放、周期类型 |

## 5. 推荐领取顺序

1. `QuerySnapshot` + `Snapshot`：与已验证 L1 字段可互相校验，价值最高。
2. `QueryCodeTable` 或 `QueryETFInfo`：只读、结构清晰，适合第二位 Agent。
3. `QueryKline` 的单个非日线周期：每个 Agent 只领取一个周期。
4. AmazingData `get_code_info/get_code_list`。
5. 其余 ThirdInfo 功能号，按类别逐项推进。
