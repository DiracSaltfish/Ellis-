...
- `tools/remote_sdk_oracle.py`：ThirdInfo 日历与日 K 线；新查询应在这里增加独立 `--kind`。
- `tools/remote_push_oracle.py`：`159518` ETF L1，只输出计数和间隔。

每次查询结束必须调用官方 `Close()`；每次订阅必须先 `UnSubscribe()` 再 `Close()`。远端临时目录、捕获和动态库在取回摘要后立即清理。

### 4.4 协议取证

官方返回容器相同，不代表 wire 相同。使用 `tools/ssl_write_interpose.c` 对官方进程的 `SSL_write/SSL_read` 做一次最小捕获，再用 `tools/analyze_ssl_write_capture.py` 生成脱敏形状摘要。
...
- Arm 实现和协议单测已存在；本轮 default/备用 query 路径以及独立授权账号的 live 复验都被服务端 `1000 / accept conn active close` 回收。这说明限制不只是同账号互踢；历史成功证据保留，但本轮没有把状态扩大到非日线。

结论：`LIVE_ALIGNED(daily only)`；分钟、周/月等周期仍为 pending。
