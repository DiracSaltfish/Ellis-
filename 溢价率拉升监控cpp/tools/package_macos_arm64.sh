...
build_dir=${1:-${project_root}/build/native-macos-arm64-release}
...
if [[ ! -x ${build_dir}/etf-premium-tgw ]]; then
    print -u2 "Release 原生 TGW 进程不存在：${build_dir}/etf-premium-tgw"
    exit 2
fi
if [[ ! -x ${build_dir}/etf-premium-tgw-audit ]]; then
    print -u2 "Release TGW 类型审计工具不存在：${build_dir}/etf-premium-tgw-audit"
    exit 2
fi
...
# The target host may have a different Homebrew simdjson ABI. Bundle the exact
# dylib used by the native TGW binaries and rewrite only those two executables
# to a package-relative path. Qt/OpenSSL/zstd already have compatible target
# installations, while simdjson intentionally changes its install-name major.
simdjson_dependency=$(/usr/bin/otool -L "${stage}/bin/etf-premium-tgw" \
    | /usr/bin/awk '/libsimdjson\.[0-9]+\.dylib/ {print $1; exit}')
if [[ -z ${simdjson_dependency} || ! -e ${simdjson_dependency} ]]; then
    print -u2 "无法定位原生 TGW 的 simdjson 动态库：${simdjson_dependency:-<empty>}"
    exit 2
fi
mkdir -p "${stage}/lib"
simdjson_name=${simdjson_dependency:t}
cp -L "${simdjson_dependency}" "${stage}/lib/${simdjson_name}"
/usr/bin/install_name_tool -id "@rpath/${simdjson_name}" "${stage}/lib/${simdjson_name}"
for binary in etf-premium-tgw etf-premium-tgw-audit; do
    /usr/bin/install_name_tool -change "${simdjson_dependency}" \
        "@executable_path/../lib/${simdjson_name}" "${stage}/bin/${binary}"
done
...
/usr/bin/codesign --force --sign "${codesign_identity}" "${stage}/bin/etf-premium-tgw"
/usr/bin/codesign --verify --strict --verbose=1 "${stage}/bin/etf-premium-tgw"
/usr/bin/codesign --force --sign "${codesign_identity}" "${stage}/bin/etf-premium-tgw-audit"
/usr/bin/codesign --verify --strict --verbose=1 "${stage}/bin/etf-premium-tgw-audit"
/usr/bin/codesign --force --sign "${codesign_identity}" "${stage}/lib/${simdjson_name}"
/usr/bin/codesign --verify --strict --verbose=1 "${stage}/lib/${simdjson_name}"

for binary in etf-premium-tgw etf-premium-tgw-audit; do
    if /usr/bin/otool -L "${stage}/bin/${binary}" | /usr/bin/grep -q '/opt/homebrew/.*/libsimdjson'; then
        print -u2 "${binary} 仍然依赖 Homebrew simdjson"
        exit 2
    fi
done
