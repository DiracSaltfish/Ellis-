...
private Q_SLOTS:
    void signalAndGlobalListsAreIndependentAndPersistent()
    {
...

        {
            ...
            auto *clearAll = window.findChild<QPushButton *>(QStringLiteral("clearAllSignalsButton"));
            QVERIFY(clearAll);
            QCOMPARE(clearAll->text(), QStringLiteral("一键移除全部信号"));
            QTest::mouseClick(clearAll, Qt::LeftButton);
            QCOMPARE(signals->rowCount(), 0);
            send(window, signal(QStringLiteral("159866.SZ"), 4, QStringLiteral("2026-08-27T10:03:00.000+08:00")));
            QCOMPARE(signals->rowCount(), 0);
            send(window, signal(QStringLiteral("159866.SZ"), 5, QStringLiteral("2026-08-27T10:04:00.000+08:00")));
            QCOMPARE(signals->rowCount(), 1);
    ...
    void liveSignalDoesNotShowOrActivateMainWindow()
    {
        QTemporaryDir directory;
        QVERIFY(directory.isValid());
        ClientSettings settings;
        ...
        MonitorWindow window(settings, directory.filePath(QStringLiteral("client-settings.json")), false);
        QVERIFY(!window.isVisible());

        const auto send = [&window](const QJsonObject &message) {
            window.processMessage(QString::fromUtf8(QJsonDocument(message).toJson(QJsonDocument::Compact)));
        };
        send(QJsonObject{{"type", "sync_begin"}, {"replay", false}});
        send(QJsonObject{{"type", "sync_complete"}});
        send(QJsonObject{{"type", "signal"}, {"signal_seq", 501}, {"symbol", "159866.SZ"},
                         {"name", "测试ETF"}, {"occurred_at", "2026-08-28T09:55:00.000+08:00"},
                         {"model", "premium"}, {"premium_ppm", 18'000},
                         {"reason", "不抢焦点回归"}, {"backfill", false}});
    ...
    void radarSignalDescriptionShowsFastAndMinuteEvidence()
    {
        QTemporaryDir directory;
        QVERIFY(directory.isValid());
        ClientSettings settings;
        ...
        send(QJsonObject{{"type", "sync_begin"}});
        send(QJsonObject{{"type", "summary"}, {"s", "159866.SZ"}, {"name", "测试ETF"},
                         {"last_price_e6", 1'005'000}, {"bid1_price_e6", 1'004'000},
                         {"iopv_e6", 0}, {"publish_wall_ns", QDateTime::currentMSecsSinceEpoch() * 1'000'000LL}});
        send(QJsonObject{{"type", "sync_complete"}});
        send(QJsonObject{{"type", "signal"}, {"signal_seq", 99}, {"symbol", "159866.SZ"},
                         {"name", "测试ETF"}, {"occurred_at", QDateTime::currentDateTime().toString(Qt::ISODateWithMs)},
                         {"model", "radar"}, {"reason", "M3+T30"}, {"bid_rise_30s_ppm", 1'600},
                         {"bid_rise_60s_ppm", 2'100}, {"bid_rise_90s_ppm", 2'600},
                         {"momentum_3m_ppm", 3'100}, {"momentum_5m_ppm", 0}, {"backfill", false}});

        auto *signals = window.findChild<QTableWidget *>(QStringLiteral("signalTable"));
        QVERIFY(signals);
        QCOMPARE(signals->rowCount(), 1);
        QVERIFY(signals->item(0, 6)->text().contains(QStringLiteral("M3+T30")));
        QVERIFY(signals->item(0, 6)->text().contains(QStringLiteral("快涨30/60/90s")));
        QVERIFY(signals->item(0, 6)->text().contains(QStringLiteral("M3/M5")));
    }
};
