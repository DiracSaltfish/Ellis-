...
private Q_SLOTS:
    void realtimeSignalsAndDiskAuditRemainDistinct()
    {
        ...
        QJsonObject historical{{"type", "signal"}, {"signal_seq", 7}, {"symbol", "159866.SZ"},
                               ...
                               {"bid_rise_30s_ppm", 1600}, {"bid_rise_60s_ppm", 2200},
                               {"bid_rise_90s_ppm", 2700}, {"momentum_3m_ppm", 3200},
                               {"momentum_5m_ppm", 5100}, {"adaptive_3m_ppm", 2600},
                               {"adaptive_5m_ppm", 5200}, {"minute_range_ppm", 3100},
                               {"minute_range_base_ppm", 1200},
                               {"model", "premium+pull+radar"}, {"reason", "30s+盘口150s+M3+T30"},
        ...
        QCOMPARE(liveTable->item(0, 3)->text(), QStringLiteral("溢价率 + 盘口拉涨 + 快速拉涨雷达"));
