...
- 信号页提供“一键移除全部信号”。它一次保存所有当前事件的移除水位并清空本机列表；不会删除 A 的 30 日历史审计，也不会影响其他 B。任一标的后续出现更新事件时仍会重新加入。
- 实时信号只按设置响铃并可显示被动系统通知。B 不调用主窗口的显示、置前或激活接口，因此不会抢占键盘焦点，也不会把 macOS 从当前全屏空间切换到 B 所在空间。
