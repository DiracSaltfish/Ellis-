...
ScheduleState MarketSchedule::stateAt(const QDateTime &localTime, bool forceQuotes) const
{
    ...
    if (weekday > 5) {
        state.cnQuotesDesired = forceQuotes;
        state.hkQuotesDesired = forceQuotes;
        state.quotesDesired = state.cnQuotesDesired || state.hkQuotesDesired;
        state.label = forceQuotes ? QStringLiteral("周末人工行情") : QStringLiteral("周末停机");
        return state;
    }

    state.cnQuotesDesired = (time >= QTime(9, 15) && time < QTime(15, 0)) || forceQuotes;
    state.hkQuotesDesired = (time >= QTime(9, 30) && time < QTime(16, 0)) || forceQuotes;
    state.quotesDesired = state.cnQuotesDesired || state.hkQuotesDesired;
    ...
    } else if (time >= QTime(15, 0) && time < QTime(16, 0)) {
        state.label = QStringLiteral("港股L1维护");
    } else {
