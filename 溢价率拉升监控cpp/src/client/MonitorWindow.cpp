...
QString signalDescription(const QJsonObject &object)
{
    ...
    QStringList metrics;
    if (model.contains(QStringLiteral("radar"))) {
        metrics.append(QStringLiteral("快涨30/60/90s %1/%2/%3%")
                           .arg(ppmToPercent(object.value(QStringLiteral("bid_rise_30s_ppm")).toInteger()), 0, 'f', 3)
                           .arg(ppmToPercent(object.value(QStringLiteral("bid_rise_60s_ppm")).toInteger()), 0, 'f', 3)
                           .arg(ppmToPercent(object.value(QStringLiteral("bid_rise_90s_ppm")).toInteger()), 0, 'f', 3));
        metrics.append(QStringLiteral("M3/M5 %1/%2%")
                           .arg(ppmToPercent(object.value(QStringLiteral("momentum_3m_ppm")).toInteger()), 0, 'f', 3)
                           .arg(ppmToPercent(object.value(QStringLiteral("momentum_5m_ppm")).toInteger()), 0, 'f', 3));
    }
    if (model.contains(QStringLiteral("pull"))) {
        metrics.append(QStringLiteral("盘口150/300s %1/%2%")
    ...
    if (model.contains(QStringLiteral("premium"))
        || object.value(QStringLiteral("iopv_e6")).toInteger() > 0
        || object.value(QStringLiteral("premium_ppm")).toInteger() != 0) {
        metrics.append(QStringLiteral("可卖溢价 %1%")
                           .arg(ppmToPercent(object.value(QStringLiteral("premium_ppm")).toInteger()), 0, 'f', 3));
    } else {
        metrics.append(QStringLiteral("无IOPV"));
    }
    return QStringLiteral("⚡ %1 · %2").arg(object.value(QStringLiteral("reason")).toString(),
...
void MonitorWindow::buildUi()
{
    ...
    auto *signalHint = new QLabel(QStringLiteral("最近触发置顶；同一标的只保留最新一次。移除只影响本机 B，A 的历史审计不会改变。"));
    signalHint->setObjectName(QStringLiteral("listHint"));
    auto *clearSignals = new QPushButton(QStringLiteral("一键移除全部信号"));
    clearSignals->setObjectName(QStringLiteral("clearAllSignalsButton"));
    clearSignals->setToolTip(QStringLiteral("清空本机 B 的信号列表；不改变 A 的历史审计、观察清单或 TGW 订阅"));
    connect(clearSignals, &QPushButton::clicked, this, &MonitorWindow::clearAllSignalRows);
    signalHeader->addWidget(signalHint, 1);
    signalHeader->addWidget(clearSignals);
    ...
    setStyleSheet(QStringLiteral(R"(
        ...
        QPushButton#clearAllSignalsButton { color:#b42318; background:#fff5f4; border-color:#f0b4ae; padding:6px 12px; }
        QPushButton#clearAllSignalsButton:hover { background:#ffe5e1; }
...
void MonitorWindow::clearAllSignalRows()
{
    const int removedCount = signalTable_->rowCount();
    if (removedCount == 0) {
        statusBar()->showMessage(QStringLiteral("本机信号列表已经为空。"), 5'000);
        return;
    }
    for (auto it = latestSignals_.constBegin(); it != latestSignals_.constEnd(); ++it) {
        dismissedSignals_.insert(it.key(), it.value());
    }
    signalTable_->setRowCount(0);
    signalItems_.clear();
    latestSignals_.clear();
    listTabs_->setTabText(0, QStringLiteral("信号列表 (0)"));
    saveSignalList();
    statusBar()->showMessage(
        QStringLiteral("已从本机信号列表移除全部 %1 项；A 的历史审计、观察清单和订阅均未改变。")
            .arg(removedCount),
        8'000);
}
