...
FAMILIES=['HSI','HHI','HSTECH','HSI','HHI','HSTECH','HSTECH','EV_PROXY']
HORIZONS=[5,15,30,60]
...
def prepare():
    with gzip.open(ROOT/'data/raw/pilot_520600.jsonl.gz','rt') as f:days=[json.loads(line) for line in f]
    fx=pd.read_csv(ROOT/'data/raw/sse_settlement_rates.csv',dtype={'适用日期':str}).set_index('适用日期')
    assert fx.index.is_unique
    futures={}
    for name in TOOLS[:3]:
        f=pd.read_csv(ROOT/f'data/raw/{name}_1min.csv',dtype={'contract_month':str})
    ...
    for day in days:
        ...
        exclude='00489' in day['missing_members']
        ...
        for c in day['components']:
            ...
            elif code=='02402' and '20260401'<=date<'20260504' and code in last_observed:
    ...
    output.to_parquet(ROOT/'data/normalized/pilot_minutes.parquet',index=False)
    pd.DataFrame(quality).to_csv(ROOT/'reports/data_quality.csv',index=False)
    pd.DataFrame(component_weights).to_csv(ROOT/'reports/component_weights.csv',index=False)
...
def walkforward(sample,horizon):
    ...
    out=[];folds=[]
    for index in range(60,len(dates)):
        train_dates=dates[index-60:index];test_date=dates[index]
        fit=sample[sample.date.isin(train_dates[:50])];valid=sample[sample.date.isin(train_dates[50:])]
        ...
        folds.append({'horizon':horizon,'test_date':test_date,'fit_start':train_dates[0],'inner_fit_end':train_dates[49],
                      'validation_start':train_dates[50],'train_end':train_dates[-1],
...
def main():
    ...
    if args.cached:
        points=pd.read_parquet(ROOT/'data/normalized/pilot_minutes.parquet');quality=pd.read_csv(ROOT/'reports/data_quality.csv')
    else:points,quality=prepare()
    summaries=[];all_oos=[];all_folds=[];label_audit=[]
    for h in HORIZONS:
        data=labels(points,h)
        data.to_parquet(ROOT/f'data/normalized/labels_{h}m.parquet',index=False)
        oos,folds=walkforward(data,h);all_oos.append(oos);all_folds.append(folds)
        for name,part in oos.groupby('model'):
            ...
            if name=='selected':summary['variance_reduction_ci95']=bootstrap_daily(part)
    ...
    combined.to_parquet(ROOT/'reports/oos_residuals.parquet',index=False)
    folds.to_csv(ROOT/'reports/folds.csv',index=False)
    pd.DataFrame(summaries).to_csv(ROOT/'reports/model_comparison.csv',index=False)
    results={'scope':'INITIAL_PRICE_ONLY_RESEARCH','target':'520600.SH','window':['2026-03-03','2026-08-03'],
    ...
    (ROOT/'reports/results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2))
