...
struct Row{std::string code;int64_t id;uint64_t seq=0;int error=0;double received=0;std::vector<unsigned char> fields,bytes;bool dirty=false;};
alignas(16) static unsigned char module[0x28];
static int latencyMs=60000;static Create createSub;static Modify modifySub;static Stop stopSub;
static std::mutex mu;static std::map<int64_t,Row> rows;static NSString *directory;static NSArray *desired=@[];static NSMutableArray *timings;static NSString *lastCommand=@"",*lastError=@"";static bool started=false,busy=false;static double lastHeartbeat=0,maxLag=0;static uint64_t heartbeats=0,unknown=0;static dispatch_source_t manager,writer;
...
static void atomicJSON(NSString*name,id obj){@autoreleasepool{NSData*d=[NSJSONSerialization dataWithJSONObject:obj options:0 error:nil];if(d){NSString*p=[directory stringByAppendingPathComponent:name];[d writeToFile:p options:NSDataWritingAtomic error:nil];chmod(p.fileSystemRepresentation,0600);}}}
static void callback(uint32_t id,int32_t err,const char*,const void*ptr){
 ...
 if(!fp||!bp||fs>1048576||bs>1048576){r.error=-21010;return;}r.fields.assign((unsigned char*)fp,(unsigned char*)fp+fs);r.bytes.assign((unsigned char*)bp,(unsigned char*)bp+bs);
}
static void exportData(){@autoreleasepool{
 ...
 {std::lock_guard<std::mutex>l(mu);for(auto&kv:rows){auto&r=kv.second;[items addObject:@{@"symbol":@(r.code.c_str()),@"sub_id":@(r.id),@"callback_seq":@(r.seq),@"error":@(r.error)}];if(r.dirty){changed.push_back(r);r.dirty=false;}}
 err=lastError;command=lastCommand;perf=[timings copy];lag=maxLag;hb=heartbeats;beat=lastHeartbeat;un=unknown;active=busy;}
 for(auto&r:changed){NSString*code=@(r.code.c_str());atomicJSON([NSString stringWithFormat:@"capture_%@.json",code],@{@"requested_windcode":code,@"sub_id":@(r.id),@"source_pid":@(getpid()),@"callback_seq":@(r.seq),@"callback_epoch_ms":@((int64_t)(r.received*1000)),@"error_code":@(r.error),@"field_info":@{@"size":@(r.fields.size()),@"hex":hex(r.fields)},@"buffer_58":@{@"size":@(r.bytes.size()),@"hex":hex(r.bytes)}});}
 atomicJSON(@"state.json",@{@"pid":@(getpid()),@"latency_ms":@(latencyMs),@"items":items,@"command":command,@"error":err?:@"",@"busy":@(active),@"operations":perf,@"heartbeat_count":@(hb),@"heartbeat_monotonic":@(beat),@"max_main_queue_lag_ms":@(lag*1000),@"unknown_callbacks":@(un),@"time":@([[NSDate date] timeIntervalSince1970])});
}}
static bool valid(NSString*c){if(![c isKindOfClass:NSString.class]||c.length!=9||![c hasSuffix:@".SZ"]||![c hasPrefix:@"1"])return false;for(int i=0;i<6;i++)if([c characterAtIndex:i]<'0'||[c characterAtIndex:i]>'9')return false;return true;}
static void tick(){@autoreleasepool{
 ...
 std::string removeCode,addCode;int64_t removeId=-1;
 {std::lock_guard<std::mutex>l(mu);lastHeartbeat=mono();heartbeats++;if(!lastError.length){
 for(auto&kv:rows)if(![desired containsObject:@(kv.second.code.c_str())]){removeId=kv.first;removeCode=kv.second.code;break;}
 if(removeId<0)for(NSString*c in desired){bool found=false;for(auto&kv:rows)if(kv.second.code==c.UTF8String)found=true;if(!found){addCode=c.UTF8String;break;}}
 ...
 double t=mono();std::string empty="SELECT etfbuynumber,etfbuyamount,etfbuymoney,etfsellnumber,etfsellamount,etfsellmoney FROM ETFComprehensive.WholeETFData WHERE windcode = '' LATENCY("+std::to_string(latencyMs)+" MS)";
 auto id=createSub(module,empty.c_str(),false);if(id<0){std::lock_guard<std::mutex>l(mu);lastError=@"create failed";return;}
 {std::lock_guard<std::mutex>l(mu);rows.emplace(id,Row{addCode,id});}
 std::string sql="SELECT etfbuynumber,etfbuyamount,etfbuymoney,etfsellnumber,etfsellamount,etfsellmoney FROM ETFComprehensive.WholeETFData WHERE windcode = '"+addCode+"' LATENCY("+std::to_string(latencyMs)+" MS)";auto result=modifySub(module,id,sql.c_str());
...
extern "C" __attribute__((visibility("default"))) int once_start(const char*path){@autoreleasepool{
 if(started)return -1;if(!path||strlen(path)>700)return -2;directory=@(path);timings=[NSMutableArray array];
 ...
 dispatch_source_set_timer(manager,dispatch_time(DISPATCH_TIME_NOW,500*NSEC_PER_MSEC),200*NSEC_PER_MSEC,10*NSEC_PER_MSEC);
 ...
 NSArray*codes=c[@"symbols"];bool ok=[codes isKindOfClass:NSArray.class]&&codes.count<=97;
 if(ok)for(id code in codes)if(!valid(code)){ok=false;break;}
 if(ok&&[NSSet setWithArray:codes].count==codes.count){int next=[c[@"latency_ms"] intValue];if(next!=60000&&next!=5000&&next!=1000){lastError=@"invalid latency";return;}if(!rows.empty()&&next!=latencyMs){lastError=@"stop before changing latency";return;}latencyMs=next;maxLag=0;desired=[codes copy];lastError=@"";lastCommand=c[@"id"];}else{lastError=@"invalid command rejected";lastCommand=c[@"id"];}
 }}
 ...
 dispatch_source_set_timer(writer,dispatch_time(DISPATCH_TIME_NOW,NSEC_PER_SEC),NSEC_PER_SEC,50*NSEC_PER_MSEC);dispatch_source_set_event_handler(writer,^{exportData();});
