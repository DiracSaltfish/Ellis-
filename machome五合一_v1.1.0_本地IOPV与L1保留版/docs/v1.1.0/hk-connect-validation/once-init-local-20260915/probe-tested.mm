...
static void exportData(){@autoreleasepool{
 ...
 for(auto&r:changed){NSString*code=@(r.code.c_str());atomicJSON([NSString stringWithFormat:@"capture_%@.json",code],@{@"requested_windcode":code,@"sub_id":@(r.id),@"source_pid":@(getpid()),@"callback_seq":@(r.seq),@"callback_epoch_ms":@(r.received*1000),@"error_code":@(r.error),@"field_info":@{@"size":@(r.fields.size()),@"hex":hex(r.fields)},@"buffer_58":@{@"size":@(r.bytes.size()),@"hex":hex(r.bytes)}});}
 atomicJSON(@"state.json",@{@"pid":@(getpid()),@"items":items,@"command":command,@"error":err?:@"",@"busy":@(active),@"operations":perf,@"heartbeat_count":@(hb),@"heartbeat_monotonic":@(beat),@"max_main_queue_lag_ms":@(lag*1000),@"unknown_callbacks":@(un),@"time":@([[NSDate date] timeIntervalSince1970])});
}}
...
static void tick(){@autoreleasepool{
 ...
 double t=mono();const char*empty="SELECT etfbuynumber,etfbuyamount,etfbuymoney,etfsellnumber,etfsellamount,etfsellmoney FROM ETFComprehensive.WholeETFData WHERE windcode = '' LATENCY(60000 MS)";
 auto id=createSub(module,empty,false);if(id<0){std::lock_guard<std::mutex>l(mu);lastError=@"create failed";return;}
 {std::lock_guard<std::mutex>l(mu);rows.emplace(id,Row{addCode,id});}
 std::string sql="SELECT etfbuynumber,etfbuyamount,etfbuymoney,etfsellnumber,etfsellamount,etfsellmoney FROM ETFComprehensive.WholeETFData WHERE windcode = '"+addCode+"' LATENCY(60000 MS)";auto result=modifySub(module,id,sql.c_str());
...
extern "C" __attribute__((visibility("default"))) int once_start(const char*path){@autoreleasepool{
 ...
 if(ok&&[NSSet setWithArray:codes].count==codes.count){desired=[codes copy];lastError=@"";lastCommand=c[@"id"];}else{lastError=@"invalid command rejected";lastCommand=c[@"id"];}
 }}
