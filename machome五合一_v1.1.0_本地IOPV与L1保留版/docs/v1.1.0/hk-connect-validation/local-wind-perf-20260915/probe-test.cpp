...
static int initialise_module(void) {
    ...
    write_status("before_register",0,"module copied");
    register_cb(g_module, (void *)subscription_callback);
    write_status("after_register",0,"callback registered");
...
int64_t wind_tbapi_subscribe(const char *windcode, int latency_ms) {
    ...
    snprintf(create_sql, sizeof(create_sql),
        ...
        "FROM ETFComprehensive.WholeETFData WHERE windcode = '' LATENCY(%d MS)", latency_ms);
    g_callback_count = 0;
    write_status("before_create",0,create_sql);
    ...
    int n = snprintf(sql, sizeof(sql),
        "SELECT etfbuynumber, etfbuyamount, etfbuymoney, etfsellnumber, etfsellamount, etfsellmoney "
        "FROM ETFComprehensive.WholeETFData WHERE windcode = '%s' LATENCY(%d MS)", windcode, latency_ms);
