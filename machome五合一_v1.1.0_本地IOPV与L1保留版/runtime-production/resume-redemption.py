...
cid=str(uuid.uuid4());send(dict(type='command',command_id=cid,module_id='redemption',action=sys.argv[1] if len(sys.argv)>1 else 'redemption_monitor_start',arguments={},expected_revision=rev,deadline_ms=30000,requested_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),requested_by='production-deployment',reason='恢复切换前已运行的申赎采集监控',agent_instance_id=h['instance_id'],config_sha256=cfg))
