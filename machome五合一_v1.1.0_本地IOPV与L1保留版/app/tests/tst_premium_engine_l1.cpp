...
private Q_SLOTS:
    void iopvLeaseCannotReleaseExternalClientSymbol() {
        LegacyL1Server::Limits limits;limits.unsubscribeGraceSeconds=0;
        LegacyL1Server server({}, {}, limits);QString error;
        QVERIFY(server.listen(QHostAddress::LocalHost,0,&error));
        QTcpSocket iopv,external;QByteArray a,b;
        iopv.connectToHost(QHostAddress::LocalHost,server.listeningPort());QVERIFY(iopv.waitForConnected());
        external.connectToHost(QHostAddress::LocalHost,server.listeningPort());QVERIFY(external.waitForConnected());
        QCOMPARE(readType(iopv,a,"hello").value("v").toInt(),1);
        QCOMPARE(readType(external,b,"hello").value("v").toInt(),1);
        const auto subscribe=QByteArrayLiteral("{\"v\":1,\"t\":\"subscribe\",\"symbols\":[\"00700.HK\"]}\n");
        iopv.write(subscribe);iopv.flush();QCOMPARE(readType(iopv,a,"ack").value("symbols").toArray().size(),1);
        external.write(subscribe);external.flush();QCOMPARE(readType(external,b,"ack").value("symbols").toArray().size(),1);
        iopv.abort();QTest::qWait(50);
        QVERIFY(server.maintainedSymbols().contains("00700.HK"));
        QuoteSnapshot quote;quote.symbol="00700.HK";quote.origTime=20260909103000000LL;quote.lastPriceE6=500000000;server.publish(quote);
        QJsonObject book;for(int i=0;i<4;++i){const auto msg=readType(external,b,"l1");if(!msg.value("books").toArray().isEmpty()){book=msg.value("books").toArray()[0].toObject();break;}}
        QCOMPARE(book.value("lp").toDouble(),500.0);
    }
...
#include "tst_premium_engine_l1.moc"
