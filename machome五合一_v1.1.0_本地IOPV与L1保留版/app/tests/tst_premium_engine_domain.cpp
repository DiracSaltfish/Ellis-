...
using namespace machome::premium::engine;
...
QuoteSnapshot quoteWithLast(qint64 bid, qint64 last, qint64 iopv = 1'000'000)
{
    QuoteSnapshot value = quote(bid, iopv);
    value.lastPriceE6 = last;
    return value;
}
...
class DomainTests final : public QObject {
    Q_OBJECT
private Q_SLOTS:
    void localClientValuationContract() {
        const auto now = QDateTime::fromString("2026-09-09T10:00:00+08:00", Qt::ISODate);
        QJsonObject raw{{"type","detail"},{"s","513090.SH"},{"iopv_e6",2000000},{"bid1_price_e6",1010000},{"last_price_e6",1020000}};
        QJsonObject nav{{"trade_date","2026-09-09"},{"calculated_at",now.toString(Qt::ISODateWithMs)},{"midpoint_iopv",1.0},{"settlement_buy_iopv",1.01}};
        auto result = withLocalValuation(raw,nav,"midpoint",now);
        QCOMPARE(result.value("iopv_e6").toInteger(),1000000LL);
        QCOMPARE(result.value("sell_premium_ppm").toInteger(),10000LL);
        QCOMPARE(result.value("exchange_iopv_e6").toInteger(),2000000LL);
        QCOMPARE(raw.value("iopv_e6").toInteger(),2000000LL);
        QCOMPARE(withLocalValuation(raw,nav,"settlement_buy",now).value("iopv_e6").toInteger(),1010000LL);
        QCOMPARE(withLocalValuation(raw,nav,"midpoint",now.addSecs(13)).value("iopv_e6").toInteger(),0LL);
        QCOMPARE(withLocalValuation(raw,nav,"midpoint",now.addDays(1)).value("iopv_e6").toInteger(),0LL);
        QCOMPARE(withLocalValuation(raw,{},"midpoint",now).value("iopv_e6").toInteger(),0LL);
    }
    void localValuationDrivesSignalsWithoutChangingRawQuote() {
        QuoteWorker worker;bool signalled=false;QJsonObject event;QuoteSnapshot output;
        connect(&worker,&QuoteWorker::resultReady,this,[&](const QuoteSnapshot &q,const QJsonObject &s,bool has,qint64,qint64,quint64){output=q;signalled=has;event=s;});
        auto now=QDateTime::fromString("2026-09-09T10:00:00+08:00",Qt::ISODate);
        QJsonObject valuation{{"calculated_at",now.toString(Qt::ISODateWithMs)},{"midpoint_iopv",1.0},{"etf_price",1.016},{"reasons",QJsonArray{}},{"run_id","one"},{"trade_date","2026-09-09"},{"pcf_sha256","pcf"}};
        auto data=fullData();data["IOPV"]=2000000;data["bid_price"]=repeated(1016000,-1000);data["last_price"]=1016000;data["offer_price"]=repeated(1017000,1000);data["high_price"]=1030000;
        worker.process(frameFor("local",false,data),now,true,true,true,true,1,true,valuation,"midpoint");
        QVERIFY(!signalled);
        data["bid_price"]=repeated(1019000,-1000);data["last_price"]=1019000;data["offer_price"]=repeated(1020000,1000);
        worker.process(frameFor("local",false,data),now.addSecs(1),true,true,true,true,1,true,valuation,"midpoint");
        QVERIFY(signalled);QCOMPARE(event.value("valuation_source").toString(),QString("local_pcf"));
        QCOMPARE(output.iopvE6,2000000LL); // Original exchange field remains original.
        QCOMPARE(event.value("premium_ppm").toInteger(),19000LL);
        worker.process(frameFor("local",false,data),now.addSecs(20),true,true,true,true,1,true,valuation,"midpoint");
        QVERIFY(!signalled); // Stale local valuation cannot fall back to exchange IOPV.
    }
    void goldenReplayFixture()
    {
        QFile file(QStringLiteral(MACHOME_TEST_SOURCE_DIR)
                   + QStringLiteral("/fixtures/premium/golden_full_snapshot.json"));
        QVERIFY(file.open(QIODevice::ReadOnly));
        const QJsonObject fixture = QJsonDocument::fromJson(file.readAll()).object();
        QVERIFY(fixture.value(QStringLiteral("provenance")).toObject()
                    .value(QStringLiteral("sanitized")).toBool());
        SnapshotParser parser;
        const auto parsed = parser.consume(frameFor(
            QStringLiteral("golden-replay"), false,
            fixture.value(QStringLiteral("frame")).toObject()), true);
        QVERIFY(parsed.snapshot.has_value());
        const QJsonObject expected = fixture.value(QStringLiteral("expected")).toObject();
        QCOMPARE(parsed.snapshot->symbol, expected.value(QStringLiteral("symbol")).toString());
        QCOMPARE(parsed.snapshot->lastPriceE6,
                 expected.value(QStringLiteral("last_price_e6")).toInteger());
        QCOMPARE(parsed.snapshot->bidPricesE6[0],
                 expected.value(QStringLiteral("bid1_price_e6")).toInteger());
        QCOMPARE(parsed.snapshot->iopvE6,
                 expected.value(QStringLiteral("iopv_e6")).toInteger());
        QCOMPARE(parsed.snapshot->qualityIssues.size(),
                 expected.value(QStringLiteral("quality_issue_count")).toInt());
    }
    ...
    void pullModelWithIopvRequiresPremiumGateAndStrict150Threshold()
    {
        ...
        QVERIFY(!engine.evaluate(quoteWithLast(995'000, 1'000'000), base, true, true).event.has_value());
        for (int second = 3; second < 150; second += 3) {
            QVERIFY(!engine.evaluate(quoteWithLast(995'000, 1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        auto gated = engine.evaluate(quoteWithLast(1'004'100, 1'000'000), base.addSecs(150), true, true);
        QVERIFY(!gated.event.has_value());
        QCOMPARE(gated.snapshot.bidRise150sPpm, calculateRatioPpm(1'004'100, 995'000));

        engine.resetAll();
        QVERIFY(!engine.evaluate(quoteWithLast(1'002'000, 1'000'000), base, true, true).event.has_value());
        for (int second = 3; second < 150; second += 3) {
            QVERIFY(!engine.evaluate(quoteWithLast(1'002'000, 1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        auto triggered = engine.evaluate(quoteWithLast(1'006'100, 1'000'000), base.addSecs(150), true, true);
        QVERIFY(triggered.event.has_value());
        ...
        engine.resetAll();
        QVERIFY(!engine.evaluate(quoteWithLast(1'000'000, 1'000'000), base, true, true).event.has_value());
        for (int second = 3; second < 150; second += 3) {
            QVERIFY(!engine.evaluate(quoteWithLast(1'000'000, 1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        auto exact = engine.evaluate(quoteWithLast(1'004'000, 1'000'000), base.addSecs(150), true, true);
    ...
    void pullModelWithoutIopvTriggersAndRequiresCompleteWindows()
    {
        auto noIopv = [](qint64 bid) {
            QuoteSnapshot value = quoteWithLast(bid, 1'000'000, 0);
        ...
        for (int second = 3; second < 149; second += 3) {
            QVERIFY(!engine.evaluate(noIopv(1'000'000), base.addSecs(second), true, true).event.has_value());
        }
    ...
    void pullModel300SecondThresholdIsStrict()
    {
        ...
        for (int second = 3; second < 300; second += 3) {
            engine.evaluate(noIopv(1'000'000), base.addSecs(second), false, true);
        }
        auto exact = engine.evaluate(noIopv(1'008'000), base.addSecs(300), false, true);
        QCOMPARE(exact.snapshot.bidRise300sPpm, 8'000);
        QVERIFY(!exact.event.has_value());
        QCOMPARE(exact.snapshot.momentum5mPpm, 8'000);
        auto above = engine.evaluate(noIopv(1'008'100), base.addSecs(301), false, true);
        QVERIFY(above.event.has_value());
        QCOMPARE(above.event->model, QStringLiteral("pull"));
        QVERIFY(above.event->reason.contains(QStringLiteral("盘口300s")));
    }

    void radarRequiresUsableIopvAndStrictPremiumGate()
    {
        auto noIopv = [](qint64 price) {
            QuoteSnapshot value = quote(price, 0);
            value.qualityIssues.append(QStringLiteral("iopv_non_positive"));
            return value;
        };
        SignalEngine engine;
        const QDateTime base(QDate(2026, 8, 28), QTime(10, 0), QTimeZone::systemTimeZone());
        QVERIFY(!engine.evaluate(noIopv(1'000'000), base, true, true).event.has_value());
        QVERIFY(!engine.evaluate(noIopv(1'000'000), base.addSecs(60), true, true).event.has_value());
        QVERIFY(!engine.evaluate(noIopv(1'000'000), base.addSecs(120), true, true).event.has_value());
        const auto missing = engine.evaluate(noIopv(1'003'000), base.addSecs(180), true, true);
        QVERIFY(!missing.event.has_value());
        QCOMPARE(missing.snapshot.momentum3mPpm, 3'000);

        SignalEngine negativeGate;
        for (int minute = 0; minute < 3; ++minute) {
            QVERIFY(!negativeGate.evaluate(quoteWithLast(995'000, 1'000'000),
                                           base.addSecs(minute * 60), true, true).event.has_value());
        }
        const auto negative = negativeGate.evaluate(quoteWithLast(995'000, 1'003'000),
                                                    base.addSecs(180), true, true);
        QCOMPARE(negative.snapshot.sellPremiumPpm, -5'000);
        QCOMPARE(negative.snapshot.momentum3mPpm, 3'000);
        QVERIFY(!negative.event.has_value());

        SignalEngine exactGate;
        for (int minute = 0; minute < 3; ++minute) {
            QVERIFY(!exactGate.evaluate(quoteWithLast(1'006'000, 1'000'000),
                                        base.addSecs(minute * 60), true, true).event.has_value());
        }
        const auto exact = exactGate.evaluate(quoteWithLast(1'006'000, 1'003'000),
                                              base.addSecs(180), true, true);
        QCOMPARE(exact.snapshot.sellPremiumPpm, 6'000);
        QCOMPARE(exact.snapshot.momentum3mPpm, 3'000);
        QVERIFY(!exact.event.has_value());

        SignalEngine aboveGate;
        for (int minute = 0; minute < 3; ++minute) {
            QVERIFY(!aboveGate.evaluate(quoteWithLast(1'006'100, 1'000'000),
                                        base.addSecs(minute * 60), true, true).event.has_value());
        }
        const auto triggered = aboveGate.evaluate(quoteWithLast(1'006'100, 1'003'000),
                                                  base.addSecs(180), true, true);
        QCOMPARE(triggered.snapshot.sellPremiumPpm, 6'100);
        QVERIFY(triggered.event.has_value());
        QCOMPARE(triggered.event->model, QStringLiteral("radar"));
        QVERIFY(triggered.event->reason.contains(QStringLiteral("M3")));
    }

    void radarAdaptiveBranchIsAnIndependentOrCondition()
    {
        auto radarQuote = [](qint64 price) {
            return quoteWithLast(1'006'100, price);
        };
        SignalEngine engine;
        const QDateTime base(QDate(2026, 8, 28), QTime(10, 0), QTimeZone::systemTimeZone());
        for (int minute = 0; minute < 3; ++minute) {
            QVERIFY(!engine.evaluate(radarQuote(1'000'000), base.addSecs(minute * 60), true, true).event.has_value());
        }
        const auto triggered = engine.evaluate(radarQuote(1'001'600), base.addSecs(180), true, true);
        QVERIFY(triggered.event.has_value());
        QCOMPARE(triggered.event->model, QStringLiteral("radar"));
        QVERIFY(triggered.event->reason.contains(QStringLiteral("A3")));
        QVERIFY(!triggered.event->reason.contains(QStringLiteral("M3")));
        QCOMPARE(triggered.snapshot.adaptive3mPpm, 1'600);
    }

    void configurableRadarPremiumGate()
    {
      const QDateTime base(QDate(2026,8,28),QTime(10,0),QTimeZone::systemTimeZone());
      auto run=[&](qint64 threshold){SignalEngine e;e.setPremiumThresholds(15000,6000,threshold);
        e.evaluate(quote(1000000,995000),base,true,true);
        for(int sec=3;sec<24;sec+=3)e.evaluate(quote(1000000,995000),base.addSecs(sec),true,true);
        e.evaluate(quote(1000500,995000),base.addSecs(24),true,true);
        e.evaluate(quote(1001000,995000),base.addSecs(27),true,true);
        return e.evaluate(quote(1001500,995000),base.addSecs(30),true,true);};
      QVERIFY(run(6000).event.has_value());
      QVERIFY(!run(10000).event.has_value());
    }

    void radarFastTickRequiresCompleteWindowTwoUpMovesAndNewHigh()
    {
        auto radarQuote = [](qint64 price) {
            return quote(price, 995'000);
        };
        const QDateTime base(QDate(2026, 8, 28), QTime(10, 0), QTimeZone::systemTimeZone());
        SignalEngine oneJump;
        QVERIFY(!oneJump.evaluate(radarQuote(1'000'000), base, true, true).event.has_value());
        for (int second = 3; second < 30; second += 3) {
            QVERIFY(!oneJump.evaluate(radarQuote(1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        const auto rejected = oneJump.evaluate(radarQuote(1'002'000), base.addSecs(30), true, true);
        QVERIFY(!rejected.event.has_value());
        QCOMPARE(rejected.snapshot.bidRise30sPpm, 2'000);

        SignalEngine engine;
        QVERIFY(!engine.evaluate(radarQuote(1'000'000), base, true, true).event.has_value());
        for (int second = 3; second < 24; second += 3) {
            QVERIFY(!engine.evaluate(radarQuote(1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        QVERIFY(!engine.evaluate(radarQuote(1'000'500), base.addSecs(24), true, true).event.has_value());
        QVERIFY(!engine.evaluate(radarQuote(1'001'000), base.addSecs(27), true, true).event.has_value());
        const auto first = engine.evaluate(radarQuote(1'001'500), base.addSecs(30), true, true);
        QVERIFY(first.event.has_value());
        QCOMPARE(first.event->model, QStringLiteral("radar"));
        QVERIFY(first.event->reason.contains(QStringLiteral("T30")));
        QVERIFY(!first.event->repeat);
        QVERIFY(!engine.evaluate(radarQuote(1'001'500), base.addSecs(33), true, true).event.has_value());
        const auto newHigh = engine.evaluate(radarQuote(1'004'500), base.addSecs(36), true, true);
        QVERIFY(newHigh.event.has_value());
        QVERIFY(newHigh.event->repeat);
    }

    void radarFastTickRejectsWindowAfterLongDataGap()
    {
        auto radarQuote = [](qint64 price) {
            return quote(price, 995'000);
        };
        ...
        SignalEngine engine;
        QVERIFY(!engine.evaluate(radarQuote(1'000'000), base, true, true).event.has_value());
        QVERIFY(!engine.evaluate(radarQuote(1'000'500), base.addSecs(120), true, true).event.has_value());
        const auto afterGap = engine.evaluate(radarQuote(1'002'000), base.addSecs(150), true, true);
        QVERIFY(!afterGap.event.has_value());
        QCOMPARE(afterGap.snapshot.bidRise30sPpm, 0);
    }

    void radarRangeExpansionMustStillPointUpward()
    {
        auto radarQuote = [](qint64 price) {
            return quote(price, 990'000);
        };
        const QDateTime base(QDate(2026, 8, 28), QTime(10, 0), QTimeZone::systemTimeZone());
        SignalEngine upward;
        upward.evaluate(radarQuote(1'000'000), base, true, true);
        upward.evaluate(radarQuote(1'000'000), base.addSecs(30), true, true);
        upward.evaluate(radarQuote(1'000'000), base.addSecs(60), true, true);
        const auto up = upward.evaluate(radarQuote(1'001'500), base.addSecs(70), true, true);
        QVERIFY(up.event.has_value());
        QVERIFY(up.event->reason.contains(QStringLiteral("RANGE_UP")));

        SignalEngine downward;
        downward.evaluate(radarQuote(1'000'000), base, true, true);
        downward.evaluate(radarQuote(999'900), base.addSecs(30), true, true);
        QVERIFY(!downward.evaluate(radarQuote(1'002'000), base.addSecs(60), true, true).event.has_value());
        const auto down = downward.evaluate(radarQuote(999'000), base.addSecs(80), true, true);
        QVERIFY(!down.event.has_value());
        QVERIFY(down.snapshot.minuteRangePpm >= 2'900);
    }

    void allThreeModelsCoalesceIntoOneOrEvent()
    {
        SignalEngine engine;
        const QDateTime base(QDate(2026, 8, 28), QTime(10, 0), QTimeZone::systemTimeZone());
        engine.evaluate(quote(1'000'000), base, true, true);
        for (int second = 3; second < 180; second += 3) {
            QVERIFY(!engine.evaluate(quote(1'000'000), base.addSecs(second), true, true).event.has_value());
        }
        const auto triggered = engine.evaluate(quote(1'017'000), base.addSecs(180), true, true);
        QVERIFY(triggered.event.has_value());
        QCOMPARE(triggered.event->model, QStringLiteral("premium+pull+radar"));
        QVERIFY(triggered.event->reason.contains(QStringLiteral("5m")));
        QVERIFY(triggered.event->reason.contains(QStringLiteral("盘口150s")));
        QVERIFY(triggered.event->reason.contains(QStringLiteral("M3")));
        QVERIFY(!triggered.event->repeat);
    }
    ...
    void completeLongResidenceScheduleBoundaries()
    {
        MarketSchedule schedule;
        const QDate weekday(2026, 8, 27);
        const auto at = [&](int h, int m, int s = 0) {
            return schedule.stateAt(QDateTime(weekday, QTime(h, m, s)), false);
        };
        QVERIFY(!at(9, 14, 59).quotesDesired);
        QCOMPARE(at(9, 15).phase, ScheduleState::Phase::AuctionWarmup);
        QVERIFY(at(9, 15).cnQuotesDesired);
        QCOMPARE(at(9, 30).phase, ScheduleState::Phase::MorningWarmup);
        QVERIFY(at(9, 30).resetWindow);
        QVERIFY(!at(9, 30, 1).resetWindow);
        QCOMPARE(at(9, 31).phase, ScheduleState::Phase::MorningSignals);
        QVERIFY(at(9, 31).allow30SecondSignal);
        QVERIFY(!at(9, 34, 59).allow300SecondSignal);
        QVERIFY(at(9, 35).allow300SecondSignal);
        QCOMPARE(at(11, 30).phase, ScheduleState::Phase::Lunch);
        QVERIFY(!at(11, 30).allow30SecondSignal);
        QCOMPARE(at(13, 0).phase, ScheduleState::Phase::AfternoonWarmup);
        QVERIFY(at(13, 0).resetWindow);
        QCOMPARE(at(13, 1).phase, ScheduleState::Phase::AfternoonSignals);
        QVERIFY(!at(13, 4, 59).allow300SecondSignal);
        QVERIFY(at(13, 5).allow300SecondSignal);
        QCOMPARE(at(14, 57).phase, ScheduleState::Phase::ClosingAuction);
        QVERIFY(!at(14, 57).allow30SecondSignal);
        QVERIFY(!at(15, 0).cnQuotesDesired);
        QVERIFY(at(15, 0).hkQuotesDesired);
        QVERIFY(!at(16, 0).quotesDesired);
        QVERIFY(!schedule.stateAt(QDateTime(QDate(2026, 8, 29), QTime(9, 31)),
                                  false).quotesDesired);
    }
};

QTEST_MAIN(DomainTests)
#include "tst_premium_engine_domain.moc"
