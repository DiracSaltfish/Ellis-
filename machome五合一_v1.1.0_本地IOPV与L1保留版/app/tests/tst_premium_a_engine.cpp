...
private Q_SLOTS:
    ...
    void v32LocalChannelsHandshakeAndCoverage() {
        QTemporaryDir root;
        const auto port = reserveLocalPort();
        QVERIFY(port != 0);
        PremiumAEngine engine;
        engine.initialize({"premium",root.path(),{{"summary_port",port},{"l1_port",0},{"listen_host","127.0.0.1"},{"replay",true}},true});
        engine.start();
        QWebSocket summary,detail;
        QList<QJsonObject> summaries,details;
        connect(&summary,&QWebSocket::textMessageReceived,this,[&](const QString &v){summaries.append(QJsonDocument::fromJson(v.toUtf8()).object());});
        connect(&detail,&QWebSocket::textMessageReceived,this,[&](const QString &v){details.append(QJsonDocument::fromJson(v.toUtf8()).object());});
        summary.open(QUrl(QString("ws://127.0.0.1:%1/ws/v3/summary").arg(port)));
        detail.open(QUrl(QString("ws://127.0.0.1:%1/ws/v3/detail").arg(port)));
        QTRY_VERIFY_WITH_TIMEOUT(summary.state()==QAbstractSocket::ConnectedState,3000);
        QTRY_VERIFY_WITH_TIMEOUT(detail.state()==QAbstractSocket::ConnectedState,3000);
        auto hasType=[](const QList<QJsonObject>& values,const QString &type){for(const auto &v:values)if(v.value("type")==type)return true;return false;};
        QTRY_VERIFY_WITH_TIMEOUT(hasType(summaries,"sync_complete"),3000);
        QVERIFY(hasType(summaries,"hello"));
        detail.sendTextMessage("{\"op\":\"subscribe\",\"symbol\":\"513090.SH\"}");
        QTRY_VERIFY_WITH_TIMEOUT(hasType(details,"detail_ack"),3000);
        for(const auto &v:details)if(v.value("type")=="detail_ack") {
            QCOMPARE(v.value("schema_version").toString(),QString("local-iopv-client.v1"));
            QVERIFY(v.contains("valuation_supported"));
            QVERIFY(v.contains("quote_ready"));
        }
        summary.close();detail.close();engine.stop();
    }
