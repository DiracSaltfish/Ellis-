...
bool AuditWriter::appendEmergencyBatchLocked(const QList<QJsonObject> &events,
...
#ifdef Q_OS_UNIX
    const QByteArray nativePath = QFile::encodeName(emergencyPath_);
    ...
    if (descriptor < 0 || ::fstat(descriptor, &status) != 0
        ...
        || (status.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) != (S_IRUSR | S_IWUSR)
        || !file.open(descriptor, QIODevice::WriteOnly | QIODevice::Append,
                      QFileDevice::AutoCloseHandle)) {
        if (descriptor >= 0 && !file.isOpen()) ::close(descriptor);
        if (error) *error = QStringLiteral("应急审计日志类型、属主或权限不安全");
        return false;
    }
    const qint64 existingBytes = status.st_size;
#else
