...
MainWindow::MainWindow(AppConfig config, QString configPath, QString agentProgram, QWidget *parent, StartMode startMode)
    ...
    int scheduledModules=0;for(const auto &m:config_.modules)if(m.enabled && m.adapter!="monitor_sync" && m.allowedActions.contains("set_operating_mode"))++scheduledModules;
    applyOperatingMode_->setText(QStringLiteral("应用到 %1 个采集模块").arg(scheduledModules));
    applyOperatingMode_->setToolTip(QStringLiteral("监控数据同步保持常驻，不参与交易时段模式切换。"));
    ...
    if (QSystemTrayIcon::isSystemTrayAvailable()) {
        ...
        tray_->setToolTip(QStringLiteral("Machome 五合一运行中心"));
...
void MainWindow::closeEvent(QCloseEvent *event) {
    ...
    if (!quitting_ && tray_ && tray_->isVisible()) {
        hide();
        tray_->showMessage(QStringLiteral("Machome 五合一运行中心"),
...
QWidget *MainWindow::buildOverview() {
    ...
    applyOperatingMode_ = new QPushButton(QStringLiteral("应用到采集模块"), page);
    ...
    operatingModeState_ = makeLabel(QStringLiteral("当前：等待采集模块状态"),
    ...
    connect(applyOperatingMode_, &QPushButton::clicked, page, [this] {
        ...
        for (auto it = moduleConfigs_.cbegin(); it != moduleConfigs_.cend(); ++it) {
            if (it.value().enabled && it.value().adapter != "monitor_sync"
    ...
    auto *grid = new QGridLayout(cardBody);
    ...
    cardsScroll->setObjectName("overviewCardsScroll");
    cardsScroll->setWidgetResizable(true);cardsScroll->setWidget(cardBody);
    auto *splitter = new QSplitter(Qt::Vertical);
    splitter->addWidget(cardsScroll);
    layout->addWidget(splitter, 1);
    auto *eventSection = new QWidget;auto *eventLayout = new QVBoxLayout(eventSection);
    eventLayout->setContentsMargins(0,0,0,0);
    auto *eventToggle=new QToolButton;eventToggle->setText(QStringLiteral("最近事件与控制结果"));eventToggle->setCheckable(true);eventToggle->setChecked(true);eventLayout->addWidget(eventToggle,0,Qt::AlignLeft);
    ...
    auto *eventPanel=ui::tablePanel(events_,"overviewEvents");eventPanel->setMinimumHeight(130);
    eventLayout->addWidget(eventPanel,1);splitter->addWidget(eventSection);splitter->setSizes({520,180});
    connect(eventToggle,&QToolButton::toggled,eventPanel,&QWidget::setVisible);
    return page;
}
...
void MainWindow::onSnapshot(const QJsonObject &message) {
    ...
    if (mode == QStringLiteral("work") || mode == QStringLiteral("weekend_test")) {
        ...
        if (operatingModeState_) {
            ...
            for(auto it=moduleConfigs_.cbegin();it!=moduleConfigs_.cend();++it)
                if(it.value().allowedActions.contains("set_operating_mode") && it.value().adapter!="monitor_sync") expected.insert(it.key());
            bool pending=false;for(const auto &id:expected)pending |= !moduleOperatingModes_.contains(id);
            modes.clear();for(const auto &id:expected)if(moduleOperatingModes_.contains(id))modes.insert(moduleOperatingModes_.value(id));
            if (pending) {
                operatingModeState_->setText(QStringLiteral("当前：等待采集模块状态"));
...
void MainWindow::updateCard(const QString &moduleId, const QJsonObject &message) {
    ...
    } else if (moduleId == "monitor_sync") {
        const auto e=telemetry.value("engine").toObject();
        card.headline->setText(QStringLiteral("人工修正文件同步 · 常驻服务"));
        metric=QStringLiteral("在线设备 %1 · 处理中 %2 · 可用 %3 GiB")
            .arg(e.value("clients").isArray()?QString::number(e.value("clients").toArray().size()):QStringLiteral("—"))
            .arg(e.contains("pending")?QString::number(e.value("pending").toInt()):QStringLiteral("—"))
            .arg(e.contains("free_bytes")?QString::number(e.value("free_bytes").toDouble()/1073741824.0,'f',1):QStringLiteral("—"));
    } else if(moduleId == "redemption") {
    ...
    if(metric.isEmpty()) metric=QStringLiteral("等待模块业务状态");
