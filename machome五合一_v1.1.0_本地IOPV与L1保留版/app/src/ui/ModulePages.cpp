...
QString operatorFieldText(const QString &raw) { return ui::fieldText(raw); }
QString operatorValueText(const QJsonValue &value) { return ui::valueText(value); }
...
void addTreeValue(QTreeWidgetItem *parent, const QString &key, const QJsonValue &value) {
    ...
    auto *item = new QTreeWidgetItem(parent, {operatorFieldText(key), operatorValueText(value)});
    item->setToolTip(0, key);
    item->setToolTip(1, displayValue(value));
}
...
QWidget *ModulePage::buildHeader(const QString &subtitle, const QList<QWidget *> &extraActions) {
    ...
    titles->addWidget(textLabel(ui::serviceTitle(config_.adapter, config_.displayName), QStringLiteral("pageTitle")));
    ...
    freshnessLabel_->setWordWrap(false);
...
void ModulePage::setContent(QWidget *content) {
    ...
    layout->addWidget(buildHeader(ui::serviceDescription(config_.adapter)));
...
void ModulePage::applySnapshot(const QJsonObject &message) {
    ...
    stateLabel_->setObjectName(ui::stateStyle(payload));
    stateLabel_->style()->unpolish(stateLabel_);
    stateLabel_->style()->polish(stateLabel_);
    freshnessLabel_->setText(QStringLiteral("更新于 %1 · 本地时间").arg(ui::localTimeText(message.value(QStringLiteral("timestamp")).toString())));
    freshnessLabel_->setToolTip(message.value(QStringLiteral("timestamp")).toString());
    if (overview_) overview_->applySnapshot(payload);
    ...
    if (acknowledgeButton_) {
        ...
        acknowledgeButton_->setToolTip(unresolved
            ? QStringLiteral("仅在完成权威外部核对后使用；后台服务会校验一次性审批票据") : QString());
    }
...
void ModulePage::applyCommand(const QJsonObject &message) {
    ...
    updateBusyControls();
    appendLog(QStringLiteral("%1  操作 %2：%3 — %4")
                  .arg(message.value(QStringLiteral("timestamp")).toString(),
                       ui::actionText(message.value(QStringLiteral("action")).toString()),
...
UploadPage::UploadPage(ModuleConfig config, QWidget *parent)
    ...
    workersExpectedLabel_ = textLabel(QStringLiteral("上传检查：按计划运行，并核对服务端接收确认"),
                                      QStringLiteral("uploadWorkersExpectedState"));
    overviewLayout->addWidget(workersExpectedLabel_);
    auto *ibkrControls = new QHBoxLayout;
    auto *ibkrLabel = textLabel(QStringLiteral("盈透 IBKR / TWS 行情"), QStringLiteral("secondaryText"));
    ibkrLabel->setWordWrap(false);
    ibkrControls->addWidget(ibkrLabel);
    ...
    tabs->addTab(overview, QStringLiteral("服务概况"));
    ...
    fundsTable_ = makeTable({QStringLiteral("基金"), QStringLiteral("名称"),
                             ...
                             QStringLiteral("单位净值"), QStringLiteral("篮子买一净值"),
    ...
    navValueEdit_->setPlaceholderText(QStringLiteral("单位净值，可留空"));
    ...
    guideLayout->addWidget(textLabel(
        QStringLiteral("上传任务页显示运行状态和最近成功时间。仅记录模式用于本地演练；包内业务组件保留原网站业务，上传成功以接收端确认结果为准。"),
...
void UploadPage::applySnapshot(const QJsonObject &message) {
    ...
    workersExpectedLabel_->setText(expected
        ? QStringLiteral("上传检查：当前任务应运行；确认超时或缺失时提示异常")
    ...
    for (const auto &value : workers) {
        ...
        setCell(workersTable_, row, 0, ui::taskText(id), worker);
...
PremiumPage::PremiumPage(ModuleConfig config, QWidget *parent)
    ...
    auto *tabs = new QTabWidget;
    if(config_.settings.value("local_iopv_enabled").toBool(false)) {
        auto *page=new QWidget;auto *layout=new QVBoxLayout(page);
        const QUrl url(config_.settings.value("iopv_web_url").toString("http://127.0.0.1:18680/"));
        auto *external=new QPushButton(QStringLiteral("在浏览器打开本地 IOPV"));
        connect(external,&QPushButton::clicked,this,[url]{QDesktopServices::openUrl(url);});layout->addWidget(external);
        layout->addWidget(createIopvWebView(url,page),1);tabs->addTab(page,QStringLiteral("本地 IOPV"));
    }
    ...
    tabs->addTab(listEditor(true), QStringLiteral("额外基础行情"));
    eventLog_ = new QPlainTextEdit;
    eventLog_->setObjectName(QStringLiteral("moduleEventLog"));
    eventLog_->setReadOnly(true);
    ...
    setContent(tabs);
    if(config_.settings.value("local_iopv_enabled").toBool(false)) {
        for(int i=0;i<tabs->count();++i)if(tabs->tabText(i)==QStringLiteral("本地 IOPV"))tabs->setCurrentIndex(i);
    }
}
...
void PremiumPage::applySnapshot(const QJsonObject &message) {
    ...
    for (auto *button : std::as_const(mutationButtons_)) {
        ...
        button->setToolTip(canMutate ? QStringLiteral("通过本机连接保存，并等待服务端确认")
                                     : QStringLiteral("需要开启模块控制权限"));
    }
...
WebullPage::WebullPage(ModuleConfig config, QWidget *parent)
    ...
    modes->addWidget(startCollector, 1, 0);
    modes->addWidget(stopCollector, 1, 1);
    modes->addWidget(restartBrowser, 1, 2);
    modes->addWidget(login, 2, 0, 1, 3);
...
void WebullPage::applySnapshot(const QJsonObject &message) {
    ...
    for (auto *button : std::as_const(mutationButtons_)) {
        ...
        button->setToolTip(canMutate
            ? (nativeEngine ? QStringLiteral("控制内置 Webull 行情服务")
                            : QStringLiteral("通过本机控制通道执行，并等待结果确认"))
            : QStringLiteral("需要启用内置服务或开启模块控制权限"));
    }
...
RealtimePage::RealtimePage(ModuleConfig config, QWidget *parent)
    ...
    watchlistLayout->addWidget(textLabel(
        QStringLiteral("每行一个 6 位深市 ETF 代码。保存观察列表可能重建 Wind 订阅，需要模块控制权限，并通过本机连接执行。"),
        QStringLiteral("secondaryText")));
    ...
    for (auto it = qmtWidgets_.begin(); it != qmtWidgets_.end(); ++it) {
        ...
        for (auto *button : {widgets.connectButton, widgets.disconnectButton,
            ...
            button->setToolTip(enabled
                ? QStringLiteral("通过后台服务的 %1 连接执行").arg(it.key())
                : QStringLiteral("需要开启模块控制权限"));
        }
        for (auto *button : {widgets.purchaseButton, widgets.redeemButton}) {
            if (!button) continue;
