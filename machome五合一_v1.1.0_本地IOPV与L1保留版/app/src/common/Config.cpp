...
bool AppConfig::isValid(QString *error) const {
    ...
    for (const auto &module : modules) {
        ...
        if (module.id == QStringLiteral("upload")) {
            ...
            if(settings.value("local_iopv_enabled").toBool(false)) {
                const QString basis=settings.value("local_iopv_basis").toString("midpoint");
                const int port=settings.value("iopv_port").toInt(18680);
                if(!QStringList{"midpoint","settlement_buy","settlement_sell"}.contains(basis)||port<1024||port>65535) {
                    if(error)*error=QStringLiteral("本地 IOPV 口径或端口无效");return false;
                }
            }
            if (module.engine == QStringLiteral("native")) {
