...
class IBMultiQuoteStream:
    """One TWS client with 30 explicit PCF component subscriptions."""
    ...
    def connect(self, components: tuple[Component, ...]) -> None:
        try:
            ...
            raise RuntimeError("ib_insync is required by private_159605_valuation_uploader") from exc
        self.disconnect()
        ib = IB()
        try:
            ...
            for component in components:
                ...
                if len(qualified) != 1:
                    raise RuntimeError(f"TWS could not uniquely qualify {component.market}:{component.symbol}")
                ticker = ib.reqMktData(qualified[0], "", False, False)
                self.subscriptions[(component.market, component.symbol)] = (component, qualified[0], ticker)
        except Exception:
            if ib.isConnected():
                ib.disconnect()
            self.subscriptions = {}
    ...
    def poll(self, wait_seconds: float = 0.05) -> tuple[MarketQuote, ...] | None:
        ...
        for component, _contract, ticker in self.subscriptions.values():
            ...
            quotes.append(MarketQuote(
                ...
                bid=bid,
                ask=ask,
                last=common.raw_positive_price(getattr(ticker, "last", None)),
