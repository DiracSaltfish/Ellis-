...
bool Machome::Webull::isScheduledIdle(const GatewayStatus &status) {
    return status.apiReportedRunning
        ...
        && status.scheduleMessage.startsWith(
            QStringLiteral("不在计划采集时间窗"));
}
...
private:
    ...
    void updateStatusFields(const QJsonObject &serverStatus) {
        ...
        status_.scheduledIdle = isScheduledIdle(status_);
    }
