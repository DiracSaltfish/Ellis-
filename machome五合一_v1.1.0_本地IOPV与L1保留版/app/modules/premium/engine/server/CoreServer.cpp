...
#include "modules/premium/engine/common/PersistenceWriter.h"
#include "modules/premium/engine/server/LegacyL1Server.h"
...
void QuoteWorker::process(const BridgeFrame &frame, const QDateTime &now, bool allow30, bool allow300,
                          ...
                          bool localEnabled, const QJsonObject &valuation, const QString &basis)
{
    ...
    const auto thresholds=valuation.value("signal_settings").toObject();
    if (!thresholds.isEmpty()) signalEngine_.setPremiumThresholds(qRound64(thresholds.value("premium_pct").toDouble(1.5)*10000),qRound64(thresholds.value("pull_pct").toDouble(0.6)*10000),qRound64(thresholds.value("radar_pct").toDouble(0.6)*10000));
    auto input=*parsed.snapshot;
    if(localEnabled){
        const auto at=QDateTime::fromString(valuation.value("calculated_at").toString(),Qt::ISODateWithMs);
        const QString field=basis=="midpoint"?"midpoint_iopv":basis=="settlement_buy"?"settlement_buy_iopv":"settlement_sell_iopv";
        const double nav=valuation.value(field).toDouble();
        bool eligible=valuation.value("eligible_for_signal").toBool(false);
        if(basis=="midpoint") {
            eligible=valuation.value("midpoint_iopv").isDouble()&&valuation.value("etf_price").isDouble();
            for(const auto &r:valuation.value("reasons").toArray()) {
                if(r.toString()!="SETTLEMENT_FX_UNAVAILABLE"&&r.toString()!="ETF_NOT_LIVE") eligible=false;
                if(r.toString()=="ETF_NOT_LIVE") eligible=false;
            }
        }
        const QString key=valuation.value("run_id").toString()+"|"+valuation.value("trade_date").toString()+"|"+valuation.value("pcf_sha256").toString()+"|"+basis+"|"+valuation.value("fx_model").toString();
        if(localKeys_.value(input.symbol)!=key){signalEngine_.resetSymbol(input.symbol);localKeys_[input.symbol]=key;}
        if(!eligible||!at.isValid()||at.msecsTo(now)<0||at.msecsTo(now)>12000||!std::isfinite(nav)||nav<=0||nav>1000000){
            signalEngine_.resetSymbol(input.symbol);Q_EMIT resultReady(input,{},false,0,0,publicationGeneration);return;
        }
        input.iopvE6=std::llround(nav*PriceScale);input.iopvStatic=false;
        input.qualityIssues.removeAll("iopv_non_positive");
        input.sellPremiumPpm=calculateRatioPpm(input.bidPricesE6[0],input.iopvE6);
        input.displayPremiumPpm=calculateRatioPpm(input.lastPriceE6,input.iopvE6);
    }
    SignalDecision decision = signalEngine_.evaluate(std::move(input), now, allow30, allow300);
    QJsonObject signal;
    if (decision.event) {
        signal = decision.event->toJson(false);
        if(localEnabled){signal.insert("type","local_iopv_signal");signal.insert("valuation_source","local_pcf");signal.insert("valuation_basis",basis);signal.insert("valuation",valuation);}
    }
    Q_EMIT resultReady(localEnabled ? *parsed.snapshot : decision.snapshot, signal, decision.event.has_value(), localEnabled ? 0 : decision.rise30sPpm,
                       localEnabled ? 0 : decision.rise300sPpm, publicationGeneration);
}
...
{
    iopvPoll_.stop();
    if(iopvNetwork_){iopvNetwork_->disconnect(this);for(auto *reply:iopvNetwork_->findChildren<QNetworkReply *>()){reply->disconnect(this);reply->abort();}}
    for(auto *socket:localSignalClients_){socket->disconnect(this);socket->close();}
    localSignalClients_.clear();
    scheduleTimer_.stop();
    statusTimer_.stop();
    if (adapterSocket_) {
...
bool CoreServer::start(QString *error)
{
    ...
    // Recover the supported reconnect window after an Agent restart.
    QFile savedSignals(QDir(dataDir).filePath("signals-"+QDate::currentDate().toString("yyyyMMdd")+".jsonl"));
    if(savedSignals.open(QIODevice::ReadOnly)){
      const auto cutoff=QDateTime::currentDateTime().addSecs(-1800);
      while(!savedSignals.atEnd()){const auto line=savedSignals.readLine(1024*1024);const auto event=QJsonDocument::fromJson(line).object();const auto at=QDateTime::fromString(event.value("occurred_at").toString(),Qt::ISODateWithMs);if(at.isValid()&&at>=cutoff&&at<=QDateTime::currentDateTime())signalHistory_.append(event);}
    }
    ...
    if(config_.value("local_iopv_enabled").toBool(false)) {
        iopvNetwork_=new QNetworkAccessManager(this);iopvPoll_.setInterval(1000);
        connect(&iopvPoll_,&QTimer::timeout,this,[this]{
            if(iopvPolling_)return;iopvPolling_=true;
            QNetworkRequest request(QUrl(QString("http://127.0.0.1:%1/api/v1/snapshots").arg(config_.value("iopv_port").toInt(18680))));request.setTransferTimeout(2000);
            auto *reply=iopvNetwork_->get(request);
            connect(reply,&QNetworkReply::readyRead,this,[reply]{if(reply->bytesAvailable()>4*1024*1024)reply->abort();});
            connect(reply,&QNetworkReply::finished,this,[this,reply]{
                iopvPolling_=false;localValuations_.clear();
                if(reply->error()==QNetworkReply::NoError&&reply->bytesAvailable()<=4*1024*1024){
                    const auto doc=QJsonDocument::fromJson(reply->readAll()).object();
                    if(doc.value("schema_version")=="intranet-iopv.v1")for(const auto &v:doc.value("snapshots").toArray()){auto p=v.toObject();p.insert("signal_settings",doc.value("signal_settings"));localValuations_.insert(p.value("symbol").toString(),p);}
                }
                // Valuation updates also reach selected B windows when ETF quotes stop.
                for (auto it=detailClients_.cbegin(); it!=detailClients_.cend(); ++it) {
                    if (!it.key()->property("localV3").toBool()) continue;
                    for (const auto &symbol : it->symbols) {
                        if (!cache_.contains(symbol)) continue;
                        auto detail=cache_.value(symbol).toDetailJson();
                        detail.insert("type","detail");
                        detail.insert("cached",true);
...
QList<QJsonObject> CoreServer::currentSignals() const
{
    QList<QJsonObject> result;
    const auto cutoff = QDateTime::currentDateTime().addSecs(-1800);
    for (const auto &signal : signalHistory_) {
        if (QDateTime::fromString(signal.value(QStringLiteral("occurred_at")).toString(), Qt::ISODateWithMs) >= cutoff)
...
void CoreServer::acceptAdapter()
{
    ...
    invalidateMarketState(QStringLiteral("adapter_connected"));
    ...
    connect(incoming, &QLocalSocket::disconnected, this, [this, incoming] {
        ...
        invalidateMarketState(QStringLiteral("adapter_disconnected"));
        adapterSocket_.clear();
        lastAdapterSymbols_.clear();
        lastAdapterQuotesDesired_ = false;
        lastAdapterSequence_ = 0;
...
void CoreServer::handleFrame(const BridgeFrame &frame)
{
    if (lastAdapterSequence_ && frame.sequence != lastAdapterSequence_ + 1) {
        ++adapterGapCount_;
        invalidateMarketState(QStringLiteral("adapter_sequence_gap"), frame.sessionId);
    ...
    if (!adapterSession_.isEmpty() && frame.sessionId != adapterSession_) {
        invalidateMarketState(QStringLiteral("adapter_session_changed"), frame.sessionId);
    }
    adapterSession_ = frame.sessionId;
    if (frame.kind == BridgeFrame::Kind::MarketEvent) {
        if (!upstreamHealthy_ && !replay_) {
            ++rejectedFrameCount_;
            writeOperational(QStringLiteral("ERROR"), QStringLiteral("adapter"),
                             QStringLiteral("market event rejected while upstream is not healthy"),
                             {{"session", frame.sessionId}, {"upstream_status", upstreamStatus_}});
            return;
        }
        ...
        if (frame.message == QStringLiteral("tgw_logged_in")
            || frame.message == QStringLiteral("simulation_session_started")) {
            healthChanged = !upstreamHealthy_ || upstreamStatus_ != frame.message;
            upstreamHealthy_ = true;
            upstreamStatus_ = frame.message;
        } else if (frame.message == QStringLiteral("tgw_session_closed")
                   || frame.message == QStringLiteral("tgw_connection_failed")
                   || frame.message == QStringLiteral("native_worker_failed")) {
            const bool needsInvalidation = upstreamHealthy_ || !cache_.isEmpty();
            if (needsInvalidation) invalidateMarketState(frame.message, frame.sessionId);
            else {
                upstreamHealthy_ = false;
                upstreamStatus_ = frame.message;
            }
            healthChanged = true;
        }
        ...
        if (healthChanged) broadcastSummary(statusObject());
    }
}

void CoreServer::invalidateMarketState(const QString &reason, const QString &session)
{
    ++publicationGeneration_;
    cache_.clear();
    latestRawRecord_ = {};
    lastCoreLatencyNs_ = 0;
    upstreamHealthy_ = false;
    upstreamStatus_ = reason;
...
void CoreServer::routeMarketFrame(const BridgeFrame &frame)
{
    ...
    const bool localEnabled=config_.value("local_iopv_enabled").toBool(false);
    const QJsonObject valuation=localValuations_.value(symbol);
    const QString basis=config_.value("local_iopv_basis").toString("midpoint");
    ...
    QMetaObject::invokeMethod(worker, [this, worker, workerIndex, frame, now, allow30, allow300,
                                       signalEligible, publicationGeneration, localEnabled, valuation, basis, replay = replay_] {
        worker->process(frame, now, allow30, allow300, replay, signalEligible, publicationGeneration, localEnabled, valuation, basis);
        workerPending_[workerIndex].fetch_sub(1);
    }, Qt::QueuedConnection);
}
...
void CoreServer::acceptWebSocket()
{
    while (monitorServer_.hasPendingConnections()) {
        ...
        if(path==QStringLiteral("/ws/v3/local-iopv-signals")){
            if(localSignalClients_.size()>=limit){socket->close();socket->deleteLater();continue;}
            localSignalClients_.insert(socket);
            connect(socket,&QWebSocket::disconnected,this,[this,socket]{localSignalClients_.remove(socket);socket->deleteLater();});
            sendJson(socket,{{"type","hello"},{"schema_version","local-iopv-signals.v1"}});
            continue;
        }
        if (path == QStringLiteral("/ws/v2/summary") || path == QStringLiteral("/ws/v3/summary")) {
            socket->setProperty("localV3",path.contains("/v3/"));
        ...
        } else if (path == QStringLiteral("/ws/v2/detail") || path == QStringLiteral("/ws/v3/detail")) {
            socket->setProperty("localV3",path.contains("/v3/"));
...
void CoreServer::handleDetailMessage(QWebSocket *socket, const QString &message)
{
    ...
    if (op == QStringLiteral("subscribe")) {
        ...
        QJsonObject ack{{"type", "detail_ack"}, {"op", "subscribe"}, {"symbol", symbol}};
        if (socket->property("localV3").toBool()) {
            ack.insert("valuation_supported", localValuations_.contains(symbol));
            ack.insert("quote_ready", upstreamHealthy_ && cache_.contains(symbol));
            ack.insert("monitored", fixedSymbols_.contains(symbol));
        }
        sendJson(socket, ack);
        if ((upstreamHealthy_ || replay_) && cache_.contains(symbol)) {
                                 ...
                                 qint64 rise30sPpm, qint64 rise300sPpm, quint64 publicationGeneration)
{
    ...
    if (hasSignal) {
        ...
        auditSignal.insert(QStringLiteral("exchange_iopv_e6"), snapshot.iopvE6);
        if(auditSignal.value("valuation_source")=="local_pcf") {
            const QString basis=auditSignal.value("valuation_basis").toString();
            const QString field=basis=="midpoint"?"midpoint_iopv":basis+"_iopv";
            auditSignal.insert("iopv_e6",static_cast<qint64>(std::llround(auditSignal.value("valuation").toObject().value(field).toDouble()*PriceScale)));
        }else auditSignal.insert("iopv_e6",snapshot.iopvE6);
    ...
    for (auto it = detailClients_.begin(); it != detailClients_.end(); ++it) {
        if(!monitored && !it.key()->property("localV3").toBool())continue;
    ...
    if (monitored && hasSignal) {
        ...
        if(auditSignal.value("valuation_source")=="local_pcf") {
            for(auto *client:localSignalClients_)sendJson(client,auditSignal);
            for(auto *client:summaryClients_)if(client->property("localV3").toBool())sendJson(client,auditSignal);
        }else broadcastSummary(auditSignal);
...
{
    if(socket->property("localV3").toBool())sendJson(socket,{{"type","hello"},{"recovery","full_sync_last_30_minutes"},{"trade_date",QDate::currentDate().toString(Qt::ISODate)}});
    ...
    for (const QuoteSnapshot &snapshot : cache_) {
        if (!upstreamHealthy_ && !replay_) break;
        if (!fixedSymbols_.contains(snapshot.symbol)) continue;
        QJsonObject object = snapshot.toSummaryJson();
        object.insert(QStringLiteral("type"), QStringLiteral("summary"));
        object.insert(QStringLiteral("backfill"), true);
    ...
    for (QJsonObject signal : signalHistory_) {
        if((signal.value("valuation_source")=="local_pcf") != socket->property("localV3").toBool())continue;
...
QJsonObject CoreServer::localMessage(QJsonObject object) const
{
    return withLocalValuation(object,
        localValuations_.value(object.value("s").toString()),
        config_.value("local_iopv_basis").toString("midpoint"), QDateTime::currentDateTime());
}
...
void CoreServer::sendJson(QWebSocket *socket, const QJsonObject &object)
{
    ...
    socket->sendTextMessage(QString::fromUtf8(compact(socket->property("localV3").toBool()?localMessage(object):object)));
}
...
{
    ScheduleState next = schedule_.stateAt(QDateTime::currentDateTime(), simulation_ || replay_ || forceQuotes_);
    if (config_.value("local_iopv_enabled").toBool(false)) {
        const auto now=QDateTime::currentDateTime();const auto t=now.time();
        if(now.date().dayOfWeek()<=5) {
            next.cnQuotesDesired=next.cnQuotesDesired||(t>=QTime(15,0)&&t<QTime(15,1));
            next.hkQuotesDesired=next.hkQuotesDesired||(t>=QTime(9,15)&&t<QTime(16,10));
            next.quotesDesired=next.cnQuotesDesired||next.hkQuotesDesired;
        }
    }
