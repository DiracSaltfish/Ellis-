                                      ...
                                      bool allow300Seconds)
{
    ...
    if (state.premiumActive) {
        if (snapshot.sellPremiumPpm < premiumThreshold_ - 2000) {
    ...
    const bool premiumCondition = hasUsableIopv && snapshot.sellPremiumPpm > premiumThreshold_ && (trigger30 || trigger300);
    ...
    const bool pullGate = !hasUsableIopv || snapshot.sellPremiumPpm > pullThreshold_;
    ...
    const bool radarCondition = hasUsableIopv && snapshot.sellPremiumPpm > radarThreshold_ && radarTrigger;
...
void SignalEngine::setPremiumThresholds(qint64 premium,qint64 pull,qint64 radar) {
 if(premium==premiumThreshold_&&pull==pullThreshold_&&radar==radarThreshold_)return;
 premiumThreshold_=premium;pullThreshold_=pull;radarThreshold_=radar;resetAll();
}
