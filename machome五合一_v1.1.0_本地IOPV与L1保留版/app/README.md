...
本目录包含 Qt 6 GUI、独立后台 Agent、四个原生业务 Engine，以及包内 IBKR/TGW/Webull/Wind helper。
...
原生 IBKR bridge 是可选构建项。启用时还需要官方 IBKR C++ API，以及与其生成代码匹配的
protobuf/Abseil；三者保持外部依赖，不复制进工程。
...
包含原生 IBKR bridge 的交付命令：

```bash
bash scripts/build-release.sh --qt-prefix /opt/homebrew --with-native-ibkr \
  --ibkr-sdk-root /Users/ellis/QMTAPI/CPP/third_party/ibkr/twsapi/client \
  --ibkr-protobuf-root /Users/ellis/QMTUI/CPP/third_party/protobuf/33.4_1 \
  --ibkr-absl-root /Users/ellis/QMTUI/CPP/third_party/abseil/20260107.1
```

脚本会把 bridge 复制进 app、让 `macdeployqt` 扫描其 protobuf/Abseil 动态依赖，并在签名前
运行未解析依赖检查和 `--validate-config`。详细边界见 `modules/ibkr/README.md`。
...
4. 用 `macdeployqt` 同时扫描 GUI、Agent、Premium TGW helper、Webull Browser helper、
   Wind Probe helper，并在启用时扫描 IBKR bridge，确保各自独有依赖都被收进包；
5. 剔除构建机 Qt 安装中“插件存在但配套框架缺失”的不可用可选插件，并验证全部
   必需可执行文件和 helper 没有未打包或构建机绝对依赖；
...
原生 IBKR bridge 使用独立 LaunchAgent，避免 TWS SDK 回调或重连影响 Hub Agent。安装 app
后先预演：

```bash
bash scripts/install-ibkr-bridge.sh
```

`--apply` 只安装 bridge 配置/plist；再显式加 `--load-bridge` 才改变 bridge 进程。该脚本不会
停止、重启或修改任何旧 Python uploader。切流仍须按
`docs/11_Go_Python到C++迁移评估与原生IBKR实施.md` 逐组完成。七个 uploader 的可安装覆盖层、
隔离回归入口和回滚说明见 `migration/newnavnav-shared-ibkr/README.md`。
