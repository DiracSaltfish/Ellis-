...
try:
    from machome_ibkr_bridge_client import (
        BridgeQuote,
        ContractSubscription,
        SharedMultiQuoteStream,
    )
except ImportError:  # pragma: no cover - the production overlay always bundles it
    BridgeQuote = None
    ContractSubscription = None
    SharedMultiQuoteStream = None
...
class SharedIBUSHoldingsBridge(IBUSHoldingsBridge):
    """Use the native bridge for the production SMART live-quote path.

    Historical minute/close/anchor queries intentionally retain ib_insync for
    functional parity, but every public historical operation is scoped to a
    short-lived TWS session. There is no fallback from SMART live quotes to a
    direct Python TWS subscription.
    """

    def __init__(self, args: IBBridgeArgs):
        super().__init__(args)
        self.shared_stream = None
        self.shared_subscription_ids: dict[str, str] = {}
        self.shared_target: tuple[str, ...] = ()
        self.shared_next_retry_at = 0.0

    def close_shared_runtime(self) -> None:
        stream, self.shared_stream = self.shared_stream, None
        self.shared_subscription_ids.clear()
        self.shared_target = ()
    ...
    def disconnect_runtime(self) -> None:
        ...
        IBUSHoldingsBridge.disconnect_runtime(self)

    def disconnect_history_runtime(self) -> None:
        """Close only the direct historical TWS session, preserving live leases."""
        IBUSHoldingsBridge.disconnect_runtime(self)

    def ensure_shared_live_subscriptions(
        self,
        symbols: Iterable[str],
        observed_at: Optional[datetime] = None,
    ) -> None:
        target = tuple(sorted({normalize_symbol(item) for item in symbols if normalize_symbol(item)}))
        if not target:
            self.close_shared_runtime()
            return
        if not self.within_active_window(observed_at):
            self.close_shared_runtime()
            return
        if SharedMultiQuoteStream is None or ContractSubscription is None:
            raise RuntimeError("native IBKR bridge client is unavailable")
        if self.shared_stream is not None and self.shared_target == target:
            try:
                if self.shared_stream.is_connected():
                    return
            except Exception:
                pass
            self.close_shared_runtime()
        now_ts = time.time()
        if self.shared_next_retry_at > now_ts:
            raise RuntimeError("native IBKR bridge reconnect backoff is active")
        subscriptions = [shared_smart_subscription(symbol) for symbol in target]
        stream = SharedMultiQuoteStream(
            subscriptions,
            timeout=max(0.2, min(float(self.args.timeout), 10.0)),
        )
        try:
            stream.connect()
        except Exception:
            stream.close()
            self.shared_next_retry_at = now_ts + max(1.0, float(self.args.connect_retry_seconds))
            raise
        self.shared_stream = stream
        self.shared_target = target
        self.shared_subscription_ids = {
            subscription.symbol: subscription.subscription_id for subscription in subscriptions
        }
        self.shared_next_retry_at = 0.0
        self.last_connect_at = time.time()
        self.last_subscription_change_at = self.last_connect_at

    def ensure_live_subscriptions(self, symbols: Iterable[str], observed_at: Optional[datetime] = None) -> None:
        if self.args.live_mode == "smart":
            self.ensure_shared_live_subscriptions(symbols, observed_at)
            return
        super().ensure_live_subscriptions(symbols, observed_at)

    def live_quotes(self, required_symbols: list[str], observed_at: Optional[datetime] = None) -> dict[str, dict]:
        if self.args.live_mode != "smart":
            return super().live_quotes(required_symbols, observed_at)
        captured_at = (observed_at or now_beijing()).astimezone(BEIJING_TZ)
        target = self.refresh_target_symbols(required_symbols, observed_at=captured_at)
        if not target:
            self.maintain_connection_window(captured_at)
            return {}
        self.ensure_shared_live_subscriptions(target, captured_at)
        quotes = self.collect_shared_live_quotes(target, captured_at)
        if self.should_warmup_live_quotes(target, quotes):
            deadline = time.time() + max(0.0, float(self.args.warmup_seconds))
            while time.time() < deadline and len(quotes) < len(target):
                time.sleep(min(0.25, max(0.02, deadline - time.time())))
                quotes = self.collect_shared_live_quotes(target, now_beijing())
        return quotes

    def collect_shared_live_quotes(self, target: set[str], observed_at: datetime) -> dict[str, dict]:
        if self.shared_stream is None:
            return {}
        values = self.shared_stream.poll(0.05)
        quotes: dict[str, dict] = {}
        for symbol in sorted(target):
            subscription_id = self.shared_subscription_ids.get(symbol)
            value = values.get(subscription_id) if subscription_id else None
            quote = shared_bridge_quote_to_live_quote(symbol, value, observed_at)
    ...
    def catchup_reference_minutes(self, required_symbols: list[str], observed_at: Optional[datetime] = None) -> int:
        try:
            return super().catchup_reference_minutes(required_symbols, observed_at)
        finally:
            if self.args.live_mode == "smart":
                self.disconnect_history_runtime()

    def resolve_missing_daily_prices(self, requests: list[dict]) -> tuple[list[dict], list[dict], list[str]]:
        try:
            return super().resolve_missing_daily_prices(requests)
        finally:
            if self.args.live_mode == "smart":
                self.disconnect_history_runtime()
    ...
    def resolve_missing_valuation_anchor_prices(self, requests: list[dict]) -> tuple[list[dict], list[str]]:
        try:
            return super().resolve_missing_valuation_anchor_prices(requests)
        finally:
            if self.args.live_mode == "smart":
                self.disconnect_history_runtime()


def shared_bridge_quote_to_live_quote(
    symbol: str,
    quote,
    ...
    price = last if last > 0 else midpoint(quote.bid, quote.ask)
    if price <= 0:
        return None
    prev_close = float(quote.close or 0)
    quote_at = (quote.exchange_timestamp or quote.observed_at or observed_at).astimezone(BEIJING_TZ)
    ...
    return {
        ...
        "quote_time": quote_at.strftime("%H:%M:%S"),
        "quote_timezone": "Asia/Shanghai",
        "source": neutralize_source_name("ibkr_us_shared_cpp"),
        "source_symbol": symbol,
        "quote_session": "us_smart_live",
        "market_data_type": quote.market_data_type,
        "bridge_sequence": quote.sequence,
        "bridge_age_ms": quote.age_ms,
    }


def shared_smart_subscription(symbol: str):
    """Match pinned identities exactly so XOP/INDA leases are coalesced."""
    normalized = normalize_symbol(symbol)
    pinned = {
        "XOP": {"primary_exchange": "ARCA", "generic_ticks": "236"},
        "INDA": {"primary_exchange": "ARCA", "generic_ticks": ""},
    }.get(normalized, {})
    return ContractSubscription.create(
        subscription_id=f"{normalized}.SMART",
        symbol=normalized,
        security_type="STK",
        exchange="SMART",
        primary_exchange=pinned.get("primary_exchange", ""),
        currency="USD",
        generic_ticks=pinned.get("generic_ticks", "236"),
    )
...
def bridge_from_args(args) -> Optional[IBUSHoldingsBridge]:
    ...
    if not bridge_args.enabled:
        return None
    return SharedIBUSHoldingsBridge(bridge_args)
