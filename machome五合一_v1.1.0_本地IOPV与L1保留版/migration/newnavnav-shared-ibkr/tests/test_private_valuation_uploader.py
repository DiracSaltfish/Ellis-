...
class PrivateValuationUploaderTests(unittest.TestCase):
    ...
    def test_ib_poll_preserves_ticker_network_time_instead_of_poll_time(self) -> None:
        class FakeBridgeQuote:
            bid = 158.61
            ask = 159.80
            last = 159.40
            market_data_type = "Live"
            observed_at = datetime(2026, 7, 10, 1, 59, 58, tzinfo=timezone.utc)

        class FakeShared:
            def poll(self, _seconds: float):
                return FakeBridgeQuote()

        stream = uploader.IBQuoteStream("127.0.0.1", 7496, 15918, 1.0)
        stream.shared = FakeShared()
