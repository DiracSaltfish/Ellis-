...
class NQMarket:
    ...
    def connect(self) -> None:
        if self.shared is not None and self.shared.is_connected() and self.contract is not None:
            return
        self.close()
        try:
        ...
        try:
            ...
            contract = select_front_nq_contract(
                list(details), datetime.now(ZoneInfo(self.family.future_timezone))
            )
            qualified = ib.qualifyContracts(contract)
            if len(qualified) != 1:
                raise SourceUnavailableError(
                    f"IBKR could not uniquely qualify the front {self.family.future_exchange} "
                    f"{self.family.reference_symbol} future"
            )
        ...
        subscription = ContractSubscription.create(
            symbol=str(getattr(contract, "symbol", "") or self.family.reference_symbol),
            security_type=str(getattr(contract, "secType", "") or "FUT"),
            exchange=str(getattr(contract, "exchange", "") or self.family.future_exchange),
        ...
        shared = SharedSingleQuoteStream(subscription, timeout=self.timeout)
        try:
            shared.connect()
        except Exception as exc:
            shared.close()
            raise SourceUnavailableError(f"native IBKR bridge unavailable: {exc}") from exc
        self.ib, self.contract, self.shared = None, contract, shared
    ...
    def close(self) -> None:
        ...
        if self.ib is not None:
            try:
                ...
                pass
        self.ib = self.contract = self.shared = None

    def quote(self) -> NQQuote | None:
        self.connect()
        try:
            value = self.shared.poll(0.05)
        except Exception as exc:
            raise SourceUnavailableError(f"native IBKR bridge poll failed: {exc}") from exc
        if value is None:
            return None
        return NQQuote(
            self.family.reference_symbol, value.bid, value.ask, value.last,
    ...
    def close_for_day(self, trading_day: date) -> float:
        ...
        try:
            ib.connect(self.host, self.port, clientId=self.client_id,
                       timeout=self.timeout, readonly=True)
            if not ib.isConnected():
                raise SourceUnavailableError("TWS history socket did not become connected")
            if self.family.calibration_anchor_kind == "xetra_close_auction_1735":
                anchor = xetra_close_auction_anchor(ib, self.contract, trading_day, self.timeout)
        ...
        while not common.STOP_EVENT.is_set():
            ...
            if time.monotonic() >= next_fx:
                try:
                    ...
                    if cfets is None or cfets.trading_day != now.date():
                        cfets = None
                        common.log(f"{family.display_name} CFETS unavailable: {exc}", error=True)
                    else:
                        age = max(0.0, (now - cfets.fetched_at).total_seconds())
                        common.log(
                            f"{family.display_name} CFETS refresh unavailable; retaining same-day "
                            f"CFETS_SPOT_RATE age={age:.1f}s: {exc}",
                            error=True,
                        )
                next_fx = time.monotonic() + 60.0
            try:
                quote = market.quote() if pcfs and cfets is not None else None
            except Exception as exc:
                market.close()
                common.log(
