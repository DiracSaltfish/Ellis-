...
class MarketHub:
    ...
    def close(self) -> None:
        if self.shared is not None:
            self.shared.close()
        self.shared, self.subscriptions, self.sina_components, self.keys = None, {}, {}, set()

    def sync(self, components: dict[tuple[str, str], Component]) -> None:
        if set(components) == self.keys and (self.shared is None or self.shared.is_connected()):
            return
        self.close()
        ...
        if ib_components:
            ...
            for key, component in ib_components.items():
                ...
                mappings[key] = (component, subscription.subscription_id)
            shared = SharedMultiQuoteStream(values, timeout=self.timeout)
            try:
                shared.connect()
    ...
    def poll(self) -> dict[tuple[str, str], Quote] | None:
        ...
        if self.subscriptions:
            if self.shared is None:
                raise SourceUnavailableError("native IBKR bridge stream is disconnected")
            try:
                values = self.shared.poll(0.05)
            except Exception as exc:
                raise SourceUnavailableError(f"native IBKR bridge poll failed: {exc}") from exc
            for key, (component, subscription_id) in self.subscriptions.items():
                value = values.get(subscription_id)
                if value is None:
                    return None
                quotes[key] = Quote(
                    component, value.bid, value.ask, value.last, IB_SOURCE,
                    value.market_data_type, value.observed_at.astimezone(common.SHANGHAI), now,
                )
