...
class IBQuoteStream:
    """XOP quote consumer backed by the one native TWS connection."""
    ...
    def connect(self) -> None:
        ...
        shared = SharedSingleQuoteStream(
            ContractSubscription.create(
                subscription_id="XOP.SMART",
                symbol=REFERENCE_SYMBOL,
                security_type=XOP_SEC_TYPE,
                exchange=XOP_EXCHANGE,
                primary_exchange=XOP_PRIMARY_EXCHANGE,
    ...
    def poll(self, wait_seconds: float = 0.05) -> IBQuote | None:
        ...
        quote = IBQuote(
            ...
            observed_at=value.observed_at.astimezone(SHANGHAI),
