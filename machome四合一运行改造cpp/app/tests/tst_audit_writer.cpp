...
private slots:
    ...
    void consecutiveCriticalEventsKeepSecureSpoolWritable();
...
void AuditWriterTests::consecutiveCriticalEventsKeepSecureSpoolWritable() {
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString databasePath = directory.filePath(QStringLiteral("audit.db"));
    const QString spoolPath = databasePath + QStringLiteral(".critical-emergency.jsonl");
    ...
    const auto event = [](const QString &id) {
        return QJsonObject{
            {QStringLiteral("event_id"), id},
            {QStringLiteral("timestamp"), QStringLiteral("2026-09-04T00:00:00.000Z")},
            {QStringLiteral("module_id"), QStringLiteral("redemption")},
            {QStringLiteral("type"), QStringLiteral("event")},
            {QStringLiteral("event_kind"), QStringLiteral("redemption.error")},
        };
    };
    QString error;
    QVERIFY2(writer.enqueueCritical(event(QStringLiteral("event-consecutive-0001")), &error),
             qPrintable(error));
    QVERIFY2(writer.enqueueCritical(event(QStringLiteral("event-consecutive-0002")), &error),
             qPrintable(error));
    QCOMPARE(QFileInfo(spoolPath).permissions()
