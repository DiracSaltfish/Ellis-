...
// The original gateway deliberately stops its collector outside the configured
// market window while keeping the API process alive. Keep that state distinct
// from a crashed/stopped collector so lifecycle readiness can remain strict in
// market hours without treating a scheduled close as a failed start.
[[nodiscard]] bool isScheduledIdle(const GatewayStatus &status);
