...
wait_for_agent_shutdown() {
    ...
    while [ "$waited" -lt 50 ]; do
        if ! /bin/launchctl print "gui/$(/usr/bin/id -u)/$LABEL" >/dev/null 2>&1 \
           && ! /usr/bin/pgrep -u "$(/usr/bin/id -u)" \
                -f '/machome-hub-agent([[:space:]]|$)' >/dev/null 2>&1; then
            return 0
        fi
        /bin/sleep 1
        waited=$((waited + 1))
    done
    machome_warn "等待旧 Hub Agent 退出超时（50 秒）；拒绝在单实例锁未释放时启动新版本"
    return 1
}
...
recover_failed_install() {
    ...
    if [ "$LOAD_AGENT" -eq 1 ]; then
        ...
        if [ "$AGENT_WAS_LOADED" -eq 1 ] && [ -f "$PLIST_PATH" ]; then
            if wait_for_agent_shutdown; then
                /bin/launchctl bootstrap "gui/$(/usr/bin/id -u)" "$PLIST_PATH" >/dev/null 2>&1
            else
                machome_warn "旧 Agent 未退出，未自动加载回滚版本；文件已经恢复，请人工核对进程"
            fi
        fi
    fi
    case "${STAGE_ROOT:-}" in
...
if [ "$LOAD_AGENT" -eq 1 ]; then
    if [ "$AGENT_WAS_LOADED" -eq 1 ]; then
        ...
        wait_for_agent_shutdown || machome_die "旧 Hub Agent 未能在安全期限内退出"
    fi
