...
class OvernightXOPQuoteStream:
    ...
    def connect(self) -> None:
        ...
        self.disconnect()
        ib = IB()
        contract = Stock("XOP", "OVERNIGHT", "USD", primaryExchange="ARCA")
        try:
            ...
            if len(qualified) != 1:
                raise RuntimeError("TWS could not uniquely qualify XOP / OVERNIGHT")
            contract = qualified[0]
            if str(getattr(contract, "symbol", "") or "").upper() != "XOP":
                raise RuntimeError("qualified contract is not XOP")
            ...
            if ib.isConnected():
                ib.disconnect()
            raise
        self.ib, self.contract, self.ticker = ib, contract, ticker
    ...
    def poll(self, wait_seconds: float = 0.05) -> common.IBQuote | None:
        if not self.is_connected() or self.ticker is None:
            raise SourceUnavailableError("IBKR XOP / OVERNIGHT stream is disconnected")
        ...
        last = common.raw_positive_price(getattr(self.ticker, "last", None))
        market_data_type = common.MARKET_DATA_TYPE_NAMES.get(int(getattr(self.ticker, "marketDataType", 0) or 0))
        observed_at = common.ib_ticker_observed_at(getattr(self.ticker, "time", None))
        if bid is None or ask is None or ask < bid or market_data_type is None or observed_at is None:
            return None
        quote = common.IBQuote(
            bid, ask, last, market_data_type, observed_at,
    ...
    def regular_session_reference(self, pcf_day: date) -> XOPRegularSessionReference:
        ...
        if not self.is_connected():
            raise SourceUnavailableError("IBKR is disconnected before regular-session calibration")
        try:
        ...
        try:
            bids = self.ib.reqHistoricalData(qualified[0], cutoff, "10 D", "1 min", "BID", True, 2, False, [], 60)
            asks = self.ib.reqHistoricalData(qualified[0], cutoff, "10 D", "1 min", "ASK", True, 2, False, [], 60)
