...
class INDAMarket:
    ...
    def connect(self) -> None:
        ...
        self.close()
        self.ib = IB()
        self.ib.connect(self.host, self.port, clientId=self.client_id, timeout=self.timeout, readonly=True)
    ...
    def nifty_contract_candidates(self, as_of: datetime) -> list[tuple[date, Any]]:
        ...
        details = self.ib.reqContractDetails(Future(NIFTY_SYMBOL, exchange=NIFTY_EXCHANGE, currency="USD"))
    ...
    def quote(self, key: str, contract: Any, symbol: str) -> MarketQuote | None:
        ...
        if key not in self.tickers:
            self.tickers[key] = self.ib.reqMktData(contract, "", False, False)
            self.ticker_contracts[key] = contract
        ...
        for _ in range(10):
            ...
            if bid is None or ask is None or ask < bid or observed_at is None or market_type is None:
                continue
            return MarketQuote(
                symbol, str(getattr(contract, "localSymbol", "") or getattr(contract, "symbol", symbol)),
                bid, ask, common.raw_positive_price(getattr(ticker, "last", None)), observed_at, market_type,
            )
        return None

    def inda_quote(self) -> MarketQuote | None:
        return self.quote("INDA", self.contract("SMART"), REFERENCE_SYMBOL)
    ...
    def nifty_quote(self, as_of: datetime) -> MarketQuote | None:
        return self.nifty_quote_for_contract(self.nifty_contract(as_of))
    ...
    def historical_bid_ask_window(
        ...
        for what in ("BID", "ASK"):
            ...
            for bar in bars:
                ...
                observed_at = observed_at.astimezone(NEW_YORK).replace(second=0, microsecond=0)
    ...
    def close(self) -> None:
        ...
        try:
            for key, ticker in self.tickers.items():
                contract = self.ticker_contracts.get(key)
