...
B 首页默认打开独立“信号列表”：同一标的只保留最新触发并置顶，重连/重启不自动清除，每行可点“本次移除”，也可点“一键移除全部信号”；“全局列表”始终按证券代码固定排序。两表都可双击进入详情。信号缓存位于设置同目录的 `client-signal-list.json`，移除只改变本机 B，不影响 A 的历史审计、观察清单或 TGW 订阅。实时信号可响铃和显示被动系统通知，但不会显示、置前或激活 B 主窗口，也不会切换 macOS 全屏空间。
...
最终 0.2.0 的备份、Git 身份、machome 部署、局域网联合调试和分层上线意见见 `docs/19_20260827_v0.2.0部署与上线意见.md`。

0.3.1 的快速拉涨雷达、可卖 IOPV 溢价严格大于 0.60% 的外层门、OR 合并语义、3 秒 tick 敏感分支、测试结果和上线限制见 `docs/20_20260828_快速拉涨雷达实现与测试.md`。
