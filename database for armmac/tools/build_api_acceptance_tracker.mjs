...
for (const [api, pdfPage, keyContract, mode, fullStatus, next] of parseTableBetween("## 2. 原生 TGW", "## 3. AmazingData")) {
  nativeRows.push({
    ...
    linuxDate: /LIVE_ALIGNED|WIRE_VERIFIED|LINUX_OBSERVED/.test(fullStatus) ? "2026-08-26" : "",
    linuxResult: /LIVE_ALIGNED|WIRE_VERIFIED/.test(fullStatus) ? "已有脱敏证据；范围见状态/证据" : "未执行",
    wire: api === "QueryKline" ? "ReqGetKline；daily=10100，weekly=10101，monthly=10102，quarterly=10103，yearly=10104（wire/tag）" : api === "QuerySnapshot" ? "ReqGetSnapshot；数据tag=11000；空数据帧 tag=\"DataEmpty\"/status=-100→公开-76；错误路径不发 ReqGetComplete" : api === "QueryCodeTable" ? "dgw*_query one-shot ReqGetReduceCodeTable；tag=11103；反引号6字段；缺包 ReqGetPackage" : api === "QuerySecuritiesInfo" ? "push WSS ReqGetCodeTableList；Security=code|market；tag=字符串109；ReqGetCodelistComplete；43字段记录" : api === "QueryExFactorTable" ? "dgw*_query one-shot ReqGetExFactor；tag=11102；5字段CSV；double 18位小数字符串；ReqGetComplete" : api === "QueryThirdInfo" ? "ReqGetThirdInfo；tag=11101（仅日历证据）" : api === "QueryETFInfo" ? "push WSS ReqGetETFCodeTableList；Security=code|market；tag=字符串111；ReqGetCodelistComplete" : "待逐接口取证",
    macDate: /LIVE_ALIGNED|ARM_IMPLEMENTED|WIRE_VERIFIED|CHANGES_REQUESTED/.test(fullStatus) ? "2026-08-26" : "",
    macResult: /LIVE_ALIGNED/.test(fullStatus) ? "同参子范围已对齐" : /ARM_IMPLEMENTED/.test(fullStatus) ? "本地实现，待 live 同参" : "待实现/复验",
    conclusion: fullStatus,
    construction: nativeConstruction(api),
    ...
    responseType: contracts[api]?.[1] || keyContract,
    callback: contracts[api]?.[2] || keyContract,
    evidence: api === "QueryKline" ? "docs/evidence/query_kline_daily.md；docs/evidence/query_kline_week.md；docs/evidence/query_kline_month.md；docs/evidence/query_kline_quarter_static.md；docs/evidence/query_kline_quarter_live.md；docs/evidence/query_kline_year_live.md" : api === "QuerySnapshot" ? "docs/evidence/query_snapshot_szse_etf.md；docs/evidence/query_snapshot_error_async_contract.md" : api === "QueryCodeTable" ? "docs/evidence/query_code_table_static.md；docs/evidence/query_code_table_live_closure.md" : api === "QuerySecuritiesInfo" ? "docs/evidence/query_securities_info_sse.md" : api === "QueryExFactorTable" ? "docs/evidence/query_ex_factor_table_000001.md" : api === "QueryETFInfo" ? "docs/evidence/query_etf_info_static.md；docs/evidence/query_etf_info_sse_etf.md" : "docs/PDF_API_PARITY_MATRIX.md",
    next,
    notes: keyContract,
    batch: "",
  });
...
summary.getRange("D6:H13").values = [
  [1, "QuerySnapshot", "SZSE ETF L1 历史", "LIVE_ALIGNED", "已完成错误码/异步合约；余多包异步交付与其它市场"],
