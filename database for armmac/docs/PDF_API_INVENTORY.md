...
| 37 | TGW-C++ | 36 | 28 | 查询 | `QueryCodeTable(code_table_spi)` | 双模式（回调章节误标 coloc，HDR/方法页为双模式） | 无入参 | `MDCodeTable{code[16],symbol[32],en_name[128],market u8,type[10],currency[4]}` pack(1)=191 | `OnMDCodeTable`+`OnStatus` | 含 2022-08-22 起退市代码 | 是 | `STATIC_MATCHED(static contract only)` | 全市场一次查询取行数/列型摘要；用异步 SPI 收齐批次 | 官方同步 wrapper 在首批回调即唤醒且覆盖结果，存在首批竞态；wire method/tag 尚未实测 |
| 38 | TGW-C++ | 36–37 | 28–29 | 查询 | `QuerySecuritiesInfo(spi,SubCodeTableItem*,cnt)` | 双模式 | `SubCodeTableItem{i32 market,char[32] code}` | `MDCodeTableRecord`（大结构，含涨跌停/单位/期权属性） | `OnMDSecuritiesInfo`+`OnStatus` | market kNone=全市场；支持 SSE/SZSE/NEEQ | 是 | `INVENTORIED` | 单市场单代码（如 `159518.SZSE`） | 返回容器列集合需先 Linux oracle 定形 |
| 39 | TGW-C++ | 36–37 | 28–29 | 查询 | `QueryExFactorTable(ex_factor_spi,code)` | 双模式 | `const char* code` | `MDExFactorTable{inner_code,code,ex_date,ex_factor double,cum_factor double}` | `OnMDExFactor`+`OnStatus` | 无 | 是 | `INVENTORIED` | `000001` 单代码 | double 精度与缩放（N38(15)）需 oracle 确认 |
...
|---|---|---|---|---|---|---|---|---|---|
| 49 | TGW-C++ | 62 | 54 | 枚举 | ErrorCode | kFailure=-100 … kSuccess=0（含 kDataEmpty=-76、kNonQueryTimePeriod=-88、kFlowOverLimit=-82、kOverMaxQueryLimit=-78 等） | 是 | `ARM_IMPLEMENTED(partial)`（GetErrorMsg 仅识别 0） | Mac 需把非零 status 对齐为官方错误码语义而非异常 |
