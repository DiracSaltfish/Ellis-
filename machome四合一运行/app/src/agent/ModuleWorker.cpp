...
#include "modules/bridge/UploadClient.h"
...
ModuleWorker::ModuleWorker(ModuleConfig config, quint64 initialControlRevision, QObject *parent)
    ...
    eventDrainTimer_.setInterval(50);
...
void ModuleWorker::startUpload() {
    upload_ = new machome::bridge::UploadClient(this);
    QString error;
    if (!upload_->setBaseUrl(QUrl(config_.settings.value(QStringLiteral("web_base_url")).toString()), &error)) {
        lastError_ = error;
        workState_ = QStringLiteral("blocked");
        headline_ = QStringLiteral("Upload adapter 配置无效，已拒绝启动");
        publishEvent(QStringLiteral("upload.configuration_error"),
                     {{QStringLiteral("message"), error}});
        upload_->deleteLater();
        upload_ = nullptr;
        return;
    }
    upload_->setHealthDirectory(AppConfig::expandPath(config_.settings.value(QStringLiteral("health_directory")).toString()));
    upload_->setProbeIntervalMs(config_.settings.value(QStringLiteral("poll_interval_ms")).toInt(2000));
    upload_->setRequestTimeoutMs(config_.settings.value(QStringLiteral("request_timeout_ms")).toInt(3000));
    upload_->setWorkerFreshnessMs(config_.settings.value(QStringLiteral("worker_stale_ms")).toInteger(35000));
    const bool workersExpected = config_.settings.value(QStringLiteral("workers_expected")).toBool(true);
    upload_->setWorkersExpected(workersExpected);
    telemetry_.insert(QStringLiteral("workers_expected"), workersExpected);
    connect(upload_, &machome::bridge::UploadClient::siteStatusChanged, this,
            [this](const machome::bridge::UploadSiteStatus &status) {
        telemetry_.insert(QStringLiteral("site"), QJsonObject{
            {QStringLiteral("reachable"), status.reachable}, {QStringLiteral("healthy"), status.healthy},
            {QStringLiteral("identity_verified"),
             status.payload.value(QStringLiteral("identity_verified")).toBool(false)},
            {QStringLiteral("service"), status.payload.value(QStringLiteral("service"))},
            {QStringLiteral("module"), status.payload.value(QStringLiteral("module"))},
            {QStringLiteral("identity_contract"),
             status.payload.value(QStringLiteral("identity_contract"))},
            {QStringLiteral("http_status"), status.httpStatus}, {QStringLiteral("latency_ms"), status.latencyMs},
            {QStringLiteral("observed_at"), iso(status.observedAt)}, {QStringLiteral("payload"), status.payload},
            {QStringLiteral("error_code"), status.errorCode}, {QStringLiteral("error"), status.errorMessage}});
        markTelemetryObserved(QStringLiteral("site"), status.observedAt.toMSecsSinceEpoch());
        if (!status.errorMessage.isEmpty()) lastError_ = status.errorMessage;
        scheduleSnapshot();
    });
    connect(upload_, &machome::bridge::UploadClient::workerStatusesChanged, this,
            [this](const machome::bridge::UploadWorkerStatusList &workers) {
        QJsonArray array;
        for (const auto &worker : workers) array.append(uploadWorkerJson(worker));
        telemetry_.insert(QStringLiteral("workers"), array);
        telemetry_.insert(QStringLiteral("worker_count"), array.size());
        scheduleSnapshot();
    });
    connect(upload_, &machome::bridge::UploadClient::healthScanFinished, this,
            [this, workersExpected](int validFiles, int invalidFiles, bool limited) {
        const auto workers = upload_ ? upload_->workerStatuses()
                                     : machome::bridge::UploadWorkerStatusList{};
        bool allFreshAndAcknowledged = !workers.isEmpty();
        int freshAcknowledgements = 0;
        for (const auto &worker : workers) {
            const bool reportsOk = worker.state.compare(
                QStringLiteral("ok"), Qt::CaseInsensitive) == 0;
            const bool acknowledged = reportsOk && worker.statusFresh
                && worker.successFresh;
            if (acknowledged) ++freshAcknowledgements;
            allFreshAndAcknowledged = allFreshAndAcknowledged && acknowledged;
        }
        const bool ackFresh = !limited && invalidFiles == 0
            && (workersExpected ? allFreshAndAcknowledged
                                : (workers.isEmpty() || allFreshAndAcknowledged));
        telemetry_.insert(QStringLiteral("worker_health"), QJsonObject{
            {QStringLiteral("ack_fresh"), ackFresh},
            {QStringLiteral("workers_expected"), workersExpected},
            {QStringLiteral("valid_files"), validFiles},
            {QStringLiteral("invalid_files"), invalidFiles},
            {QStringLiteral("limited"), limited},
            {QStringLiteral("fresh_acknowledgements"), freshAcknowledgements},
            {QStringLiteral("worker_count"), workers.size()}});
        markTelemetryObserved(QStringLiteral("worker_health"));
        scheduleSnapshot();
    });
    connect(upload_, &machome::bridge::UploadClient::aggregateStatusChanged, this,
            [this](machome::bridge::UploadClient::AggregateState state, int healthy, int total,
                   const QString &summary) {
        using State = machome::bridge::UploadClient::AggregateState;
        workState_ = state == State::Healthy ? QStringLiteral("active")
                     : state == State::ScheduledIdle ? QStringLiteral("scheduled_idle")
                     : state == State::Degraded ? QStringLiteral("degraded")
                     : state == State::Offline ? QStringLiteral("blocked") : QStringLiteral("warming");
        telemetry_.insert(QStringLiteral("healthy_workers"), healthy);
        telemetry_.insert(QStringLiteral("total_workers"), total);
        headline_ = summary;
        scheduleSnapshot();
    });
    connect(upload_, &machome::bridge::UploadClient::probeError, this,
            [this](const QString &code, const QString &message) {
        lastError_ = QStringLiteral("%1: %2").arg(code, message);
        publishEvent(QStringLiteral("upload.error"), {{QStringLiteral("code"), code},
...
QJsonObject ModuleWorker::uploadWorkerJson(const machome::bridge::UploadWorkerStatus &worker) const {
    return {{QStringLiteral("source"), worker.source}, {QStringLiteral("pid"), worker.pid},
            {QStringLiteral("state"), worker.state}, {QStringLiteral("stage"), worker.stage},
            {QStringLiteral("updated_at"), iso(worker.updatedAt)},
            {QStringLiteral("last_success_at"), iso(worker.lastSuccessAt)},
            {QStringLiteral("last_failure_at"), iso(worker.lastFailureAt)},
            {QStringLiteral("heartbeat_at"), iso(worker.lastHeartbeatAt)},
            {QStringLiteral("status_age_ms"), worker.statusAgeMs},
            {QStringLiteral("success_age_ms"), worker.successAgeMs},
            {QStringLiteral("status_fresh"), worker.statusFresh},
            {QStringLiteral("success_fresh"), worker.successFresh},
            {QStringLiteral("accepted"), worker.accepted},
            {QStringLiteral("symbols"), QJsonArray::fromStringList(worker.symbols)},
            {QStringLiteral("detail"), worker.detail}, {QStringLiteral("last_error"), worker.lastError}};
}

void ModuleWorker::startPremium() {
    machome::premium::PremiumClient::Config options;
    options.host = config_.settings.value(QStringLiteral("host")).toString(QStringLiteral("127.0.0.1"));
    options.summaryPort = static_cast<quint16>(config_.settings.value(QStringLiteral("summary_port")).toInt(8421));
    options.l1Port = static_cast<quint16>(config_.settings.value(QStringLiteral("l1_port")).toInt(19195));
    options.maximumWebSocketMessageBytes = config_.settings
        .value(QStringLiteral("maximum_message_bytes")).toInteger(512 * 1024);
    premium_ = new machome::premium::PremiumClient(options, this);
    connect(premium_, &machome::premium::PremiumClient::summaryStateChanged, this,
            [this](auto state, const QString &detail) {
        telemetry_.insert(QStringLiteral("summary_channel"), QJsonObject{{QStringLiteral("state"), channelStateName(state)},
                                                                          {QStringLiteral("detail"), detail}});
        markTelemetryObserved(QStringLiteral("summary_channel"));
        updatePremiumBusinessReadiness();
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::detailStateChanged, this,
            [this](auto state, const QString &detail) {
        const QJsonObject payload{{QStringLiteral("state"), channelStateName(state)},
                                  {QStringLiteral("detail"), detail}};
        telemetry_.insert(QStringLiteral("detail_channel"), payload);
        publishEvent(QStringLiteral("premium.detail_state"), payload);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::l1StateChanged, this,
            [this](auto state, const QString &detail) {
        telemetry_.insert(QStringLiteral("l1_channel"), QJsonObject{{QStringLiteral("state"), channelStateName(state)},
                                                                     {QStringLiteral("detail"), detail}});
        markTelemetryObserved(QStringLiteral("l1_channel"));
        updatePremiumBusinessReadiness();
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::freshnessChanged, this,
            [this](bool summaryFresh, bool l1Fresh, qint64 summaryAge, qint64 l1Age) {
        telemetry_.insert(QStringLiteral("freshness"), QJsonObject{{QStringLiteral("summary_fresh"), summaryFresh},
                                                                    {QStringLiteral("l1_fresh"), l1Fresh},
                                                                    {QStringLiteral("summary_age_ms"), summaryAge},
                                                                    {QStringLiteral("l1_age_ms"), l1Age}});
        markTelemetryObserved(QStringLiteral("freshness"));
        updatePremiumBusinessReadiness();
        workState_ = summaryFresh ? QStringLiteral("active") : QStringLiteral("degraded");
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::statusReceived, this, [this](const QJsonObject &status) {
        ...
        if (!desired && !phase.isEmpty()) workState_ = QStringLiteral("scheduled_idle");
        headline_ = phase.isEmpty() ? QStringLiteral("8421 状态已接收") : QStringLiteral("市场阶段 %1").arg(phase);
        finishPending(QStringLiteral("premium_status"), true, QStringLiteral("状态已刷新"), status);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::l1StatusReceived, this, [this](const QJsonObject &status) {
        telemetry_.insert(QStringLiteral("l1_status"), status);
        markTelemetryObserved(QStringLiteral("l1_status"));
        updatePremiumBusinessReadiness();
        publishEvent(QStringLiteral("premium.l1_status"), status);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::summaryReceived, this,
            [this](const QString &symbol, QJsonObject summary) {
        if (!summary.contains(QStringLiteral("symbol"))) summary.insert(QStringLiteral("symbol"), symbol);
        publishEvent(QStringLiteral("premium.summary"), summary);
    });
    connect(premium_, &machome::premium::PremiumClient::symbolRemoved, this,
            [this](const QString &symbol, QJsonObject event) {
        if (!event.contains(QStringLiteral("symbol")))
            event.insert(QStringLiteral("symbol"), symbol);
        publishEvent(QStringLiteral("premium.symbol_removed"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::detailHelloReceived, this,
            [this](const QJsonObject &hello) {
        telemetry_.insert(QStringLiteral("detail_max_symbols"),
                          hello.value(QStringLiteral("max_symbols")));
        publishEvent(QStringLiteral("premium.detail_hello"), hello);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::detailSubscriptionsChanged,
            this, [this](const QStringList &desired,
                         const QStringList &acknowledged) {
        telemetry_.insert(
            QStringLiteral("detail_subscriptions"),
            QJsonObject{{QStringLiteral("desired"),
                         QJsonArray::fromStringList(desired)},
                        {QStringLiteral("acknowledged"),
                         QJsonArray::fromStringList(acknowledged)},
                        {QStringLiteral("limit"), 4}});
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::detailAcknowledged, this,
            [this](const QString &operation, const QString &symbol,
                   QJsonObject acknowledgement) {
        acknowledgement.insert(QStringLiteral("symbol"), symbol);
        publishEvent(QStringLiteral("premium.detail_ack"), acknowledgement);
        const QString key = operation == QStringLiteral("subscribe")
            ? QStringLiteral("premium_detail_subscribe:") + symbol
            : QStringLiteral("premium_detail_unsubscribe:") + symbol;
        finishPending(key, true,
                      operation == QStringLiteral("subscribe")
                          ? QStringLiteral("详情订阅已由 A 端确认")
                          : QStringLiteral("详情退订已由 A 端确认"),
                      acknowledgement);
    });
    connect(premium_, &machome::premium::PremiumClient::detailReceived, this,
            [this](const QString &symbol, QJsonObject detail) {
        if (!detail.contains(QStringLiteral("symbol")))
            detail.insert(QStringLiteral("symbol"), symbol);
        publishEvent(QStringLiteral("premium.detail"), detail);
    });
    connect(premium_, &machome::premium::PremiumClient::detailSubscriptionFailed,
            this, [this](const QString &operation, const QString &symbol,
                         QJsonObject error) {
        error.insert(QStringLiteral("symbol"), symbol);
        const QString key = operation == QStringLiteral("subscribe")
            ? QStringLiteral("premium_detail_subscribe:") + symbol
            : QStringLiteral("premium_detail_unsubscribe:") + symbol;
        finishPending(key, false, QStringLiteral("A 端拒绝详情%1")
                                       .arg(operation == QStringLiteral("subscribe")
                                                ? QStringLiteral("订阅")
                                                : QStringLiteral("退订")),
                      error);
    });
    connect(premium_, &machome::premium::PremiumClient::signalReceived, this,
            [this](const QString &symbol, QJsonObject signal) {
        if (!signal.contains(QStringLiteral("symbol"))) signal.insert(QStringLiteral("symbol"), symbol);
        telemetry_.insert(QStringLiteral("signal_count"), ++signalCount_);
        publishEvent(QStringLiteral("premium.signal"), signal);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::syncBeginReceived, this, [this](const QJsonObject &event) {
        publishEvent(QStringLiteral("premium.sync_begin"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::syncCompleteReceived, this, [this](const QJsonObject &event) {
        publishEvent(QStringLiteral("premium.sync_complete"), event);
        finishPending(QStringLiteral("premium_sync"), true, QStringLiteral("同步完成"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::rawSnapshotReceived, this, [this](const QJsonObject &event) {
        publishEvent(QStringLiteral("premium.raw_snapshot"), event);
        finishPending(QStringLiteral("premium_raw_snapshot"), true, QStringLiteral("原始快照已返回"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::watchlistAcknowledged, this, [this](const QJsonObject &event) {
        const bool accepted = event.value(QStringLiteral("accepted")).toBool();
        if (accepted) telemetry_.insert(QStringLiteral("watchlist"), event.value(QStringLiteral("symbols")));
        publishEvent(QStringLiteral("premium.watchlist_ack"), event);
        finishPending(QStringLiteral("premium_set_watchlist"), accepted,
                      accepted ? QStringLiteral("观察清单已确认")
                               : QStringLiteral("服务端拒绝观察清单，原值保持不变"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::l1HotlistAcknowledged, this, [this](const QJsonObject &event) {
        const bool accepted = event.value(QStringLiteral("accepted")).toBool();
        if (accepted) telemetry_.insert(QStringLiteral("l1_hotlist"), event.value(QStringLiteral("symbols")));
        publishEvent(QStringLiteral("premium.l1_hotlist_ack"), event);
        finishPending(QStringLiteral("premium_set_l1_hotlist"), accepted,
                      accepted ? QStringLiteral("L1 热清单已确认")
                               : QStringLiteral("服务端拒绝 L1 热清单，原值保持不变"), event);
    });
    connect(premium_, &machome::premium::PremiumClient::serverErrorReceived, this,
            [this](const QString &channel, const QJsonObject &error) {
        lastError_ = QStringLiteral("%1: %2").arg(channel, compactJson(error));
        publishEvent(QStringLiteral("premium.error"), error);
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::protocolError, this,
            [this](const QString &channel, const QString &reason) {
        lastError_ = QStringLiteral("%1: %2").arg(channel, reason);
        publishEvent(QStringLiteral("premium.protocol_error"), {{QStringLiteral("channel"), channel},
                                                                 {QStringLiteral("message"), reason}});
        scheduleSnapshot();
    });
    connect(premium_, &machome::premium::PremiumClient::commandRejectedLocally, this,
            [this](const QString &channel, const QString &operation, const QString &reason) {
        static const QHash<QString, QString> pendingKeys{
            {QStringLiteral("sync"), QStringLiteral("premium_sync")},
            {QStringLiteral("raw_snapshot"), QStringLiteral("premium_raw_snapshot")},
            {QStringLiteral("set_watchlist"), QStringLiteral("premium_set_watchlist")},
            {QStringLiteral("set_l1_hotlist"), QStringLiteral("premium_set_l1_hotlist")}};
        const QString message = QStringLiteral("%1 本地拒绝 %2：%3").arg(channel, operation, reason);
        lastError_ = message;
        finishPending(pendingKeys.value(operation), false, message,
                      {{QStringLiteral("channel"), channel},
                       {QStringLiteral("operation"), operation},
                       {QStringLiteral("outcome_uncertain"), false}});
        publishEvent(QStringLiteral("premium.command_rejected"),
                     {{QStringLiteral("channel"), channel},
                      {QStringLiteral("operation"), operation},
                      {QStringLiteral("message"), reason}});
        scheduleSnapshot();
...
void ModuleWorker::startWebull() {
    Machome::Webull::WebullClientConfig options;
    options.apiBaseUrl = QUrl(config_.settings.value(QStringLiteral("api_base_url")).toString());
    options.streamUrl = QUrl(config_.settings.value(QStringLiteral("stream_url")).toString());
    const bool webullControlAllowed = config_.controlEnabled
        && config_.ownership != QStringLiteral("shadow");
    if (webullControlAllowed) {
        options.controlBaseUrl = QUrl(
            config_.settings.value(QStringLiteral("control_base_url")).toString());
    }
    options.symbol = config_.settings.value(QStringLiteral("default_symbol")).toString(QStringLiteral("XOP"));
    options.tokenFile = AppConfig::expandPath(config_.settings.value(QStringLiteral("token_file")).toString());
    if (webullControlAllowed) {
        options.controlTokenFile = AppConfig::expandPath(
            config_.settings.value(QStringLiteral("control_token_file")).toString());
    }
    options.pollIntervalMs = config_.settings.value(QStringLiteral("poll_interval_ms")).toInt(2000);
    options.requestTimeoutMs = config_.settings.value(QStringLiteral("request_timeout_ms")).toInt(3000);
    options.controlRequestTimeoutMs = config_.settings.value(
        QStringLiteral("control_timeout_ms")).toInt(45000);
    options.defaultStaleAfterMs = config_.settings.value(QStringLiteral("freshness_ms")).toInteger(15000);
    webull_ = new WebullClient(options, this);
    connect(webull_, &WebullClient::statusUpdated, this, [this](const Machome::Webull::GatewayStatus &status) {
        ...
        headline_ = status.apiLive ? QStringLiteral("API 在线 · auth %1 · data %2").arg(status.authState, status.dataState)
    ...
    connect(webull_, &WebullClient::bookUpdated, this, [this](const Machome::Webull::BookSnapshot &book) {
    ...
    connect(webull_, &WebullClient::clientsUpdated, this, [this](const Machome::Webull::ClientList &clients) {
        QJsonArray array;
    ...
    connect(webull_, &WebullClient::symbolsUpdated, this, [this](const Machome::Webull::SymbolList &symbols) {
        QJsonArray array;
    ...
    connect(webull_, &WebullClient::logEntry, this, [this](const Machome::Webull::LogEntry &entry) {
    ...
    connect(webull_, &WebullClient::errorOccurred, this, [this](const Machome::Webull::ApiError &error) {
    ...
    connect(webull_, &WebullClient::controlCompleted, this,
    ...
    connect(webull_, &WebullClient::controlFailed, this,
...
void ModuleWorker::verifyWebullControlPostconditions(
    ...
    for (const auto &requestId : requestIds) {
        ...
        const bool freshStatus = probeBarrier >= 0
            && status.statusProbeIssuedMonotonicMs > probeBarrier;
        const bool freshReady = probeBarrier >= 0
        ...
        if (action == QStringLiteral("set_schedule_mode")) {
            ...
            if (mode == QStringLiteral("force_running")) {
                satisfied = satisfied && freshReady && status.apiReady
            ...
            satisfied = freshStatus && freshReady && status.apiLive && status.apiReady
            ...
            satisfied = freshStatus && freshReady && status.apiLive && status.apiReady && status.collectorRunning
                && status.browserState == QStringLiteral("running")
            ...
            // The OS window itself needs the runbook's visual check. The
            // machine-verifiable side effect is that the gateway is alive and
            // the collector that owns the browser is running.
            satisfied = freshStatus && freshReady && status.apiLive
                && status.collectorRunning;
            expectation = QStringLiteral("API 在线且持有登录窗口的 collector 已运行");
        }
...
void ModuleWorker::requestSnapshot() {
    ...
    if (upload_) upload_->refresh();
    if (premium_) {
        premium_->requestStatus();
        premium_->requestL1Status();
    }
...
void ModuleWorker::submitCommand(const QJsonObject &command) {
    ...
    if (premium_) {
        if (action == QStringLiteral("premium_sync")) {
            if (rememberPending(action, command)) premium_->requestSync();
            return;
        }
        if (action == QStringLiteral("premium_raw_snapshot")) {
            if (rememberPending(action, command)) premium_->requestRawSnapshot();
            return;
        }
        if (action == QStringLiteral("premium_detail_subscribe")) {
            ...
            if (rememberPending(action + u':' + symbol, command)) {
                premium_->subscribeDetail(symbol);
            }
            return;
        }
        if (action == QStringLiteral("premium_detail_unsubscribe")) {
            const QString symbol = arguments.value(QStringLiteral("symbol"))
                                       .toString().trimmed().toUpper();
            if (rememberPending(action + u':' + symbol, command)) {
                premium_->unsubscribeDetail(symbol);
            }
            return;
        }
        if (action == QStringLiteral("premium_set_watchlist")) {
            ...
            QStringList symbols;
            for (const auto &value : arguments.value(QStringLiteral("symbols")).toArray()) symbols.push_back(value.toString());
            if (rememberPending(action, command)) premium_->setWatchlist(symbols);
            return;
        }
        if (action == QStringLiteral("premium_set_l1_hotlist")) {
            if (!allowMutation(command)) return;
            QStringList symbols;
            for (const auto &value : arguments.value(QStringLiteral("symbols")).toArray()) symbols.push_back(value.toString());
            if (rememberPending(action, command)) premium_->setL1Hotlist(symbols);
            return;
        }
    }
    if (webull_) {
...
void ModuleWorker::publishSnapshot() {
    ...
    if (config_.adapter == QStringLiteral("upload")) transportAlive = telemetry_.value(QStringLiteral("site")).toObject().value(QStringLiteral("reachable")).toBool();
    else if (config_.adapter == QStringLiteral("premium")) transportAlive = telemetry_.value(QStringLiteral("summary_channel")).toObject().value(QStringLiteral("state")).toString() == QStringLiteral("connected");
    ...
    else transportAlive = telemetry_.value(QStringLiteral("health")).toObject()
                              .value(QStringLiteral("ready")).toBool(false);
    ...
    if (config_.adapter == QStringLiteral("premium")) {
        ...
        if (!config_.settings.value(QStringLiteral("control_base_url")).toString().isEmpty()) {
