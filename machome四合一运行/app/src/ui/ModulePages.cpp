...
void setCell(QTableWidget *table, int row, int column, const QString &value,
    ...
    item->setText(value);
    if (!raw.isEmpty()) {
        item->setData(Qt::UserRole, QString::fromUtf8(QJsonDocument(raw).toJson(QJsonDocument::Compact)));
    }
}
...
void addTreeValue(QTreeWidgetItem *parent, const QString &key, const QJsonValue &value) {
    if (value.isObject()) {
        auto *item = new QTreeWidgetItem(parent, {key, QStringLiteral("{…}")});
        const auto object = value.toObject();
        for (auto it = object.begin(); it != object.end(); ++it) addTreeValue(item, it.key(), it.value());
        return;
    }
    if (value.isArray()) {
        auto *item = new QTreeWidgetItem(parent, {key, QStringLiteral("[%1]").arg(value.toArray().size())});
        int index = 0;
        for (const auto &entry : value.toArray()) addTreeValue(item, QString::number(index++), entry);
        return;
    }
    new QTreeWidgetItem(parent, {key, displayValue(value)});
}
...
QString realtimeStatusText(const QJsonValue &value) {
    ...
    return names.value(state, state.isEmpty() ? QStringLiteral("—") : state);
}
...
QWidget *ModulePage::buildHeader(const QString &subtitle, const QList<QWidget *> &extraActions) {
    ...
    startButton_ = new QPushButton(QStringLiteral("启动服务"));
    stopButton_ = new QPushButton(QStringLiteral("停止服务"));
    restartButton_ = new QPushButton(QStringLiteral("重启服务"));
...
void ModulePage::setContent(QWidget *content) {
    ...
    layout->addWidget(buildHeader(QStringLiteral("独立线程采集；业务进程保持进程隔离")));
...
void ModulePage::updateControlState(const QJsonObject &snapshot) {
    ...
    const bool allowed = payload.value(QStringLiteral("control_enabled")).toBool(config_.controlEnabled) &&
                         payload.value(QStringLiteral("ownership")).toString(config_.ownership) == QStringLiteral("owner");
    for (auto *button : {startButton_, stopButton_, restartButton_}) {
        if (!button) continue;
        button->setEnabled(allowed && !commandBusy_);
        button->setToolTip(allowed ? QStringLiteral("将显示影响范围确认")
                                   : QStringLiteral("需在配置中设置 ownership=owner 且 control_enabled=true"));
    }
}
...
void ModulePage::applySnapshot(const QJsonObject &message) {
    ...
    stateLabel_->setText(workState.isEmpty() ? lifecycle : lifecycle + QStringLiteral(" · ") + workState);
...
void ModulePage::applyEvent(const QJsonObject &message) {
    appendLog(QStringLiteral("%1  %2  %3")
                  .arg(message.value(QStringLiteral("timestamp")).toString(),
                       message.value(QStringLiteral("event_kind")).toString(),
...
void ModulePage::appendLog(const QString &text) {
    ...
    if (eventLog_->document()->blockCount() > 2000) {
        QTextCursor cursor(eventLog_->document());
...
UploadPage::UploadPage(ModuleConfig config, QWidget *parent)
    ...
    workersExpectedLabel_ = textLabel(QStringLiteral("Worker 预期：应运行（fail-closed）"),
    ...
    workersTable_ = makeTable({QStringLiteral("子服务"), QStringLiteral("托管"), QStringLiteral("运行"),
                               QStringLiteral("PID"), QStringLiteral("阶段"), QStringLiteral("Heartbeat"),
                               QStringLiteral("Last success"), QStringLiteral("Last failure"),
                               QStringLiteral("精确操作")});
    tabs->addTab(workersTable_, QStringLiteral("上传器矩阵"));

    auto *web = new QWidget;
    auto *webLayout = new QVBoxLayout(web);
    webLayout->addWidget(textLabel(QStringLiteral("网站继续使用现有 Vue/Go 页面；这里保留所有深链入口。"),
                                   QStringLiteral("secondaryText")));
    auto *home = new QPushButton(QStringLiteral("打开 Upload 网站首页"));
    connect(home, &QPushButton::clicked, this, [this] {
        QDesktopServices::openUrl(QUrl(config_.settings.value(QStringLiteral("web_base_url")).toString()));
    });
    webLayout->addWidget(home);
    auto *fundRow = new QHBoxLayout;
    fundEdit_ = new QLineEdit;
    fundEdit_->setPlaceholderText(QStringLiteral("基金代码，例如 SZ159518"));
    auto *fund = new QPushButton(QStringLiteral("打开基金详情 / 二级历史页"));
    connect(fund, &QPushButton::clicked, this, [this] {
        const QString symbol = fundEdit_->text().trimmed();
        if (symbol.isEmpty()) return;
        QUrl url(config_.settings.value(QStringLiteral("web_base_url")).toString());
        url.setPath(QStringLiteral("/funds/%1").arg(QString::fromUtf8(QUrl::toPercentEncoding(symbol))));
        QDesktopServices::openUrl(url);
    });
    fundRow->addWidget(fundEdit_, 1);
    fundRow->addWidget(fund);
    webLayout->addLayout(fundRow);
    for (const auto &route : {QStringLiteral("/yesterday-redemption"), QStringLiteral("/debug"),
                              QStringLiteral("/visits"), QStringLiteral("/navsettings"),
                              QStringLiteral("/guide"), QStringLiteral("/contact")}) {
        auto *button = new QPushButton(QStringLiteral("打开 %1").arg(route));
        connect(button, &QPushButton::clicked, this, [this, route] {
            QUrl url(config_.settings.value(QStringLiteral("web_base_url")).toString());
            url.setPath(route);
            QDesktopServices::openUrl(url);
        });
        webLayout->addWidget(button);
    }
    webLayout->addStretch();
    tabs->addTab(web, QStringLiteral("网站与二级页面"));
...
void UploadPage::applySnapshot(const QJsonObject &message) {
    ...
    const bool expected = telemetry.value(QStringLiteral("workers_expected")).toBool(
        config_.settings.value(QStringLiteral("workers_expected")).toBool(true));
    workersExpectedLabel_->setText(expected
        ? QStringLiteral("Worker 预期：应运行（fail-closed；超时或缺失将告警）")
        : QStringLiteral("Worker 预期：允许排程休眠（网站健康时显示 scheduled_idle）"));
    const QJsonArray workers = arrayCandidate(telemetry.value(QStringLiteral("workers")));
    const auto units = payload.value(QStringLiteral("process")).toObject().value(QStringLiteral("units")).toArray();
    QHash<QString, QJsonObject> unitMap;
    QHash<QString, QJsonObject> workerMap;
    for (const auto &value : units) {
        const auto object = value.toObject();
        unitMap.insert(object.value(QStringLiteral("id")).toString(), object);
    }
    for (const auto &value : workers) {
        const auto object = value.toObject();
        const QString id = firstValue(object, {QStringLiteral("source"), QStringLiteral("id"),
                                                QStringLiteral("label")}, {});
        if (!id.isEmpty()) workerMap.insert(id, object);
    }
    QStringList rowIds;
    for (const auto &value : units) {
        const QString id = value.toObject().value(QStringLiteral("id")).toString();
        if (!id.isEmpty() && !rowIds.contains(id)) rowIds.append(id);
    }
    for (const auto &value : workers) {
        const auto object = value.toObject();
        const QString id = firstValue(object, {QStringLiteral("source"), QStringLiteral("id"),
    ...
    unitControlButtons_.clear();
    workersTable_->setRowCount(rowIds.size());
    int row = 0;
    for (const auto &id : rowIds) {
        const QJsonObject worker = workerMap.value(id);
        const QJsonObject unit = unitMap.value(id);
        const QJsonObject raw = worker.isEmpty() ? unit : worker;
        setCell(workersTable_, row, 0, id, raw);
        setCell(workersTable_, row, 1, firstValue(unit, {QStringLiteral("loaded")}));
        setCell(workersTable_, row, 2, firstValue(unit, {QStringLiteral("running")}));
        setCell(workersTable_, row, 3, firstValue(unit, {QStringLiteral("pid")}));
        setCell(workersTable_, row, 4, firstValue(worker, {QStringLiteral("stage")}));
        setCell(workersTable_, row, 5, firstValue(worker, {QStringLiteral("heartbeat_at"), QStringLiteral("heartbeat")}));
        setCell(workersTable_, row, 6, firstValue(worker, {QStringLiteral("last_success_at"), QStringLiteral("last_success")}));
        setCell(workersTable_, row, 7, firstValue(worker, {QStringLiteral("last_failure_at"), QStringLiteral("last_failure")}));
        if (!unit.isEmpty()) {
            auto *controls = new QWidget(workersTable_);
            ...
            const QList<QPair<QString, QString>> actions{
                {QStringLiteral("start_service"), QStringLiteral("启动")},
                {QStringLiteral("stop_service"), QStringLiteral("停止")},
                {QStringLiteral("restart_service"), QStringLiteral("重启")}};
            for (const auto &[action, title] : actions) {
                auto *button = new QPushButton(title, controls);
                button->setObjectName(QStringLiteral("uploadUnit_%1_%2").arg(action, id));
                button->setProperty("riskAction", true);
                button->setProperty("lifecycleAction", action);
                button->setToolTip(QStringLiteral("仅作用于配置 unit id: %1").arg(id));
                connect(button, &QPushButton::clicked, this, [this, action, id] {
                    send(action, {{QStringLiteral("target_unit"), id}});
                });
                controlLayout->addWidget(button);
                unitControlButtons_.append(button);
            }
            workersTable_->setCellWidget(row, 8, controls);
        }
        ++row;
    }
    updateBusyControls();
...
void UploadPage::updateBusyControls() {
    ...
    const bool ownerControl = payload.value(QStringLiteral("control_enabled"))
        ...
        && !commandBusy_;
    for (const auto &guard : std::as_const(unitControlButtons_)) {
        if (!guard) continue;
        const QString action = guard->property("lifecycleAction").toString();
        guard->setEnabled(ownerControl && config_.allowedActions.contains(action));
    }
}
...
void PremiumPage::applyEvent(const QJsonObject &message) {
    ...
    if (kind.contains(QStringLiteral("signal"), Qt::CaseInsensitive)) {
        addSignal(payload);
        if (!message.value(QStringLiteral("replayed")).toBool(false)
    ...
    else if (kind.contains(QStringLiteral("summary"), Qt::CaseInsensitive)) upsertSummary(payload);
...
void PremiumPage::addSignal(const QJsonObject &payload) {
...
void PremiumPage::upsertSummary(const QJsonObject &payload) {
    ...
    if (symbol.isEmpty()) return;
    int row = -1;
    const auto found = summariesTable_->findItems(symbol, Qt::MatchExactly);
    if (!found.isEmpty()) row = found.first()->row();
    if (row < 0) {
...
WebullPage::WebullPage(ModuleConfig config, QWidget *parent)
    ...
    symbolEdit_->setToolTip(QStringLiteral("标的由 Webull gateway 配置决定；切换需修改配置并重启 adapter"));
...
void WebullPage::applySnapshot(const QJsonObject &message) {
    ...
    for (auto *button : std::as_const(mutationButtons_)) {
        ...
        button->setToolTip(canMutate
            ...
            : QStringLiteral("需 logic/owner 权限及 control_base_url；shadow 模式保持禁用"));
    }
    ...
    staleLabel_->setText(freshness.contains(QStringLiteral("fresh"), Qt::CaseInsensitive)
                             ? QStringLiteral("FRESH") : QStringLiteral("STALE / %1").arg(freshness));
...
void RealtimePage::applySnapshot(const QJsonObject &message) {
    ...
    for (const auto &window : std::as_const(pcfWindows_)) {
        if (window) window->applyQmtTelemetry(qmtBackends_);
    }
}
...
void PcfDetailWindow::setMutationEnabled(bool enabled) {
    ...
    for (auto it = qmtWidgets_.begin(); it != qmtWidgets_.end(); ++it) {
        ...
        for (auto *button : {widgets.connectButton, widgets.disconnectButton, widgets.syncButton,
                             widgets.purchaseButton, widgets.redeemButton}) {
            if (!button) continue;
...
void PcfDetailWindow::updateQmtTab(const QString &backend, const QJsonObject &snapshot) {
    ...
    widgets.state->setText(QStringLiteral("%1 · %2 · 可用资金 %3 · 节流剩余 %4 ms")
                               .arg(backend, ready ? QStringLiteral("READY") : state, cash,
