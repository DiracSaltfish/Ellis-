...
MainWindow::MainWindow(AppConfig config, QString configPath, QString agentProgram, QWidget *parent)
    ...
    for (const auto &module : config_.modules) {
        ...
        connect(page, &ModulePage::alertRequested, this,
                [this](const QString &moduleId, const QString &title, const QString &message,
...
void MainWindow::updateCard(const QString &moduleId, const QJsonObject &message) {
    ...
    const auto process = payload.value(QStringLiteral("process")).toObject();
    ...
    card.state->setText(workState.isEmpty() ? lifecycle : lifecycle + QStringLiteral(" · ") + workState);
    ...
    if (moduleId == QStringLiteral("upload")) {
        metric = QStringLiteral("上传器 %1/%2 · HTTP %3")
                     .arg(firstText(process, {QStringLiteral("running_count")}, QStringLiteral("0")),
                          firstText(process, {QStringLiteral("total_count")}, QStringLiteral("0")),
                          firstText(telemetry, {QStringLiteral("site.http_status"), QStringLiteral("site.status")}, QStringLiteral("—")));
    } else if (moduleId == QStringLiteral("premium")) {
        metric = QStringLiteral("ready %1/%2 · signals %3")
                     .arg(firstText(telemetry, {QStringLiteral("status.ready_count"), QStringLiteral("status.ready")}),
                          firstText(telemetry, {QStringLiteral("status.watch_count"), QStringLiteral("status.watch")}),
                          firstText(telemetry, {QStringLiteral("signal_count")}, QStringLiteral("0")));
    } else if (moduleId == QStringLiteral("webull")) {
        metric = QStringLiteral("auth %1 · data %2 · clients %3")
                     .arg(firstText(telemetry, {QStringLiteral("status.auth_state"), QStringLiteral("status.auth.state")}),
                          firstText(telemetry, {QStringLiteral("status.data_state"), QStringLiteral("status.data.state")}),
                          firstText(telemetry, {QStringLiteral("client_count")}, QStringLiteral("0")));
    } else {
        metric = QStringLiteral("monitor %1 · Wind %2 · symbols %3")
                     .arg(firstText(telemetry, {QStringLiteral("health.monitoring"), QStringLiteral("snapshot.monitoring")}),
                          firstText(telemetry, {QStringLiteral("health.wind_state"), QStringLiteral("health.wind.state")}),
...
void MainWindow::appendEventRow(const QJsonObject &message, const QString &kindOverride) {
    ...
    const QStringList values{message.value(QStringLiteral("timestamp")).toString(),
                             ...
                             message.value(QStringLiteral("state")).toString(), content.left(500)};
    for (int column = 0; column < values.size(); ++column) events_->setItem(0, column, new QTableWidgetItem(values[column]));
    while (events_->rowCount() > 500) events_->removeRow(events_->rowCount() - 1);
}
...
void MainWindow::requestCommand(const QString &moduleId, const QString &action,
    ...
    if (highRisk) {
        ...
        const QString impact = action == QStringLiteral("redemption_qmt_order")
                                   ...
                                   ? QStringLiteral("原 Webull runtime 在采集未运行时会切换到 force_running 并启动采集，然后打开登录窗口。")
