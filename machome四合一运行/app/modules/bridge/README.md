...
Hub 通过模块设置 `workers_expected` 显式传入这个状态，并在 Upload 页面展示。默认值是 `true`（fail-closed）：因为各 uploader 有不同市场和任务时段，Hub 不根据本机时钟自行猜测。只有部署排程已明确判定当前允许休眠时才应设为 `false`；未来接入动态排程器时，仍应将“每个 source 是否应运行”作为明确输入，而非由健康文件反推。
