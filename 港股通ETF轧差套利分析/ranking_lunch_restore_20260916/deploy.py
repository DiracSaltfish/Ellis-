...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('ranking-session-restore-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='ranking-session-restore-20260916',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Restore shared-session ranking on lunch or after-close restart with archived as-of FX and PCF',ranking_source=expected['source_hashes'])
