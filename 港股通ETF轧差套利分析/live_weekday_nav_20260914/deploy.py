...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('weekday-nav-fix-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='weekday-nav-fix-20260914',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Previous and next date controls skip Saturday and Sunday',ranking_source=expected['source_hashes'])
