...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('hk-no-trade-240s-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='hk-no-trade-240s-20260916',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Accept same-day HK last-trade quotes with source receipt age up to 240 seconds, for internal L1 and Sina consistently',ranking_source=expected['source_hashes'])
