...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('ranking-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='premium-ranking-watchlist-20260912',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Minute premium screen; three daily ranking snapshots; 197 watchlist, QC annotation only',ranking_source=expected['source_hashes'])
 ...
 rank=get('/api/v1/ranking');assert rank['version']=='premium-screen.v1'
 ...
 page=urllib.request.urlopen('http://127.0.0.1/ranking').read().decode();assert '每日溢价排名' in page
 result={'backup':str(backup),'run_id':h['run_id'],'candidates':197,'added':76,'sh':sum(r['symbol'].startswith('5') for r in rows),'history_unchanged':len(hist),'ranking':rank,'helper_sha256':sha(helper)}
