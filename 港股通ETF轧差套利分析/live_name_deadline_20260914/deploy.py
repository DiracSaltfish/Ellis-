...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('name-deadline-fix-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='name-deadline-fix-20260914',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Append creation purchase deadline to original fund display names; remove header brand',ranking_source=expected['source_hashes'])
