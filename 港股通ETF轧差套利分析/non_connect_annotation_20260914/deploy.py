...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('non-connect-display-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='non-connect-display-20260914',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Display non-buy-eligible HK constituents using official Shanghai/Shenzhen union; no formula or scoring changes',ranking_source=expected['source_hashes'])
