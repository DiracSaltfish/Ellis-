...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('ui-format-fix-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='ui-format-fix-20260914',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Integer book lots, basket units in ten thousands, hide empty component issues',ranking_source=expected['source_hashes'])
