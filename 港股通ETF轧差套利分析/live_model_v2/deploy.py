...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('model-inference-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='premium-model-inference-20260912',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Native tree inference, 29 quality-controlled features, T-day predicted FX, frozen 1445 primary model',ranking_source=expected['source_hashes'])
