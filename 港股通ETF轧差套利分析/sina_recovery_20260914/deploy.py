...
backup=Path('/Users/ellis/Library/Application Support/MachomeHub/backups')/time.strftime('sina-recovery-fix-%Y%m%d-%H%M%S');backup.mkdir(parents=True)
...
try:
 ...
 m=json.loads(manifest.read_text());m['assets']=expected['assets'];m['web_assets']=expected['assets'];m.update(patch='sina-recovery-fix-20260914',helper_sha256=sha(helper),universe_sha256=sha(uni),changes='Freeze daily Sina plan; distinguish last trade age from response freshness; persistent rejection reasons; tagged historical recovery',ranking_source=expected['source_hashes'])
